...
struct holy_font_node *holy_font_list;

static int register_font (holy_font_t font);
static void font_init (holy_font_t font);
static void free_font (holy_font_t font);
static void remove_font (holy_font_t font);
...
static holy_uint8_t unknown_glyph_bitmap[] = {
  /*       76543210 */
  0x7C,				/*  ooooo   */
  0x82,				/* o     o  */
  0xBA,				/* o ooo o  */
...
static struct holy_font_glyph *unknown_glyph;
...
/* The font structure used when no other font is loaded.  This functions
   ...
   check for a NULL holy_font_t to avoid dereferencing a null pointer.  */
static struct holy_font null_font;

/* Flag to ensure module is initialized only once.  */
static holy_uint8_t font_loader_initialized;

#if HAVE_FONT_SOURCE
static struct holy_font_glyph *ascii_font_glyph[0x80];
#endif

static struct holy_font_glyph *
ascii_glyph_lookup (holy_uint32_t code)
{
#if HAVE_FONT_SOURCE
  ...
  if (ascii_failback_initialized == 0)
      ...
      for (current = 0; current < 0x80; current++)
	{
	  ascii_font_glyph[current] =
	    holy_malloc (sizeof (struct holy_font_glyph) + ASCII_BITMAP_SIZE);
	  ...
	  holy_memcpy (ascii_font_glyph[current]->bitmap,
...
holy_font_loader_init (void)
{
  ...
  unknown_glyph = holy_malloc (sizeof (struct holy_font_glyph)
  ...
  holy_memcpy (unknown_glyph->bitmap,
...
/* Open the next section in the file.
   ...
   holy_errno is set appropriately with an error message.

   If 1 is returned due to being at the end of the file, then section->eof is
   set to 1; otherwise, section->eof is set to 0.  */
static int
open_section (holy_file_t file, struct font_file_section *section)
{
  holy_ssize_t retval;
  holy_uint32_t raw_length;

  section->file = file;
  section->eof = 0;

  /* Read the FOURCC section name.  */
  retval = holy_file_read (file, section->name, 4);
  ...
  retval = holy_file_read (file, &raw_length, 4);
  if (retval >= 0 && retval < 4)
    {
      /* EOF encountered.  */
      section->eof = 1;
  ...
  section->length = holy_be_to_cpu32 (raw_length);
...
/* Load the character index (CHIX) section contents from the font file.  This
   ...
   holy_errno is set appropriately).  */
static int
load_font_index (holy_file_t file, holy_uint32_t sect_length, struct
		 holy_font *font)
{
  unsigned i;
  holy_uint32_t last_code;

#if FONT_DEBUG >= 2
  holy_dprintf ("font", "load_font_index(sect_length=%d)\n", sect_length);
#endif

  /* Sanity check: ensure section length is divisible by the entry size.  */
  if ((sect_length % FONT_CHAR_INDEX_ENTRY_SIZE) != 0)
    {
      holy_error (holy_ERR_BAD_FONT,
  ...
  font->char_index = holy_malloc (font->num_chars
				  * sizeof (struct char_index_entry));
  if (!font->char_index)
    return 1;
  font->bmp_idx = holy_malloc (0x10000 * sizeof (holy_uint16_t));
  if (!font->bmp_idx)
    return 1;
  holy_memset (font->bmp_idx, 0xff, 0x10000 * sizeof (holy_uint16_t));


#if FONT_DEBUG >= 2
  holy_dprintf ("font", "num_chars=%d)\n", font->num_chars);
#endif
  ...
  for (i = 0; i < font->num_chars; i++)
    {
      ...
      if (holy_file_read (file, &entry->code, 4) != 4)
	return 1;
      entry->code = holy_be_to_cpu32 (entry->code);
	...
	font->bmp_idx[entry->code] = i;
      ...
      if (holy_file_read (file, &entry->storage_flags, 1) != 1)
	return 1;

      /* Read glyph data offset; convert to native byte order.  */
      if (holy_file_read (file, &entry->offset, 4) != 4)
	return 1;
      entry->offset = holy_be_to_cpu32 (entry->offset);
...
read_section_as_string (struct font_file_section *section)
{
  ...
  str = holy_malloc (section->length + 1);
  if (!str)
    return 0;

  ret = holy_file_read (section->file, str, section->length);
  if (ret < 0 || ret != (holy_ssize_t) section->length)
    {
      holy_free (str);
      return 0;
    }
		       ...
		       holy_int16_t * value)
{
  ...
  if (holy_file_read (section->file, &raw_value, 2) != 2)
    return 1;

  *value = holy_be_to_cpu16 (raw_value);
  return 0;
}
...
/* Load a font and add it to the beginning of the global font list.
   Returns 0 upon success, nonzero upon failure.  */
holy_font_t
holy_font_load (const char *filename)
{
  holy_file_t file = 0;
...
#if FONT_DEBUG >= 1
  holy_dprintf ("font", "add_font(%s)\n", filename);
#endif

  if (filename[0] == '(' || filename[0] == '/' || filename[0] == '+')
    file = holy_buffile_open (filename, 1024);
  else
    {
      ...
      if (!prefix)
	{
	  holy_error (holy_ERR_FILE_NOT_FOUND, N_("variable `%s' isn't set"),
		      "prefix");
	  goto fail;
	}
      fullname = holy_malloc (holy_strlen (prefix) + holy_strlen (filename) + 1
			      + sizeof ("/fonts/") + sizeof (".pf2"));
      if (!fullname)
	goto fail;
      ptr = holy_stpcpy (fullname, prefix);
      ptr = holy_stpcpy (ptr, "/fonts/");
      ptr = holy_stpcpy (ptr, filename);
      ptr = holy_stpcpy (ptr, ".pf2");
      *ptr = 0;
      file = holy_buffile_open (fullname, 1024);
      holy_free (fullname);
    }
  if (!file)
    goto fail;
...
#if FONT_DEBUG >= 3
  holy_dprintf ("font", "opened FILE section\n");
#endif
  if (holy_memcmp (section.name, FONT_FORMAT_SECTION_NAMES_FILE,
...
#if FONT_DEBUG >= 3
  holy_dprintf ("font", "section name ok\n");
#endif
  if (section.length != 4)
    {
      holy_error (holy_ERR_BAD_FONT,
...
#if FONT_DEBUG >= 3
  holy_dprintf ("font", "section length ok\n");
#endif
  /* Check the file format type code.  */
  if (holy_file_read (file, magic, 4) != 4)
    goto fail;

#if FONT_DEBUG >= 3
  holy_dprintf ("font", "read magic ok\n");
#endif

  if (holy_memcmp (magic, FONT_FORMAT_PFF2_MAGIC, 4) != 0)
    {
      holy_error (holy_ERR_BAD_FONT, "invalid font magic %x %x %x %x",
		  magic[0], magic[1], magic[2], magic[3]);
      goto fail;
    }

#if FONT_DEBUG >= 3
  holy_dprintf ("font", "compare magic ok\n");
#endif

  /* Allocate the font object.  */
  font = (holy_font_t) holy_zalloc (sizeof (struct holy_font));
  if (!font)
    goto fail;
...
#if FONT_DEBUG >= 3
  holy_dprintf ("font", "allocate font ok; loading font info\n");
#endif
  ...
  while (1)
...
#if FONT_DEBUG >= 2
      holy_dprintf ("font", "opened section %c%c%c%c ok\n",
		   section.name[0], section.name[1],
		   section.name[2], section.name[3]);
#endif

      if (holy_memcmp (section.name, FONT_FORMAT_SECTION_NAMES_FONT_NAME,
		       sizeof (FONT_FORMAT_SECTION_NAMES_FONT_NAME) - 1) == 0)
	{
      ...
      else if (holy_memcmp (section.name,
			    ...
			    1) == 0)
	{
      ...
      else if (holy_memcmp (section.name, FONT_FORMAT_SECTION_NAMES_WEIGHT,
	       ...
	       == 0)
	{
	  ...
	  if (holy_strcmp (wt, "normal") == 0)
	    font->weight = FONT_WEIGHT_NORMAL;
	  else if (holy_strcmp (wt, "bold") == 0)
	       ...
	       == 0)
	{
      ...
      else if (holy_memcmp (section.name, FONT_FORMAT_SECTION_NAMES_DESCENT,
			    ...
			    1) == 0)
	{
      ...
      else if (holy_memcmp (section.name, FONT_FORMAT_SECTION_NAMES_DATA,
...
#if FONT_DEBUG >= 3
	  holy_dprintf ("font", "Unhandled section type, skipping.\n");
#endif
	  holy_off_t section_end = holy_file_tell (file) + section.length;
	  if ((int) holy_file_seek (file, section_end) == -1)
	    goto fail;
	}
    }
  ...
  if (!font->name)
    {
      holy_dprintf ("font", "Font has no name.\n");
      font->name = holy_strdup ("Unknown");
    }

#if FONT_DEBUG >= 1
  holy_dprintf ("font", "Loaded font `%s'.\n"
...
static int
read_be_uint16 (holy_file_t file, holy_uint16_t * value)
{
  if (holy_file_read (file, value, 2) != 2)
    return 1;
  *value = holy_be_to_cpu16 (*value);
  return 0;
}

static int
read_be_int16 (holy_file_t file, holy_int16_t * value)
{
  /* For the signed integer version, use the same code as for unsigned.  */
  return read_be_uint16 (file, (holy_uint16_t *) value);
}
...
find_glyph (const holy_font_t font, holy_uint32_t code)
{
...
static struct holy_font_glyph *
holy_font_get_glyph_internal (holy_font_t font, holy_uint32_t code)
{
  ...
  if (index_entry)
    {
      struct holy_font_glyph *glyph = 0;
      holy_uint16_t width;
	...
	return 0;
      ...
      holy_file_seek (font->file, index_entry->offset);
	  ...
	  || read_be_int16 (font->file, &dwidth) != 0)
	{
      ...
      glyph = holy_malloc (sizeof (struct holy_font_glyph) + len);
      if (!glyph)
	{
	  remove_font (font);
	  return 0;
      ...
      if (len != 0)
	{
	  if (holy_file_read (font->file, glyph->bitmap, len) != len)
	    {
	      remove_font (font);
	      holy_free (glyph);
	      return 0;
...
static void
free_font (holy_font_t font)
{
  if (font)
    {
      if (font->file)
	holy_file_close (font->file);
      holy_free (font->name);
      holy_free (font->family);
      holy_free (font->char_index);
      holy_free (font->bmp_idx);
      holy_free (font);
    }
}
...
static int
register_font (holy_font_t font)
{
  struct holy_font_node *node = 0;

  node = holy_malloc (sizeof (struct holy_font_node));
  if (!node)
    return 1;

  node->value = font;
  node->next = holy_font_list;
  holy_font_list = node;
...
static void
remove_font (holy_font_t font)
{
  struct holy_font_node **nextp, *cur;

  for (nextp = &holy_font_list, cur = *nextp;
...
holy_font_t
holy_font_get (const char *font_name)
{
  ...
  for (node = holy_font_list; node; node = node->next)
    {
      ...
      if (holy_strcmp (font->name, font_name) == 0)
	return font;
    }

  /* If no font by that name is found, return the first font in the list
     as a fallback.  */
  if (holy_font_list && holy_font_list->value)
    return holy_font_list->value;
  else
...
const char *
holy_font_get_name (holy_font_t font)
{
  return font->name;
}
...
int
holy_font_get_max_char_width (holy_font_t font)
{
...
int
holy_font_get_descent (holy_font_t font)
{
  return font->descent;
}
...
int
holy_font_get_xheight (holy_font_t font)
{
...
int
holy_font_get_leading (holy_font_t font)
{
  return font->leading;
}
...
int
holy_font_get_height (holy_font_t font)
{
...
holy_font_get_glyph (holy_font_t font, holy_uint32_t code)
{
  struct holy_font_glyph *glyph = 0;
  if (font)
    glyph = holy_font_get_glyph_internal (font, code);
  if (glyph == 0)
    {
...
static int
get_font_diversity (holy_font_t a, holy_font_t b)
{
  int d;
  ...
  if (a->ascent && b->ascent)
    d += holy_abs (a->ascent - b->ascent) * 8;
  else
    /* Penalty for missing attributes.  */
    d += 50;

  if (a->max_char_height && b->max_char_height)
    d += holy_abs (a->max_char_height - b->max_char_height) * 8;
  else
    /* Penalty for missing attributes.  */
    d += 50;
...
struct holy_font_glyph *
holy_font_get_glyph_with_fallback (holy_font_t font, holy_uint32_t code)
{
  struct holy_font_glyph *glyph;
  struct holy_font_node *node;
  /* Keep track of next node, in case there's an I/O error in
     holy_font_get_glyph_internal() and the font is removed from the list.  */
  ...
  int best_diversity;
  struct holy_font_glyph *best_glyph;
  ...
  for (node = holy_font_list; node; node = next)
    {
      holy_font_t curfont;
      ...
      glyph = holy_font_get_glyph_internal (curfont, code);
      if (glyph && !font)
	return glyph;
      if (glyph)
	{
...
#if 0
static struct holy_font_glyph *
holy_font_dup_glyph (struct holy_font_glyph *glyph)
{
  static struct holy_font_glyph *ret;
  ret = holy_malloc (sizeof (*ret) + (glyph->width * glyph->height + 7) / 8);
  if (!ret)
    return NULL;
  holy_memcpy (ret, glyph, sizeof (*ret)
...
/* FIXME: suboptimal.  */
static void
holy_font_blit_glyph (struct holy_font_glyph *target,
		      struct holy_font_glyph *src, unsigned dx, unsigned dy)
{
...
static void
holy_font_blit_glyph_mirror (struct holy_font_glyph *target,
			     struct holy_font_glyph *src,
...
struct blit_comb_ctx
{
  struct holy_font_glyph *glyph;
  int *device_width;
  struct holy_video_signed_rect bounds;
};

/* Helper for blit_comb.  */
static void
do_blit (struct holy_font_glyph *src, signed dx, signed dy,
	 struct blit_comb_ctx *ctx)
{
  if (ctx->glyph)
    holy_font_blit_glyph (ctx->glyph, src, dx - ctx->glyph->offset_x,
...
static void
blit_comb (const struct holy_unicode_glyph *glyph_id,
	   struct holy_font_glyph *glyph,
	   struct holy_video_signed_rect *bounds_out,
	   struct holy_font_glyph *main_glyph,
	   struct holy_font_glyph **combining_glyphs, int *device_width)
{
  struct blit_comb_ctx ctx = {
    .glyph = glyph,
  ...
  const struct holy_unicode_combining *comb;
  ...
  comb = holy_unicode_get_comb (glyph_id);

  for (i = 0; i < glyph_id->ncomb; i++)
    {
      holy_int16_t space = 0;
      /* Center by default.  */
      holy_int16_t targetx;
	...
	continue;
      ...
      if (comb[i].code
	  == holy_UNICODE_COMBINING_GRAPHEME_JOINER)
	continue;
      switch (comb[i].type)
	{
	case holy_UNICODE_COMB_OVERLAY:
	...
	case holy_UNICODE_COMB_SYRIAC_SUPERSCRIPT_ALAPH:
	case holy_UNICODE_COMB_ARABIC_DAMMAH:
	case holy_UNICODE_COMB_ARABIC_DAMMATAN:
	case holy_UNICODE_COMB_ARABIC_FATHATAN:
	case holy_UNICODE_COMB_ARABIC_FATHAH:
	case holy_UNICODE_COMB_ARABIC_SUPERSCRIPT_ALIF:
	case holy_UNICODE_COMB_ARABIC_SUKUN:
	case holy_UNICODE_COMB_ARABIC_SHADDA:
	case holy_UNICODE_COMB_HEBREW_RAFE:
	case holy_UNICODE_STACK_ABOVE:
	stacked_above:
	  space = combining_glyphs[i]->offset_y
	    - holy_font_get_xheight (combining_glyphs[i]->font) - 1;
	  if (space <= 0)
	    space = 1 + (holy_font_get_xheight (main_glyph->font)) / 8;
	  /* Fallthrough.  */
	case holy_UNICODE_STACK_ATTACHED_ABOVE:
	  do_blit (combining_glyphs[i], targetx,
	...
	case holy_UNICODE_COMB_HEBREW_DAGESH:
	  do_blit (combining_glyphs[i], targetx,
	...
	case holy_UNICODE_COMB_HEBREW_SHEVA:
	case holy_UNICODE_COMB_HEBREW_HIRIQ:
	case holy_UNICODE_COMB_HEBREW_QAMATS:
	case holy_UNICODE_COMB_HEBREW_TSERE:
	case holy_UNICODE_COMB_HEBREW_SEGOL:
	  /* TODO: placement in final kaf and under reish.  */

	case holy_UNICODE_COMB_HEBREW_HATAF_SEGOL:
	case holy_UNICODE_COMB_HEBREW_HATAF_PATAH:
	case holy_UNICODE_COMB_HEBREW_HATAF_QAMATS:
	case holy_UNICODE_COMB_HEBREW_PATAH:
	case holy_UNICODE_COMB_HEBREW_QUBUTS:
	case holy_UNICODE_COMB_HEBREW_METEG:
	  /* TODO: Put kasra and kasratan under shadda.  */
	case holy_UNICODE_COMB_ARABIC_KASRA:
	case holy_UNICODE_COMB_ARABIC_KASRATAN:
	  /* I don't know how ypogegrammeni differs from subscript. */
	case holy_UNICODE_COMB_YPOGEGRAMMENI:
	case holy_UNICODE_STACK_BELOW:
	stacked_below:
	  space = -(combining_glyphs[i]->offset_y
	...
	case holy_UNICODE_STACK_ATTACHED_BELOW:
	...
	case holy_UNICODE_COMB_MN:
	  switch (comb[i].code)
	    {
	    case holy_UNICODE_THAANA_ABAFILI:
	    case holy_UNICODE_THAANA_AABAAFILI:
	    case holy_UNICODE_THAANA_UBUFILI:
	    case holy_UNICODE_THAANA_OOBOOFILI:
	    case holy_UNICODE_THAANA_EBEFILI:
	    case holy_UNICODE_THAANA_EYBEYFILI:
	    case holy_UNICODE_THAANA_OBOFILI:
	    case holy_UNICODE_THAANA_OABOAFILI:
	    case holy_UNICODE_THAANA_SUKUN:
	      goto stacked_above;
	    case holy_UNICODE_THAANA_IBIFILI:
	    case holy_UNICODE_THAANA_EEBEEFILI:
	      goto stacked_below;
	    }
	  /* Fall through.  */
	default:
...
static struct holy_font_glyph *
holy_font_construct_dry_run (holy_font_t hinted_font,
			     const struct holy_unicode_glyph *glyph_id,
			     struct holy_video_signed_rect *bounds,
			     struct holy_font_glyph **combining_glyphs,
			     int *device_width)
{
  struct holy_font_glyph *main_glyph = NULL;
  holy_uint32_t desired_attributes = 0;
  unsigned i;
  holy_uint32_t base = glyph_id->base;
  const struct holy_unicode_combining *comb;

  if (glyph_id->attributes & holy_UNICODE_GLYPH_ATTRIBUTE_RIGHT_JOINED)
    desired_attributes |= holy_FONT_CODE_RIGHT_JOINED;

  if (glyph_id->attributes & holy_UNICODE_GLYPH_ATTRIBUTE_LEFT_JOINED)
    desired_attributes |= holy_FONT_CODE_LEFT_JOINED;
  ...
  if (base == 'i' || base == 'j')
    {
      for (i = 0; i < glyph_id->ncomb; i++)
	if (comb[i].type == holy_UNICODE_STACK_ABOVE)
	  break;
      if (i < glyph_id->ncomb && base == 'i')
	base = holy_UNICODE_DOTLESS_LOWERCASE_I;
      if (i < glyph_id->ncomb && base == 'j')
	base = holy_UNICODE_DOTLESS_LOWERCASE_J;
    }

  main_glyph = holy_font_get_glyph_with_fallback (hinted_font, base
						  | desired_attributes);

  if (!main_glyph)
    main_glyph = holy_font_get_glyph_with_fallback (hinted_font,
						    base);
  ...
  for (i = 0; i < glyph_id->ncomb; i++)
    combining_glyphs[i]
      = holy_font_get_glyph_with_fallback (main_glyph->font,
					   comb[i].code);
...
static struct holy_font_glyph **render_combining_glyphs = 0;
static holy_size_t render_max_comb_glyphs = 0;

static void
ensure_comb_space (const struct holy_unicode_glyph *glyph_id)
{
  ...
  holy_free (render_combining_glyphs);
  render_combining_glyphs = holy_malloc (render_max_comb_glyphs
...
int
holy_font_get_constructed_device_width (holy_font_t hinted_font,
					const struct holy_unicode_glyph
					*glyph_id)
{
  int ret;
  struct holy_font_glyph *main_glyph;
  ...
  main_glyph = holy_font_construct_dry_run (hinted_font, glyph_id, NULL,
...
struct holy_font_glyph *
holy_font_construct_glyph (holy_font_t hinted_font,
			   const struct holy_unicode_glyph *glyph_id)
{
  struct holy_font_glyph *main_glyph;
  struct holy_video_signed_rect bounds;
  static struct holy_font_glyph *glyph = 0;
  static holy_size_t max_glyph_size = 0;

  ensure_comb_space (glyph_id);

  main_glyph = holy_font_construct_dry_run (hinted_font, glyph_id,
  ...
  if (max_glyph_size < sizeof (*glyph) + (bounds.width * bounds.height + holy_CHAR_BIT - 1) / holy_CHAR_BIT)
    {
      holy_free (glyph);
      max_glyph_size = (sizeof (*glyph) + (bounds.width * bounds.height + holy_CHAR_BIT - 1) / holy_CHAR_BIT) * 2;
      if (max_glyph_size < 8)
	max_glyph_size = 8;
      glyph = holy_malloc (max_glyph_size);
    }
  if (!glyph)
    {
      holy_errno = holy_ERR_NONE;
  ...
  if (glyph_id->attributes & holy_UNICODE_GLYPH_ATTRIBUTE_MIRROR)
    holy_font_blit_glyph_mirror (glyph, main_glyph,
				 main_glyph->offset_x - glyph->offset_x,
				 (glyph->height + glyph->offset_y)
				 - (main_glyph->height +
				    main_glyph->offset_y));
  else
    holy_font_blit_glyph (glyph, main_glyph,
			  main_glyph->offset_x - glyph->offset_x,
			  (glyph->height + glyph->offset_y)
...
holy_err_t
holy_font_draw_glyph (struct holy_font_glyph * glyph,
		      holy_video_color_t color, int left_x, int baseline_y)
{
  struct holy_video_bitmap glyph_bitmap;
  ...
  glyph_bitmap.mode_info.mode_type
    = (1 << holy_VIDEO_MODE_TYPE_DEPTH_POS) | holy_VIDEO_MODE_TYPE_1BIT_BITMAP;
  glyph_bitmap.mode_info.blit_format = holy_VIDEO_BLIT_FORMAT_1BIT_PACKED;
  ...
  return holy_video_blit_bitmap (&glyph_bitmap, holy_VIDEO_BLIT_BLEND,
