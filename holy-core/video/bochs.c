...
static void
vbe_write (holy_uint16_t val, holy_uint16_t addr)
{
  holy_outw (addr, BOCHS_VBE_INDEX);
  holy_outw (val, BOCHS_VBE_DATA);
}

static holy_uint16_t
vbe_read (holy_uint16_t addr)
{
  holy_outw (addr, BOCHS_VBE_INDEX);
  return holy_inw (BOCHS_VBE_DATA);
}
...
struct saved_state
{
  ...
  holy_uint8_t g[256];
  holy_uint8_t b[256];
  holy_uint8_t vbe[BOCHS_VBE_MAX];
  int vbe_enable;
...
save_state (struct saved_state *st)
{
  ...
  holy_vga_sr_write (st->sr[holy_VGA_SR_MEMORY_MODE], holy_VGA_SR_MEMORY_MODE);
}
...
static holy_err_t
holy_video_bochs_video_init (void)
{
  /* Reset frame buffer.  */
...
static holy_err_t
holy_video_bochs_video_fini (void)
{
...
static holy_err_t
holy_video_bochs_set_palette (unsigned int start, unsigned int count,
...
/* Helper for holy_video_bochs_setup.  */
static int
...
static holy_err_t
holy_video_bochs_setup (unsigned int width, unsigned int height,
			...
			holy_video_mode_type_t mode_mask)
{
  ...
  if (width > BOCHS_MAX_WIDTH)
    return holy_error (holy_ERR_IO, "width must be at most",
		       BOCHS_MAX_WIDTH);

  if (height > BOCHS_MAX_HEIGHT)
    return holy_error (holy_ERR_IO, "height must be at most",
		       BOCHS_MAX_HEIGHT);

  if (width & (BOCHS_WIDTH_ALIGN - 1))
    return holy_error (holy_ERR_IO, "width must be a multiple of %d",
		       BOCHS_WIDTH_ALIGN);
  ...
  if (depth != 32 && depth != 24 && depth != 16 && depth != 15 && depth != 8
      && depth != 4)
    return holy_error (holy_ERR_IO, "only 32, 24, 16, 15 and 8-bpp are"
		       " supported by bochs video");

  if (depth == 4)
    return holy_error (holy_ERR_NOT_IMPLEMENTED_YET, "4-bpp isn't cupported");
  ...
  err = holy_video_bochs_set_palette (0, holy_VIDEO_FBSTD_NUMCOLORS,
				      holy_video_fbstd_colors);
  return err;
}

static struct holy_video_adapter holy_video_bochs_adapter =
  {
    .name = "Bochs PCI Video Driver",
    .id = holy_VIDEO_DRIVER_BOCHS,

    .prio = holy_VIDEO_ADAPTER_PRIO_NATIVE,

    .init = holy_video_bochs_video_init,
    .fini = holy_video_bochs_video_fini,
    .setup = holy_video_bochs_setup,
    .get_info = holy_video_fb_get_info,
    .get_info_and_fini = holy_video_fb_get_info_and_fini,
    .set_palette = holy_video_bochs_set_palette,
...
holy_MOD_INIT(video_bochs)
{
  holy_video_register (&holy_video_bochs_adapter);
}

holy_MOD_FINI(video_bochs)
{
  holy_video_unregister (&holy_video_bochs_adapter);
}
