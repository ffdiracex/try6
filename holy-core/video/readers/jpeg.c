...
static const holy_uint8_t jpeg_zigzag_order[64] = {
  0, 1, 8, 16, 9, 2, 3, 10,
...
struct holy_jpeg_data
{
  holy_file_t file;
  struct holy_video_bitmap **bitmap;
  holy_uint8_t *bitmap_ptr;

  unsigned image_width;
  unsigned image_height;

  holy_uint8_t *huff_value[4];
  int huff_offset[4][16];
  int huff_maxval[4][16];

  holy_uint8_t quan_table[2][64];
  int comp_index[3][3];
...
static holy_uint8_t
holy_jpeg_get_byte (struct holy_jpeg_data *data)
{
  holy_uint8_t r;
...
holy_jpeg_get_word (struct holy_jpeg_data *data)
{
  holy_uint16_t r;

  r = 0;
  holy_file_read (data->file, &r, sizeof (holy_uint16_t));

  return holy_be_to_cpu16 (r);
}

static int
holy_jpeg_get_bit (struct holy_jpeg_data *data)
{
  int ret;

  if (data->bit_mask == 0)
    {
      data->bit_save = holy_jpeg_get_byte (data);
      if (data->bit_save == JPEG_ESC_CHAR)
	{
	  if (holy_jpeg_get_byte (data) != 0)
	    {
...
static int
holy_jpeg_get_number (struct holy_jpeg_data *data, int num)
{
  int value, i, msb;
  ...
  msb = value = holy_jpeg_get_bit (data);
  for (i = 1; i < num; i++)
    value = (value << 1) + (holy_jpeg_get_bit (data) != 0);
  if (!msb)
    value += 1 - (1 << num);
...
static int
holy_jpeg_get_huff_code (struct holy_jpeg_data *data, int id)
{
  int code;
  unsigned i;
  ...
  for (i = 0; i < ARRAY_SIZE (data->huff_maxval[id]); i++)
    {
      code <<= 1;
      if (holy_jpeg_get_bit (data))
	code++;
      if (code < data->huff_maxval[id][i])
	return data->huff_value[id][code + data->huff_offset[id][i]];
    }
  holy_error (holy_ERR_BAD_FILE_TYPE, "jpeg: huffman decode fails");
  return 0;
}

static holy_err_t
holy_jpeg_decode_huff_table (struct holy_jpeg_data *data)
{
  int id, ac, n, base, ofs;
  holy_uint32_t next_marker;
  holy_uint8_t count[16];
  unsigned i;

  next_marker = data->file->offset;
  next_marker += holy_jpeg_get_word (data);

  while (data->file->offset + sizeof (count) + 1 <= next_marker)
    {
      id = holy_jpeg_get_byte (data);
      ac = (id >> 4) & 1;
      id &= 0xF;
      if (id > 1)
	return holy_error (holy_ERR_BAD_FILE_TYPE,
      ...
      if (holy_file_read (data->file, &count, sizeof (count)) !=
	  sizeof (count))
	return holy_errno;
	...
	n += count[i];
      ...
      id += ac * 2;
      data->huff_value[id] = holy_malloc (n);
      if (holy_errno)
	return holy_errno;

      if (holy_file_read (data->file, data->huff_value[id], n) != n)
	return holy_errno;
  ...
  if (data->file->offset != next_marker)
    holy_error (holy_ERR_BAD_FILE_TYPE, "jpeg: extra byte in huffman table");

  return holy_errno;
}

static holy_err_t
holy_jpeg_decode_quan_table (struct holy_jpeg_data *data)
{
  int id;
  holy_uint32_t next_marker;
  ...
  while (data->file->offset + sizeof (data->quan_table[id]) + 1
	...
	return holy_error (holy_ERR_BAD_FILE_TYPE,
      ...
      if (holy_file_read (data->file, &data->quan_table[id],
...
static holy_err_t
holy_jpeg_decode_sof (struct holy_jpeg_data *data)
{
  int i, cc;
  holy_uint32_t next_marker;
  ...
  if (holy_jpeg_get_byte (data) != 8)
    return holy_error (holy_ERR_BAD_FILE_TYPE,
		       "jpeg: only 8-bit precision is supported");

  data->image_height = holy_jpeg_get_word (data);
  data->image_width = holy_jpeg_get_word (data);

  if ((!data->image_height) || (!data->image_width))
    return holy_error (holy_ERR_BAD_FILE_TYPE, "jpeg: invalid image size");

  cc = holy_jpeg_get_byte (data);
  if (cc != 1 && cc != 3)
    return holy_error (holy_ERR_BAD_FILE_TYPE,
  ...
  for (i = 0; i < cc; i++)
    {
      ...
      id = holy_jpeg_get_byte (data) - 1;
      if ((id < 0) || (id >= 3))
	return holy_error (holy_ERR_BAD_FILE_TYPE, "jpeg: invalid index");

      ss = holy_jpeg_get_byte (data);	/* Sampling factor.  */
      if (!id)
	{
	  holy_uint8_t vs, hs;
	...
	return holy_error (holy_ERR_BAD_FILE_TYPE,
			   "jpeg: sampling method not supported");
      data->comp_index[id][0] = holy_jpeg_get_byte (data);
    }

  if (data->file->offset != next_marker)
    holy_error (holy_ERR_BAD_FILE_TYPE, "jpeg: extra byte in sof");

  return holy_errno;
}

static holy_err_t
holy_jpeg_decode_dri (struct holy_jpeg_data *data)
{
  if (holy_jpeg_get_word (data) != 4)
    return holy_error (holy_ERR_BAD_FILE_TYPE,
	"jpeg: DRI marker length must be 4");

  data->dri = holy_jpeg_get_word (data);

  return holy_errno;
}

static void
holy_jpeg_idct_transform (jpeg_data_unit_t du)
{
  int *pd;
  int i;
...
static void
holy_jpeg_decode_du (struct holy_jpeg_data *data, int id, jpeg_data_unit_t du)
{
  int h1, h2, qt;
  unsigned pos;

  holy_memset (du, 0, sizeof (jpeg_data_unit_t));
  ...
  data->dc_value[id] +=
    holy_jpeg_get_number (data, holy_jpeg_get_huff_code (data, h1));
  ...
  while (pos < ARRAY_SIZE (data->quan_table[qt]))
    {
      ...
      num = holy_jpeg_get_huff_code (data, h2);
      if (!num)
	break;

      val = holy_jpeg_get_number (data, num & 0xF);
      num >>= 4;
      pos += num;
  ...
  holy_jpeg_idct_transform (du);
}

static void
holy_jpeg_ycrcb_to_rgb (int yy, int cr, int cb, holy_uint8_t * rgb)
{
  int dd;
...
static holy_err_t
holy_jpeg_decode_sos (struct holy_jpeg_data *data)
{
  int i, cc;
  holy_uint32_t data_offset;

  data_offset = data->file->offset;
  data_offset += holy_jpeg_get_word (data);
  ...
  for (i = 0; i < cc; i++)
	...
	return holy_error (holy_ERR_BAD_FILE_TYPE, "jpeg: invalid index");
      ...
      ht = holy_jpeg_get_byte (data);
      data->comp_index[id][1] = (ht >> 4);
      data->comp_index[id][2] = (ht & 0xF) + 2;
    }

  holy_jpeg_get_byte (data);	/* Skip 3 unused bytes.  */
  holy_jpeg_get_word (data);

  if (data->file->offset != data_offset)
    return holy_error (holy_ERR_BAD_FILE_TYPE, "jpeg: extra byte in sos");
...
static holy_err_t
holy_jpeg_decode_data (struct holy_jpeg_data *data)
{
  ...
  for (; data->r1 < nr1 && (!data->dri || rst);
	...
	for (r2 = 0; r2 < (1U << data->log_vs); r2++)
	  for (c2 = 0; c2 < (1U << data->log_hs); c2++)
	    holy_jpeg_decode_du (data, 0, data->ydu[r2 * 2 + c2]);

	if (data->color_components >= 3)
	  {
	    holy_jpeg_decode_du (data, 1, data->cbdu);
	    holy_jpeg_decode_du (data, 2, data->crdu);
	  }
	...
	for (r2 = 0; r2 < nr2; r2++, ptr2 += (data->image_width - nc2) * 3)
	  for (c2 = 0; c2 < nc2; c2++, ptr2 += 3)
	      ...
	      if (data->color_components >= 3)
		{
		  ...
		  cr = data->crdu[i0];
		  cb = data->cbdu[i0];
		  holy_jpeg_ycrcb_to_rgb (yy, cr, cb, ptr2);
		}
	      else
		{
		  ptr2[0] = yy;
...
static void
holy_jpeg_reset (struct holy_jpeg_data *data)
{
  data->bit_mask = 0x0;
...
static holy_uint8_t
holy_jpeg_get_marker (struct holy_jpeg_data *data)
{
  holy_uint8_t r;

  r = holy_jpeg_get_byte (data);

  if (r != JPEG_ESC_CHAR)
    {
      holy_error (holy_ERR_BAD_FILE_TYPE, "jpeg: invalid maker");
      return 0;
    }

  return holy_jpeg_get_byte (data);
}

static holy_err_t
holy_jpeg_decode_jpeg (struct holy_jpeg_data *data)
{
  if (holy_jpeg_get_marker (data) != JPEG_MARKER_SOI)	/* Start Of Image.  */
    return holy_error (holy_ERR_BAD_FILE_TYPE, "jpeg: invalid jpeg file");

  while (holy_errno == 0)
    {
      holy_uint8_t marker;

      marker = holy_jpeg_get_marker (data);
      if (holy_errno)
	break;

      holy_dprintf ("jpeg", "jpeg marker: %x\n", marker);

      switch (marker)
	{
	case JPEG_MARKER_DHT:	/* Define Huffman Table.  */
	  holy_jpeg_decode_huff_table (data);
	  break;
	case JPEG_MARKER_DQT:	/* Define Quantization Table.  */
	  holy_jpeg_decode_quan_table (data);
	  break;
	case JPEG_MARKER_SOF0:	/* Start Of Frame 0.  */
	  holy_jpeg_decode_sof (data);
	  break;
	case JPEG_MARKER_DRI:	/* Define Restart Interval.  */
	  holy_jpeg_decode_dri (data);
	  break;
	case JPEG_MARKER_SOS:	/* Start Of Scan.  */
	  if (holy_jpeg_decode_sos (data))
	    break;
	  /* FALLTHROUGH */
	...
	case JPEG_MARKER_RST7:
	  holy_jpeg_decode_data (data);
	  holy_jpeg_reset (data);
	  break;
	...
	default:		/* Skip unrecognized marker.  */
	  {
	    ...
	    sz = holy_jpeg_get_word (data);
	    if (holy_errno)
	      return (holy_errno);
	    holy_file_seek (data->file, data->file->offset + sz - 2);
	  }
	}
    }
...
static holy_err_t
holy_video_reader_jpeg (struct holy_video_bitmap **bitmap,
			const char *filename)
{
  holy_file_t file;
  struct holy_jpeg_data *data;
  ...
  if (data != NULL)
    {
      ...
      data->file = file;
      data->bitmap = bitmap;
      holy_jpeg_decode_jpeg (data);

      for (i = 0; i < 4; i++)
	holy_free (data->huff_value[i]);
...
static holy_err_t
holy_cmd_jpegtest (holy_command_t cmdd __attribute__ ((unused)),
		   int argc, char **args)
{
  ...
  holy_video_reader_jpeg (&bitmap, args[0]);
  if (holy_errno != holy_ERR_NONE)
    return holy_errno;
...
static struct holy_video_bitmap_reader jpg_reader = {
  .extension = ".jpg",
  .reader = holy_video_reader_jpeg,
  .next = 0
};

static struct holy_video_bitmap_reader jpeg_reader = {
  .extension = ".jpeg",
  .reader = holy_video_reader_jpeg,
  .next = 0
};
...
holy_MOD_INIT (jpeg)
{
  holy_video_bitmap_reader_register (&jpg_reader);
  holy_video_bitmap_reader_register (&jpeg_reader);
#if defined(JPEG_DEBUG)
  cmd = holy_register_command ("jpegtest", holy_cmd_jpegtest,
...
#endif
  holy_video_bitmap_reader_unregister (&jpeg_reader);
  holy_video_bitmap_reader_unregister (&jpg_reader);
}
