...
static holy_err_t scale_nn (struct holy_video_bitmap *dst,
                            struct holy_video_bitmap *src);
static holy_err_t scale_bilinear (struct holy_video_bitmap *dst,
                                  struct holy_video_bitmap *src);

static holy_err_t
verify_source_bitmap (struct holy_video_bitmap *src)
{
  /* Verify the simplifying assumptions. */
  if (src == 0)
    return holy_error (holy_ERR_BUG,
                       "null src bitmap in holy_video_bitmap_create_scaled");
...
static holy_err_t
holy_video_bitmap_scale (struct holy_video_bitmap *dst,
                         struct holy_video_bitmap *src,
                         enum holy_video_bitmap_scale_method scale_method)
{
  switch (scale_method)
    {
    case holy_VIDEO_BITMAP_SCALE_METHOD_FASTEST:
    case holy_VIDEO_BITMAP_SCALE_METHOD_NEAREST:
      return scale_nn (dst, src);
    case holy_VIDEO_BITMAP_SCALE_METHOD_BEST:
    case holy_VIDEO_BITMAP_SCALE_METHOD_BILINEAR:
      return scale_bilinear (dst, src);
    default:
      return holy_error (holy_ERR_BUG, "Invalid scale_method value");
    }
}
...
/* This function creates a new scaled version of the bitmap SRC.  The new
   ...
   not equal to holy_ERR_NONE, and the bitmap DST is either not created, or
   ...
   greatly simplified.  */
holy_err_t
holy_video_bitmap_create_scaled (struct holy_video_bitmap **dst,
                                 ...
                                 scale_method)
{
  *dst = 0;
  ...
  holy_err_t err = verify_source_bitmap(src);
  ...
  holy_err_t ret;
  ret = holy_video_bitmap_create (dst, dst_width, dst_height,
                                  src->mode_info.blit_format);
  if (ret != holy_ERR_NONE)
    return ret;                 /* Error. */

  ret = holy_video_bitmap_scale (*dst, src, scale_method);
              ...
              holy_video_bitmap_h_align_t h_align)
{
  ...
  switch (h_align)
    {
    case holy_VIDEO_BITMAP_H_ALIGN_LEFT:
      *x = 0;
      break;
    case holy_VIDEO_BITMAP_H_ALIGN_CENTER:
      *x = (*w - new_w) / 2;
      break;
    case holy_VIDEO_BITMAP_H_ALIGN_RIGHT:
      *x = *w - new_w;
      break;
    default:
      ret = holy_error (holy_ERR_BUG, "Invalid h_align value");
      break;
    }
  *w = new_w;
  return ret;
              ...
              holy_video_bitmap_v_align_t v_align)
{
  ...
  switch (v_align)
    {
    case holy_VIDEO_BITMAP_V_ALIGN_TOP:
      *y = 0;
      break;
    case holy_VIDEO_BITMAP_V_ALIGN_CENTER:
      *y = (*h - new_h) / 2;
      break;
    case holy_VIDEO_BITMAP_V_ALIGN_BOTTOM:
      *y = *h - new_h;
      break;
    default:
      ret = holy_error (holy_ERR_BUG, "Invalid v_align value");
      break;
    }
  *h = new_h;
  return ret;
...
holy_err_t
holy_video_bitmap_scale_proportional (struct holy_video_bitmap **dst,
                                      int dst_width, int dst_height,
                                      struct holy_video_bitmap *src,
                                      enum holy_video_bitmap_scale_method
                                      scale_method,
...
{
  *dst = 0;
  holy_err_t ret = verify_source_bitmap(src);
  if (ret != holy_ERR_NONE)
    return ret;
  if (dst_width <= 0 || dst_height <= 0)
    return holy_error (holy_ERR_BUG,
  ...
  switch (selection_method)
    {
    case holy_VIDEO_BITMAP_SELECTION_METHOD_CROP:
      ...
      break;
    case holy_VIDEO_BITMAP_SELECTION_METHOD_PADDING:
      if (sw * dh < dw * sh)
        ret = make_h_align (&dx0, &dw, sw * dh / sh, h_align);
      else
        ret = make_v_align (&dy0, &dh, sh * dw / sw, v_align);
      break;
    case holy_VIDEO_BITMAP_SELECTION_METHOD_FITWIDTH:
      if (sw * dh < dw * sh)
        ret = make_v_align (&sy0, &sh, sw * dh / dw, v_align);
      else
        ret = make_v_align (&dy0, &dh, sh * dw / sw, v_align);
      break;
    case holy_VIDEO_BITMAP_SELECTION_METHOD_FITHEIGHT:
      if (sw * dh < dw * sh)
        ret = make_h_align (&dx0, &dw, sw * dh / sh, h_align);
      else
        ret = make_h_align (&sx0, &sw, sh * dw / dh, h_align);
      break;
    default:
      ret = holy_error (holy_ERR_BUG, "Invalid selection_method value");
      break;
    }
  ...
  if (ret == holy_ERR_NONE)
    {
      ...
      src->data = (holy_uint8_t *) src->data + sx0 * bytes_per_pixel
                  + sy0 * sstride;
      (*dst)->mode_info.width = dw;
      (*dst)->mode_info.height = dh;
      (*dst)->data = (holy_uint8_t *) (*dst)->data + dx0 * bytes_per_pixel
                     + dy0 * dstride;
...
static holy_err_t
verify_bitmaps (struct holy_video_bitmap *dst, struct holy_video_bitmap *src)
{
  /* Verify the simplifying assumptions. */
  if (dst == 0 || src == 0)
    return holy_error (holy_ERR_BUG, "null bitmap in scale function");
  ...
  if (dst->mode_info.width == 0 || dst->mode_info.height == 0
    ...
    return holy_error (holy_ERR_BUG, "bitmap has a zero dimension");
...
/* Nearest neighbor bitmap scaling algorithm.
   ...
   greatly simplified.  */
static holy_err_t
scale_nn (struct holy_video_bitmap *dst, struct holy_video_bitmap *src)
{
  holy_err_t err = verify_bitmaps(dst, src);
  if (err != holy_ERR_NONE)
    return err;

  holy_uint8_t *ddata = dst->data;
  holy_uint8_t *sdata = src->data;
  ...
  holy_uint8_t *dptr, *dline_end, *sline;
...
/* Bilinear interpolation image scaling algorithm.
   ...
   greatly simplified.  */
static holy_err_t
scale_bilinear (struct holy_video_bitmap *dst, struct holy_video_bitmap *src)
{
  holy_err_t err = verify_bitmaps(dst, src);
  if (err != holy_ERR_NONE)
    return err;
