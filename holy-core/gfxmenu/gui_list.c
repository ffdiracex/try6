...
struct holy_gui_list_impl
{
  struct holy_gui_list list;
  ...
  int item_icon_space;
  int item_spacing;
  holy_font_t item_font;
  int selected_item_font_inherit;
  holy_font_t selected_item_font;
  holy_video_rgba_color_t item_color;
  int selected_item_color_inherit;
  holy_video_rgba_color_t selected_item_color;

  int draw_scrollbar;
  int need_to_recreate_scrollbar;
  char *scrollbar_frame_pattern;
  char *scrollbar_thumb_pattern;
  holy_gfxmenu_box_t scrollbar_frame;
  holy_gfxmenu_box_t scrollbar_thumb;
  ...
  holy_gfxmenu_box_t menu_box;
  holy_gfxmenu_box_t selected_item_box;
  holy_gfxmenu_box_t item_box;

  holy_gfxmenu_icon_manager_t icon_manager;

  holy_gfxmenu_view_t view;
};

typedef struct holy_gui_list_impl *list_impl_t;
...
list_destroy (void *vself)
{
  ...
  holy_free (self->menu_box_pattern);
  holy_free (self->item_box_pattern);
  holy_free (self->selected_item_box_pattern);
  if (self->menu_box)
  ...
  if (self->icon_manager)
    holy_gfxmenu_icon_manager_destroy (self->icon_manager);
  ...
  holy_free (self->scrollbar_thumb_pattern);
  holy_free (self->scrollbar_frame_pattern);
  holy_free (self);
}
...
get_num_shown_items (list_impl_t self)
{
  ...
  holy_gfxmenu_box_t box = self->menu_box;
  int box_top_pad = box->get_top_pad (box);
  int box_bottom_pad = box->get_bottom_pad (box);
  holy_gfxmenu_box_t itembox = self->item_box;
  holy_gfxmenu_box_t selbox = self->selected_item_box;
  ...
  int max_top_pad = holy_max (item_top_pad, sel_top_pad);
  int max_bottom_pad = holy_max (item_bottom_pad, sel_bottom_pad);
...
check_boxes (list_impl_t self)
{
  if (self->need_to_recreate_boxes)
    {
      holy_gui_recreate_box (&self->menu_box,
                             self->menu_box_pattern,
                             self->theme_dir);

      holy_gui_recreate_box (&self->item_box,
                             self->item_box_pattern,
                             self->theme_dir);

      holy_gui_recreate_box (&self->selected_item_box,
...
check_scrollbar (list_impl_t self)
{
  if (self->need_to_recreate_scrollbar)
    {
      holy_gui_recreate_box (&self->scrollbar_frame,
                             self->scrollbar_frame_pattern,
                             self->theme_dir);

      holy_gui_recreate_box (&self->scrollbar_thumb,
  ...
  /* Sanity checks. */
  holy_gfxmenu_box_t frame = self->scrollbar_frame;
  holy_gfxmenu_box_t thumb = self->scrollbar_thumb;
  holy_gfxmenu_box_t menu = self->menu_box;
...
get_item_icon (list_impl_t self, int item_index)
{
  ...
  entry = holy_menu_get_entry (self->view->menu, item_index);
  if (! entry)
    return 0;

  return holy_gfxmenu_icon_manager_get_icon (self->icon_manager, entry);
}
...
draw_menu (list_impl_t self, int num_shown_items)
{
  ...
  int ascent = holy_font_get_ascent (self->item_font);
  int descent = holy_font_get_descent (self->item_font);
  int selected_ascent = holy_font_get_ascent (self->selected_item_font);
  int selected_descent = holy_font_get_descent (self->selected_item_font);
  ...
  int max_leftpad = holy_max (item_leftpad, sel_leftpad);
  int max_toppad = holy_max (item_toppad, sel_toppad);
  int item_top = 0;
  int menu_index;
  int visible_index;
  struct holy_video_rect oviewport;

  holy_video_get_viewport (&oviewport.x, &oviewport.y,
			   &oviewport.width, &oviewport.height);
  holy_video_set_viewport (oviewport.x + boxpad,
			   oviewport.y + boxpad,
  ...
  holy_video_rect_t svpsave, sviewport;
  sviewport.x = max_leftpad + text_left_offset;
  int text_viewport_width = cwidth - sviewport.x;
  sviewport.height = text_box_height;

  holy_video_color_t item_color;
  holy_video_color_t sel_color;
  item_color = holy_video_map_rgba_color (self->item_color);
  sel_color = holy_video_map_rgba_color (self->selected_item_color);
  ...
  for (visible_index = 0, menu_index = self->first_shown_index;
       ...
       visible_index++, menu_index++)
    {
      ...
      if (icon != 0)
        holy_video_blit_bitmap (icon, holy_VIDEO_BLIT_BLEND,
                                max_leftpad,
                                item_top + icon_top_offset,
                                0, 0, self->icon_width, self->icon_height);

      const char *item_title =
        holy_menu_get_entry (self->view->menu, menu_index)->title;

      sviewport.y = item_top + top_pad;
      sviewport.width = viewport_width;
      holy_gui_set_viewport (&sviewport, &svpsave);
      holy_font_draw_string (item_title,
                             font,
                             color,
                             0,
                             text_top_offset);
      holy_gui_restore_viewport (&svpsave);

      item_top += text_box_height + item_vspace;
    }
  holy_video_set_viewport (oviewport.x,
			   oviewport.y,
			   oviewport.width,
			   oviewport.height);
}
...
static void
list_paint (void *vself, const holy_video_rect_t *region)
{
  list_impl_t self = vself;
  ...
  holy_gui_set_viewport (&self->bounds, &vpsave);
  {
    ...
    int box_bottom_pad = box->get_bottom_pad (box);
    holy_video_rect_t vpsave2, content_rect;
    int num_shown_items = get_num_shown_items (self);
    ...
    holy_gui_set_viewport (&content_rect, &vpsave2);
    draw_menu (self, num_shown_items);
    holy_gui_restore_viewport (&vpsave2);
...
static void
list_set_parent (void *vself, holy_gui_container_t parent)
{
  list_impl_t self = vself;
  self->parent = parent;
}
...
static void
list_set_bounds (void *vself, const holy_video_rect_t *bounds)
{
  list_impl_t self = vself;
  self->bounds = *bounds;
}
...
static void
list_get_bounds (void *vself, holy_video_rect_t *bounds)
{
  list_impl_t self = vself;
  *bounds = self->bounds;
}
  ...
  if (check_boxes (self))
    {
      ...
      int max_left_pad = holy_max (item_left_pad, sel_left_pad);
      int max_right_pad = holy_max (item_right_pad, sel_right_pad);
      int max_top_pad = holy_max (item_top_pad, sel_top_pad);
      int max_bottom_pad = holy_max (item_bottom_pad, sel_bottom_pad);

      *width = holy_font_get_string_width (self->item_font, "Typical OS");
      width_s = holy_font_get_string_width (self->selected_item_font,
					    "Typical OS");
      if (*width < width_s)
	*width = width_s;
...
{
  list_impl_t self = vself;
  if (holy_strcmp (name, "item_font") == 0)
    {
      self->item_font = holy_font_get (value);
      if (self->selected_item_font_inherit)
        self->selected_item_font = self->item_font;
    }
  else if (holy_strcmp (name, "selected_item_font") == 0)
    {
      if (! value || holy_strcmp (value, "inherit") == 0)
        {
          self->selected_item_font = self->item_font;
        ...
        {
          self->selected_item_font = holy_font_get (value);
          self->selected_item_font_inherit = 0;
        }
    }
  else if (holy_strcmp (name, "item_color") == 0)
    {
      holy_video_rgba_color_t color;
      if (holy_video_parse_color (value, &color) == holy_ERR_NONE)
        {
  ...
  else if (holy_strcmp (name, "selected_item_color") == 0)
    {
      if (! value || holy_strcmp (value, "inherit") == 0)
        {
          self->selected_item_color = self->item_color;
        ...
        {
          holy_video_rgba_color_t color;
          if (holy_video_parse_color (value, &color)
              == holy_ERR_NONE)
            {
  ...
  else if (holy_strcmp (name, "icon_width") == 0)
    {
      self->icon_width = holy_strtol (value, 0, 10);
      holy_gfxmenu_icon_manager_set_icon_size (self->icon_manager,
                                               self->icon_width,
                                               self->icon_height);
    }
  else if (holy_strcmp (name, "icon_height") == 0)
    {
      self->icon_height = holy_strtol (value, 0, 10);
      holy_gfxmenu_icon_manager_set_icon_size (self->icon_manager,
                                               self->icon_width,
                                               self->icon_height);
    }
  else if (holy_strcmp (name, "item_height") == 0)
    {
      self->item_height = holy_strtol (value, 0, 10);
    }
  else if (holy_strcmp (name, "item_padding") == 0)
    {
      self->item_padding = holy_strtol (value, 0, 10);
    }
  else if (holy_strcmp (name, "item_icon_space") == 0)
    {
      self->item_icon_space = holy_strtol (value, 0, 10);
    }
  else if (holy_strcmp (name, "item_spacing") == 0)
    {
      self->item_spacing = holy_strtol (value, 0, 10);
    }
  else if (holy_strcmp (name, "visible") == 0)
    {
      self->visible = holy_strcmp (value, "false") != 0;
    }
  else if (holy_strcmp (name, "menu_pixmap_style") == 0)
    {
      self->need_to_recreate_boxes = 1;
      holy_free (self->menu_box_pattern);
      self->menu_box_pattern = value ? holy_strdup (value) : 0;
    }
  else if (holy_strcmp (name, "item_pixmap_style") == 0)
    {
      self->need_to_recreate_boxes = 1;
      holy_free (self->item_box_pattern);
      self->item_box_pattern = value ? holy_strdup (value) : 0;
      if (self->selected_item_box_pattern_inherit)
        {
          holy_free (self->selected_item_box_pattern);
          self->selected_item_box_pattern = value ? holy_strdup (value) : 0;
        }
    }
  else if (holy_strcmp (name, "selected_item_pixmap_style") == 0)
    {
      if (!value || holy_strcmp (value, "inherit") == 0)
        {
          holy_free (self->selected_item_box_pattern);
          char *tmp = self->item_box_pattern;
          self->selected_item_box_pattern = tmp ? holy_strdup (tmp) : 0;
  ...
  else if (holy_strcmp (name, "scrollbar_frame") == 0)
    {
      self->need_to_recreate_scrollbar = 1;
      holy_free (self->scrollbar_frame_pattern);
      self->scrollbar_frame_pattern = value ? holy_strdup (value) : 0;
    }
  else if (holy_strcmp (name, "scrollbar_thumb") == 0)
    {
      self->need_to_recreate_scrollbar = 1;
      holy_free (self->scrollbar_thumb_pattern);
      self->scrollbar_thumb_pattern = value ? holy_strdup (value) : 0;
    }
  else if (holy_strcmp (name, "scrollbar_thumb_overlay") == 0)
    {
      self->scrollbar_thumb_overlay = holy_strcmp (value, "true") == 0;
    }
  else if (holy_strcmp (name, "scrollbar_width") == 0)
    {
      self->scrollbar_width = holy_strtol (value, 0, 10);
    }
  else if (holy_strcmp (name, "scrollbar_slice") == 0)
    {
      if (holy_strcmp (value, "west") == 0)
        self->scrollbar_slice = SCROLLBAR_SLICE_WEST;
      else if (holy_strcmp (value, "center") == 0)
        self->scrollbar_slice = SCROLLBAR_SLICE_CENTER;
      else if (holy_strcmp (value, "east") == 0)
        self->scrollbar_slice = SCROLLBAR_SLICE_EAST;
    }
  else if (holy_strcmp (name, "scrollbar_left_pad") == 0)
    {
      self->scrollbar_left_pad = holy_strtol (value, 0, 10);
    }
  else if (holy_strcmp (name, "scrollbar_right_pad") == 0)
    {
      self->scrollbar_right_pad = holy_strtol (value, 0, 10);
    }
  else if (holy_strcmp (name, "scrollbar_top_pad") == 0)
    {
      self->scrollbar_top_pad = holy_strtol (value, 0, 10);
    }
  else if (holy_strcmp (name, "scrollbar_bottom_pad") == 0)
    {
      self->scrollbar_bottom_pad = holy_strtol (value, 0, 10);
    }
  else if (holy_strcmp (name, "scrollbar") == 0)
    {
      self->draw_scrollbar = holy_strcmp (value, "false") != 0;
    }
...
{
  list_impl_t self = vself;
  holy_gfxmenu_icon_manager_set_theme_path (self->icon_manager,
					    view->theme_path);
  self->view = view;
}
...
static struct holy_gui_component_ops list_comp_ops =
  {
...
static struct holy_gui_list_ops list_ops =
{
...
holy_gui_list_new (void)
{
  ...
  self->menu_box = holy_gfxmenu_create_box (0, 0);
  self->item_box = holy_gfxmenu_create_box (0, 0);
  self->selected_item_box = holy_gfxmenu_create_box (0, 0);

  self->icon_manager = holy_gfxmenu_icon_manager_new ();
  if (! self->icon_manager)
    {
