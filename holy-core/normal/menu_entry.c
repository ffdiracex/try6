...
init_line (struct screen *screen, struct line *linep)
{
  linep->len = 0;
  linep->max_len = 80;
  linep->buf = holy_malloc ((linep->max_len + 1) * sizeof (linep->buf[0]));
  linep->pos = holy_zalloc (screen->nterms * sizeof (linep->pos[0]));
  ...
  if (linep->max_len < linep->len + extra)
    {
      ...
      linep->buf = holy_realloc (linep->buf, (linep->max_len + 1) * sizeof (linep->buf[0]));
      if (! linep->buf)
	return 0;
    }
...
{
  holy_size_t width = holy_getstringwidth (linep->buf, linep->buf + linep->len,
					   term_screen->term);
...
advance_to (struct screen *screen, int c)
{
  ...
  screen->column = holy_unicode_get_comb_end (screen->lines[screen->line].buf
...
print_empty_line (int y, struct per_term_screen *term_screen)
{
  ...
  holy_term_gotoxy (term_screen->term,
		    (struct holy_term_coordinate) { term_screen->geo.first_entry_x,
			y + term_screen->geo.first_entry_y });

  for (i = 0; i < term_screen->geo.entry_width + 1; i++)
    holy_putcode (' ', term_screen->term);
}
...
print_updown (int upflag, int downflag, struct per_term_screen *term_screen)
{
  ...
  if (upflag && downflag)
    holy_putcode (holy_UNICODE_UPDOWNARROW, term_screen->term);
  else if (upflag)
    holy_putcode (holy_UNICODE_UPARROW, term_screen->term);
  else if (downflag)
    holy_putcode (holy_UNICODE_DOWNARROW, term_screen->term);
  else
    holy_putcode (' ', term_screen->term);
}
	       ...
	       int up, int down, enum update_mode mode)
{
  ...
  holy_size_t t = holy_getstringwidth (linep->buf, linep->buf + screen->column,
				       term_screen->term);
  ...
  holy_term_setcursor (term_screen->term, 0);
  ...
  if (mode != NO_LINE)
      ...
      do
	{
	  ...
	  if (!*pos)
	    *pos = holy_zalloc ((linep->len + 1) * sizeof (**pos));
	  ...
	  if (i == region_start || linep == screen->lines + screen->line
	      || (i > region_start && mode == ALL_LINES))
	    {
	      ...
	      holy_print_ucs4_menu (linep->buf,
				    linep->buf + linep->len,
  ...
  if (screen->lines[screen->line].pos[term_screen - screen->terms])
    {
      const struct holy_term_pos *cpos;
	...
	holy_term_gotoxy (term_screen->term,
			  (struct holy_term_coordinate) { cpos->x + term_screen->geo.first_entry_x,
			      cpos->y + y
			      + term_screen->geo.first_entry_y });
      else
	holy_term_gotoxy (term_screen->term,
  ...
  holy_term_setcursor (term_screen->term, 1);

  holy_term_refresh (term_screen->term);
}
...
insert_string (struct screen *screen, const char *s, int update)
{
  ...
  while (*s)
      ...
      if (*s == '\n')
	{
	  ...
	  /* Make a new line.  */
	  screen->num_lines++;
	  screen->lines = holy_realloc (screen->lines,
					screen->num_lines
	  ...
	  if (screen->line < screen->num_lines - 2)
	    holy_memmove (screen->lines + screen->line + 2,
      ...
      else
	{
	  ...
	  /* Insert the string.  */
	  current_linep = screen->lines + screen->line;
	  unicode_msg = holy_malloc ((p - s) * sizeof (holy_uint32_t));

	  if (!unicode_msg)
	    return 0;

	  size = holy_utf8_to_ucs4 (unicode_msg, (p - s),
				    (holy_uint8_t *) s, (p - s), 0);
	  ...
	  holy_memmove (current_linep->buf + screen->column + size,
			current_linep->buf + screen->column,
			(current_linep->len - screen->column)
			* sizeof (current_linep->buf[0]));
	  holy_memmove (current_linep->buf + screen->column,
			unicode_msg,
...
destroy_screen (struct screen *screen)
{
  ...
  holy_free (screen->killed_text);
  holy_free (screen->lines);
  holy_free (screen->terms);
  holy_free (screen);
}

/* Make a new screen.  */
static struct screen *
make_screen (holy_menu_entry_t entry)
{
  struct screen *screen;
  unsigned i;

  /* Initialize the screen.  */
  screen = holy_zalloc (sizeof (*screen));
  if (! screen)
    return 0;
  ...
  screen->num_lines = 1;
  screen->lines = holy_malloc (sizeof (struct line));
  if (! screen->lines)
    goto fail;
  ...
  if (screen->column > 0)
    {
      ...
      screen->column--;
      screen->column = holy_unicode_get_comb_start (linep->buf,
						    linep->buf + screen->column)
	- linep->buf;

      holy_unicode_aglomerate_comb (screen->lines[screen->line].buf + screen->column,
...
delete_char (struct screen *screen, int update)
{
  ...
  if (linep->len > screen->column)
	...
	screen->terms[i].orig_num = get_logical_num_lines (linep, &screen->terms[i]);
      ...
      holy_memmove (linep->buf + screen->column,
	...
	return 0;
      ...
      holy_memmove (linep->buf + linep->len, next_linep->buf,
      ...
      for (i = 0; i < screen->nterms; i++)
	{
      ...
      holy_free (next_linep->buf);
      holy_memmove (next_linep,
		    next_linep + 1,
		    (screen->num_lines - screen->line - 2)
		    * sizeof (struct line));
      screen->num_lines--;
  ...
  if (size > 0)
    {
      ...
      p = holy_realloc (p, offset + size + 1);
      if (! p)
	return 0;

      holy_memmove (p + offset, linep->buf + screen->column, size);
      p[offset + size] = '\0';
...
static void
store_completion (const char *item, holy_completion_type_t type,
		  int count __attribute__ ((unused)))
{
  ...
  if (completion_buffer.max_len < (completion_buffer.len
				   + (int) holy_strlen (item) + 1 + 1))
    {
      holy_size_t new_len;

      new_len = completion_buffer.len + holy_strlen (item) + 80;
      p = holy_realloc (completion_buffer.buf, new_len);
      if (! p)
	{
	  /* Possibly not fatal.  */
  ...
  holy_strcpy (p, item);
  completion_buffer.len += holy_strlen (item);
}
...
complete (struct screen *screen, int continuous, int update)
{
  ...
  linep = screen->lines + screen->line;
  u8 = holy_ucs4_to_utf8_alloc (linep->buf, screen->column);
  if (!u8)
    return 1;

  insert = holy_normal_do_completion (u8, &restore, store_completion);
  
  if (completion_buffer.buf)
    {
      buflen = holy_strlen (completion_buffer.buf);
      ucs4 = holy_malloc (sizeof (holy_uint32_t) * (buflen + 1));
      ...
      if (!ucs4)
	{
      ...
      ucs4len = holy_utf8_to_ucs4 (ucs4, buflen,
				   (holy_uint8_t *) completion_buffer.buf,
				   buflen, 0);
      ucs4[ucs4len] = 0;
	...
	for (i = 0; i < screen->nterms; i++)
	  {
	    unsigned width = holy_term_width (screen->terms[i].term);
	    if (width > 2)
	      width -= 2;
	    if (width > 15)
	      width -= 6;
	    ...
	    holy_uint32_t *p = ucs4;

	    pos = holy_term_getxy (screen->terms[i].term);

	    screen->completion_shown = 1;

	    holy_term_gotoxy (screen->terms[i].term,
			      (struct holy_term_coordinate) { 0,
				  screen->terms[i].geo.timeout_y });
	    if (screen->terms[i].geo.timeout_lines >= 2)
	      {
		holy_puts_terminal ("   ", screen->terms[i].term);
		switch (completion_type)
		  {
		  case holy_COMPLETION_TYPE_COMMAND:
		    holy_puts_terminal (_("Possible commands are:"),
					screen->terms[i].term);
		    break;
		  case holy_COMPLETION_TYPE_DEVICE:
		    holy_puts_terminal (_("Possible devices are:"),
					screen->terms[i].term);
		    break;
		  case holy_COMPLETION_TYPE_FILE:
		    holy_puts_terminal (_("Possible files are:"),
					screen->terms[i].term);
		    break;
		  case holy_COMPLETION_TYPE_PARTITION:
		    holy_puts_terminal (_("Possible partitions are:"),
					screen->terms[i].term);
		    break;
		  case holy_COMPLETION_TYPE_ARGUMENT:
		    holy_puts_terminal (_("Possible arguments are:"),
					screen->terms[i].term);
		    break;
		  default:
		    holy_puts_terminal (_("Possible things are:"),
					screen->terms[i].term);
		    break;
		  }
	    ...
	    if (p != ucs4)
	      holy_putcode (holy_UNICODE_LEFTARROW, screen->terms[i].term);
	    else
	      holy_putcode (' ', screen->terms[i].term);

	    holy_print_ucs4 (p, ucs4 + ucs4len < endp ? ucs4 + ucs4len : endp,
			     0, 0, screen->terms[i].term);

	    if (ucs4 + ucs4len > endp)
	      holy_putcode (holy_UNICODE_RIGHTARROW, screen->terms[i].term);
	    holy_term_gotoxy (screen->terms[i].term, pos);
	  }
    }
  ...
  holy_free (completion_buffer.buf);
  return 1;
}
...
clear_completions (struct per_term_screen *term_screen)
{
  ...
  pos = holy_term_getxy (term_screen->term);
  holy_term_gotoxy (term_screen->term,
		    (struct holy_term_coordinate) { 0,
  ...
  for (i = 0; i < term_screen->geo.timeout_lines; i++)
    {
      for (j = 0; j < holy_term_width (term_screen->term) - 1; j++)
	holy_putcode (' ', term_screen->term);
      if (i + 1 < term_screen->geo.timeout_lines)
	holy_putcode ('\n', term_screen->term);
    }

  holy_term_gotoxy (term_screen->term, pos);
  holy_term_refresh (term_screen->term);
}
...
run (struct screen *screen)
{
  ...
  holy_cls ();
  holy_printf ("  ");
  holy_printf_ (N_("Booting a command list"));
  holy_printf ("\n\n");
  ...
  {
    int i;
    holy_size_t size = 0, tot_size = 0;

    for (i = 0; i < screen->num_lines; i++)
      tot_size += holy_get_num_of_utf8_bytes (screen->lines[i].buf,
					      screen->lines[i].len) + 1;

    script = holy_malloc (tot_size + 1);
    if (! script)
      return 0;

    for (i = 0; i < screen->num_lines; i++)
      {
	size += holy_ucs4_to_utf8 (screen->lines[i].buf, screen->lines[i].len,
				   (holy_uint8_t *) script + size,
				   tot_size - size);
	script[size++] = '\n';
      }
    script[size] = '\0';
  }
  holy_script_execute_new_scope (script, 0, dummy);
  holy_free (script);
...
void
holy_menu_entry_run (holy_menu_entry_t entry)
{
  struct screen *screen;
  int prev_c;
 ...
 refresh:
  ...
  for (i = 0; i < (unsigned) screen->num_lines; i++)
    {
      holy_free (screen->lines[i].pos);
      screen->lines[i].pos = holy_zalloc (screen->nterms * sizeof (screen->lines[i].pos[0]));
  ...
  screen->terms = holy_zalloc (screen->nterms * sizeof (screen->terms[0]));
  if (!screen->terms)
    {
      holy_print_error ();
      destroy_screen (screen);
  ...
  /* Draw the screen.  */
  for (i = 0; i < screen->nterms; i++)
    holy_menu_init_page (0, 1, &screen->terms[i].geo,
			 screen->terms[i].term);
  update_screen_all (screen, 0, 0, 1, 1, ALL_LINES);
  for (i = 0; i < screen->nterms; i++)
    holy_term_setcursor (screen->terms[i].term, 1);
  prev_c = '\0';
  ...
  while (1)
	...
	case holy_TERM_CTRL | 'l':
	  /* FIXME: centering.  */
	  goto refresh;

	case holy_TERM_CTRL | 'o':
	...
	case holy_TERM_KEY_F2:
	  holy_cmdline_run (1, 0);
	  goto refresh;

	case holy_TERM_CTRL | 'x':
	case holy_TERM_KEY_F10:
	  run (screen);
	  goto refresh;

	case holy_TERM_CTRL | 'r':
	case holy_TERM_CTRL | 's':
	case holy_TERM_CTRL | 't':
	  /* FIXME */
	  break;
