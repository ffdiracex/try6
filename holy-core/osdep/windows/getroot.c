...
holy_guess_root_devices (const char *dir)
{
  ...
  dirwindows = holy_util_get_windows_path (dir);
  if (!dirwindows)
    return 0;
  ...
  if (!mntpointwindows)
    {
      free (dirwindows);
      holy_util_info ("can't get volume path name: %d", (int) GetLastError ());
      return 0;      
    }
  ...
  os_dev[0] = holy_util_tchar_to_utf8 (volumename);
  free (dirwindows);
  free (mntpointwindows);
...
{
  HANDLE hd;
  LPTSTR name = holy_util_get_windows_path (os_dev);
  ...
  if (((name[0] == '/') || (name[0] == '\\')) &&
	  ...
	  || ((name[4] == 'A' || name[4] == 'a' || name[4] == 'B' || name[4] == 'b')
	      && name[5] == ':' && name[6] == '\0')))
    {
      holy_util_info ("Matches full disk pattern");
      ret = holy_util_tchar_to_utf8 (name);
      free (name);
      return ret;
    }
  ...
  if (hd == INVALID_HANDLE_VALUE)
    {
      holy_util_info ("CreateFile failed");
      ret = holy_util_tchar_to_utf8 (name);
      free (name);
      return ret;
    }
  ...
  if (!DeviceIoControl(hd, IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS,
    ...
    {
      holy_util_info ("IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS failed");
      ret = holy_util_tchar_to_utf8 (name);
      CloseHandle (hd);
      free (name);
      return ret;
...
holy_disk_addr_t
holy_util_find_partition_start_os (const char *os_dev)
{
  HANDLE hd;
  LPTSTR name = holy_util_get_windows_path (os_dev);
  VOLUME_DISK_EXTENTS exts;
...
holy_util_biosdisk_is_floppy (holy_disk_t disk)
{
  ...
  name = holy_util_get_windows_path (dname);
