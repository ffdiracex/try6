...
holy_util_find_root_devices_from_poolname (char *poolname)
{
  ...
  for (i = 0; i < nvlist_count; i++)
	...
	{
#ifdef __sun__
	  if (holy_memcmp (device, "/dev/dsk/", sizeof ("/dev/dsk/") - 1)
	      == 0)
	    device = xasprintf ("/dev/rdsk/%s",
				device + sizeof ("/dev/dsk/") - 1);
	  else if (holy_memcmp (device, "/devices", sizeof ("/devices") - 1)
		   == 0
		   && holy_memcmp (device + strlen (device) - 4,
				   ",raw", 4) != 0)
	  ...
	  case 1:
	      ...
	      while (1)
		{
		  if (strncmp (ptr, poolname, strlen (poolname)) == 0
		      && holy_isspace(ptr[strlen (poolname)]))
		    st++;
		  if (!holy_isspace (*ptr))
		    break;
		  ptr++;
...
find_root_devices_from_libzfs (const char *dir)
{
  ...
  holy_find_zpool_from_dir (dir, &poolname, &poolfs);
  if (! poolname)
    return NULL;

  devices = holy_util_find_root_devices_from_poolname (poolname);
...
char *
holy_find_device (const char *dir, dev_t dev)
{
  DIR *dp;
  char *saved_cwd;
  struct dirent *ent;
  ...
  holy_util_info ("changing current directory to %s", dir);
  if (chdir (dir) < 0)
    {
      free (saved_cwd);
      closedir (dp);
  ...
  while ((ent = readdir (dp)) != 0)
      ...
      if (S_ISDIR (st.st_mode))
	{
	  ...
	  res = holy_find_device (ent->d_name, dev);
...
#endif
	{
	  ...
	  if (chdir (saved_cwd) < 0)
	    holy_util_error ("%s", _("cannot restore the original directory"));
...
holy_guess_root_devices (const char *dir_in)
{
  ...
  struct stat st;
  dev_t dev;
  char *dir = holy_canonicalize_file_name (dir_in);

  if (!dir)
    holy_util_error (_("failed to get canonical path of `%s'"), dir_in);

#ifdef __linux__
  if (!os_dev)
    os_dev = holy_find_root_devices_from_mountinfo (dir, NULL);
#endif /* __linux__ */
      ...
      for (cur = os_dev; *cur; cur++)
	{
	  ...
	  if (strcmp (*cur, "/dev/root") == 0
	    ...
	    *cur = tmp;
	    ...
	    {
	      *cur = holy_canonicalize_file_name (tmp);
	      if (*cur == NULL)
		holy_util_error (_("failed to get canonical path of `%s'"), tmp);
	      free (tmp);
	    }
	  ...
	  free (*cur);
	  dev = st.st_rdev;
	  *cur = holy_find_device (dm ? "/dev/mapper" : "/dev", dev);
	}
      if (!*cur)
	return os_dev;
      for (cur = os_dev; *cur; cur++)
  ...
  if (stat (dir, &st) < 0)
    holy_util_error (_("cannot stat `%s': %s"), dir, strerror (errno));
  free (dir);
  ...
  os_dev[0] = holy_find_device ("/dev", dev);
...
holy_util_pull_lvm_by_command (const char *os_dev)
{
  ...
  char *optr;
  char *vgid = NULL;
  holy_size_t vgidlen = 0;

  vgid = holy_util_get_vg_uuid (os_dev);
  if (vgid)
    vgidlen = holy_strlen (vgid);
  ...
  while (getline (&buf, &len, vgs) > 0)
    {
      ...
      if (vgid && (holy_strncmp (vgid, ptr, vgidlen) != 0
		   || ptr[vgidlen] != ':'))
	continue;
      if (vgid)
	ptr += vgidlen + 1;
...
void
holy_find_zpool_from_dir (const char *dir, char **poolname, char **poolfs)
{
  char *slash;
  ...
  *poolname = *poolfs = NULL;
  ...
  /* Solaris.  */
  {
    ...
    FILE *mnttab = holy_util_fopen ("/etc/mnttab", "r");
    if (! mnttab)
      return;
...
void
holy_util_pull_lvm_by_command (const char *os_dev __attribute__ ((unused)))
{
}
