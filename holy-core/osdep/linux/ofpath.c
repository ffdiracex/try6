...
#define holy_util_warn holy_util_info
#define _(x) x
  ...
  while (1)
      ...
      if (fd < 0 || fstat (fd, &st) < 0)
	{
	  ...
	  if (!strcmp(sysfs_path, "/sys"))
	    {
	      holy_util_info (_("`obppath' not found in parent dirs of `%s',"
	...
	{
	  holy_util_info (_("cannot read `%s': %s"), path, strerror (errno));
	  close(fd);
	  free (path);
	  free (of_path);
	  free (sysfs_path);
...
#endif
  if (!out)
    holy_util_error (_("failed to get canonical path of `%s'"), in);
  return out;
}
...
vendor_is_ATA(const char *path)
{
  ...
  if (fd < 0)
    holy_util_error (_("cannot open `%s': %s"), bufname, strerror (errno));
...
check_sas (const char *sysfs_path, int *tgt, unsigned long int *sas_address)
{
  ...
  *q = '\0';
  ...
  if (read (fd, phy, sizeof (phy) - 1) < 0)
    holy_util_error (_("cannot read `%s': %s"), path, strerror (errno));
...
char *
holy_util_devname_to_ofpath (const char *sys_devname)
{
  ...
  if (device[0] == 'h' && device[1] == 'd')
    ...
    {
      holy_util_warn (_("unknown device type %s"), device);
      ofpath = NULL;
    }
