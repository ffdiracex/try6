...
holy_MOD_INIT(gcry_whirlpool)
{
  COMPILE_TIME_ASSERT(sizeof (whirlpool_context_t) <= holy_CRYPTO_MAX_MD_CONTEXT_SIZE);
  holy_md_register (&_gcry_digest_spec_whirlpool);
}

holy_MOD_FINI(gcry_whirlpool)
{
  holy_md_unregister (&_gcry_digest_spec_whirlpool);
}
