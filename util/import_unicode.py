...
outfile.write ("#include <holy/unicode.h>\n")
outfile.write ("\n")
outfile.write ("struct holy_unicode_compact_range holy_unicode_compact[] = {\n")
...
for line in infile:
    ...
    if lastcode + 1 != curcode or curbiditype != lastbiditype \
        ...
        if begincode != -2 and (lastbiditype != "L" or lastcombtype != 0 or \
                                    lastmirrortype):
            outfile.write (("{0x%x, 0x%x, holy_BIDI_TYPE_%s, %d, %d, holy_JOIN_TYPE_%s},\n" \
...
outfile.write ("struct holy_unicode_bidi_pair holy_unicode_bidi_pairs[] = {\n")
...
outfile.write ("struct holy_unicode_arabic_shape holy_unicode_arabic_shapes[] = {\n ")
