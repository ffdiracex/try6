...
#if holy_TARGET_WORDSIZE == 64
typedef Elf64_Rela elf_reloc_t;
#define holy_PE32_MACHINE holy_PE32_MACHINE_X86_64
#else
typedef Elf32_Rel elf_reloc_t;
#define holy_PE32_MACHINE holy_PE32_MACHINE_I386
#endif
...
static holy_uint32_t offset, image_base;
...
write_section_data (FILE* fp, const char *name, char *image,
                    struct holy_pe32_coff_header *pe_chdr,
                    struct holy_pe32_section_table *pe_shdr)
{
  int *section_map;
  int i;
  holy_uint32_t last_category = 0;
  holy_uint32_t idx, idx_reloc;
  char *pe_strtab = (image + pe_chdr->symtab_offset
		     + pe_chdr->num_symbols * sizeof (struct holy_pe32_symbol));
  ...
  for (i = 0; i < pe_chdr->num_sections; i++, pe_shdr++)
    {
      holy_uint32_t category;
      const char *shname = pe_shdr->name;
      holy_size_t secsize;

      if (shname[0] == '/' && holy_isdigit (shname[1]))
      {
	...
	holy_util_error ("out of order sections");
                     ...
                     int *symtab_map)
{
  ...
  for (i = first_reloc_section; i < reloc_sections_end; i++)
    {
      struct holy_pe32_section_table *pe_sec;
      struct holy_pe32_reloc *pe_rel;
      elf_reloc_t *rel;
      int num_rels, j, modified;

      pe_sec = pe_shdr + shdr[i].sh_link;
      pe_rel = (struct holy_pe32_reloc *) (image + pe_sec->relocations_offset);
      ...
      for (j = 0; j < pe_sec->num_relocations; j++, pe_rel++)
	    ...
	    case 1:
	      type = R_X86_64_64;
	      rel[num_rels].r_addend = *(holy_int64_t *)addr;
	      *(holy_int64_t *)addr = 0;
	      modified = 1;
	      break;
	    case 4:
	      type = R_X86_64_PC32;
	      rel[num_rels].r_addend = *(holy_int32_t *)addr;
	      *addr = 0;
	      modified = 1;
	      break;
	    case 14:
	      type = R_X86_64_PC64;
	      rel[num_rels].r_addend = *(holy_uint64_t *)addr - 8;
	      *(holy_uint64_t *)addr = 0;
	      modified = 1;
	      break;
#else
	    case holy_PE32_REL_I386_DIR32:
	      type = R_386_32;
	      break;
	    case holy_PE32_REL_I386_REL32:
	      type = R_386_PC32;
	      break;
#endif
	    default:
	      holy_util_error ("unknown pe relocation type %d", pe_rel->type);
	    }
	      ...
	      R_X86_64_PC32
	      ...
	      )
            {
              ...
              if (((code != 0xe8) && (code != 0xe9)) || (*addr))
                holy_util_error ("invalid relocation (%x %x)", code, *addr);
#endif
      ...
      if (modified)
        holy_util_write_image_at (image + pe_sec->raw_data_offset,
      ...
      holy_util_write_image_at (rel, shdr[i].sh_size, offset, fp, name);
...
write_symbol_table (FILE* fp, const char *name, char *image,
                    ...
                    int *section_map)
{
  struct holy_pe32_symbol *pe_symtab;
  char *pe_strtab;
  Elf_Sym *symtab;
  int *symtab_map, num_syms;
  int i;

  pe_symtab = (struct holy_pe32_symbol *) (image + pe_chdr->symtab_offset);
  ...
  for (i = 0; i < (int) pe_chdr->num_symbols;
       i += pe_symtab->num_aux + 1, pe_symtab += pe_symtab->num_aux + 1)
    {
      ...
      if (! pe_symtab->section)
        type = STT_NOTYPE;
      else if (pe_symtab->type == holy_PE32_DT_FUNCTION)
        type = STT_FUNC;
      else
        type = STT_OBJECT;

      if (pe_symtab->storage_class == holy_PE32_SYM_CLASS_EXTERNAL)
        bind = STB_GLOBAL;
      else
        bind = STB_LOCAL;

      if ((pe_symtab->type != holy_PE32_DT_FUNCTION) && (pe_symtab->num_aux))
        {
          if (! pe_symtab->value)
            type = STT_SECTION;
	  ...
	  if (pe_symtab->long_name[0])
	    ...
	    symname = pe_strtab + pe_symtab->long_name[1];
          ...
          if ((strcmp (symname, "_holy_mod_init")) &&
              (strcmp (symname, "_holy_mod_fini")) &&
	      (strcmp (symname, "holy_mod_init")) &&
              (strcmp (symname, "holy_mod_fini")) &&
              (bind == STB_LOCAL))
              continue;
  ...
  holy_util_write_image_at (symtab, shdr[symtab_section].sh_size,
                            offset, fp, name);
...
write_string_table (FILE *fp, const char *name)
{
  ...
  holy_util_write_image_at (strtab, strtab_len, offset, fp,
			    name);
  offset += strtab_len;
...
write_section_header (FILE *fp, const char *name)
{
  ...
  holy_util_write_image_at (shdr, sizeof (Elf_Shdr) * num_sections,
                            offset, fp, name);

  holy_util_write_image_at (&ehdr, sizeof (Elf_Ehdr), 0, fp, name);
}
...
convert_pe (FILE* fp, const char *name, char *image)
{
  ...
  if (image[0] == 'M' && image[1] == 'Z')
    pe_chdr = (struct holy_pe32_coff_header *) (image + (holy_le_to_cpu32 (((holy_uint32_t *)image)[0xf]) + 4));
  else
    pe_chdr = (struct holy_pe32_coff_header *) image;
  if (holy_le_to_cpu16 (pe_chdr->machine) != holy_PE32_MACHINE)
    holy_util_error ("invalid coff image (%x != %x)",
		     holy_le_to_cpu16 (pe_chdr->machine), holy_PE32_MACHINE);
  ...
  pe_shdr = (struct holy_pe32_section_table *) ((char *) (pe_chdr + 1) + pe_chdr->optional_header_size);
...
main (int argc, char *argv[])
{
  ...
  if (argc > 2)
    ...
    out = in;
  image = holy_util_read_image (in);

  fp = holy_util_fopen (out, "wb");
  if (! fp)
    holy_util_error ("cannot open %s", out);
