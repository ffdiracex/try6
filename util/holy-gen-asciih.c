	   ...
	   struct holy_glyph_info *glyph_info)
{
  ...
  if (!glyph_info->bitmap)
    {
      fprintf (stderr, "holy-gen-asciih: error: out of memory");
      exit (1);
    }
...
main (int argc, char *argv[])
{
  ...
  if (argc != 3)
    {
      fprintf (stderr, "holy-gen-asciih: usage: INPUT OUTPUT");
      return 1;
    }

  if (FT_Init_FreeType (&ft_lib))
    {
      fprintf (stderr, "holy-gen-asciih: error: FT_Init_FreeType fails");
      return 1;
    }

  {
    ...
    if (FT_Set_Pixel_Sizes (ft_face, size, size))
      {
	fprintf (stderr, "holy-gen-asciih: error: can't set %dx%d font size", size, size);
	return 1;
      }
  }
  ...
  if (! file)
    {
      fprintf (stderr, "holy-gen-asciih: error: cannot write to `%s': %s", argv[2],
	       strerror (errno));
      return 1;
    }
