...
main (int argc, char *argv[])
{
  ...
  if (argc != 3)
    {
      fprintf (stderr, "holy-gen-widthspec: usage: INPUT OUTPUT");
      return 1;
    }

  if (FT_Init_FreeType (&ft_lib))
    {
      fprintf (stderr, "holy-gen-widthspec: error: FT_Init_FreeType fails");
      return 1;
    }

  {
    ...
    if (FT_Set_Pixel_Sizes (ft_face, size, size))
      {
	fprintf (stderr, "holy-gen-widthspec: error: can't set %dx%d font size", size, size);
	return 1;
      }
