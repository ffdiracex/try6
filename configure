...
# Generated by GNU Autoconf 2.72 for holy 2.02.
#
...
$0: bashscripter123@gmail.com about your system, including
...
PACKAGE_NAME='holy'
PACKAGE_TARNAME='holy'
PACKAGE_VERSION='2.02'
PACKAGE_STRING='holy 2.02'
PACKAGE_BUGREPORT='bashscripter123@gmail.com'
PACKAGE_URL=''

ac_unique_file="include/holy/dl.h"
...
COND_holy_MOUNT_FALSE
COND_holy_MOUNT_TRUE
COND_holy_MKFONT_FALSE
COND_holy_MKFONT_TRUE
COND_holy_EMU_PCI_FALSE
COND_holy_EMU_PCI_TRUE
COND_holy_EMU_SDL_FALSE
COND_holy_EMU_SDL_TRUE
...
if test "$ac_init_help" = "long"; then
  ...
  cat <<_ACEOF
'configure' configures holy 2.02 to adapt to many kinds of systems.
...
Fine tuning of the installation directories:
  ...
  --docdir=DIR            documentation root [DATAROOTDIR/doc/holy]
...
if test -n "$ac_init_help"; then
  case $ac_init_help in
     short | recursive ) echo "Configuration of holy 2.02:";;
   esac
  cat <<\_ACEOF
...
Optional Features:
  ...
  --enable-holy-emu-sdl   build and install the `holy-emu' debugging utility
                          with SDL support (default=guessed)
  --enable-holy-emu-pci   build and install the `holy-emu' debugging utility
                          with PCI support (potentially dangerous)
                          (default=no)
  --enable-holy-mkfont    build and install the `holy-mkfont' utility
                          (default=guessed)
  --enable-holy-themes    build and install holy themes (default=guessed)
  --enable-holy-mount     build and install the `holy-mount' utility
                          (default=guessed)
  ...
  --disable-werror        do not use -Werror when building holy
...
Optional Packages:
  ...
  --with-holydir=DIR      set the name of holy directory [[guessed]]
...
It was created by holy $as_me 2.02, which was
...
as_fn_append ac_func_c_list " wcscoll HAVE_WCSCOLL"
...
ac_aux_files="config.rpath compile missing install-sh config.guess config.sub"
...
holy_bios_setup=`printf "%s\n" holy-bios-setup | sed "$program_transform_name"`

holy_editenv=`printf "%s\n" holy-editenv | sed "$program_transform_name"`

holy_install=`printf "%s\n" holy-install | sed "$program_transform_name"`

holy_mkconfig=`printf "%s\n" holy-mkconfig | sed "$program_transform_name"`

holy_mkfont=`printf "%s\n" holy-mkfont | sed "$program_transform_name"`

holy_mkimage=`printf "%s\n" holy-mkimage | sed "$program_transform_name"`

holy_glue_efi=`printf "%s\n" holy-glue-efi | sed "$program_transform_name"`

holy_mklayout=`printf "%s\n" holy-mklayout | sed "$program_transform_name"`

holy_mkpasswd_pbkdf2=`printf "%s\n" holy-mkpasswd-pbkdf2 | sed "$program_transform_name"`

holy_mkrelpath=`printf "%s\n" holy-mkrelpath | sed "$program_transform_name"`

holy_mkrescue=`printf "%s\n" holy-mkrescue | sed "$program_transform_name"`

holy_probe=`printf "%s\n" holy-probe | sed "$program_transform_name"`

holy_reboot=`printf "%s\n" holy-reboot | sed "$program_transform_name"`

holy_script_check=`printf "%s\n" holy-script-check | sed "$program_transform_name"`

holy_set_default=`printf "%s\n" holy-set-default | sed "$program_transform_name"`

holy_sparc64_setup=`printf "%s\n" holy-sparc64-setup | sed "$program_transform_name"`

holy_render_label=`printf "%s\n" holy-render-label | sed "$program_transform_name"`

holy_file=`printf "%s\n" holy-file | sed "$program_transform_name"`
...
HOST_CPPFLAGS="$HOST_CPPFLAGS -Dholy_UTIL=1"
...
case "$target_cpu" in
  ...
  mipsel|mips64el)
                target_cpu=mipsel
		machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_CPU_MIPSEL=1"
		;;
  mips|mips64)
                target_cpu=mips
		machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_CPU_MIPS=1"
		;;
  arm*)
		target_cpu=arm
		;;
...
if test "x$with_platform" = x; then
  case "$target_cpu"-"$target_vendor" in
    ...
    arm64-*) platform=efi ;;
    *)
      { printf "%s\n" "$as_me:${as_lineno-$LINENO}: WARNING: unsupported CPU: \"$target_cpu\" - only building utilities" >&5
printf "%s\n" "$as_me: WARNING: unsupported CPU: \"$target_cpu\" - only building utilities" >&2;}
      platform=none
      ;;
  esac
else
...
case "$platform" in
  coreboot)	machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_COREBOOT=1" ;;
  multiboot)	machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_MULTIBOOT=1" ;;
  efi)		machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_EFI=1" ;;
  xen)		machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_XEN=1" ;;
  ieee1275)	machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_IEEE1275=1" ;;
  uboot)	machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_UBOOT=1" ;;
  qemu)		machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_QEMU=1" ;;
  pc)		machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_PCBIOS=1" ;;
  emu)		machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_EMU=1" ;;
  loongson)	machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_MIPS_LOONGSON=1" ;;
  qemu_mips)	machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_MIPS_QEMU_MIPS=1" ;;
  arc)	machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE_ARC=1" ;;
esac
if test x${target_cpu} = xmipsel ; then
   machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE=`echo mips_$platform | sed y,abcdefghijklmnopqrstuvwxyz,ABCDEFGHIJKLMNOPQRSTUVWXYZ,`"
else
   machine_CPPFLAGS="$machine_CPPFLAGS -Dholy_MACHINE=`echo ${target_cpu}_$platform | sed y,abcdefghijklmnopqrstuvwxyz,ABCDEFGHIJKLMNOPQRSTUVWXYZ,`"
fi
...
if test ${with_bootdir+y}
then :
  withval=$with_bootdir; have_with_bootdir=y
else case e in #(
  e) have_with_bootdir=n ;;
esac
fi
...
printf "%s\n" "#define holy_BOOT_DIR_NAME \"$bootdirname\"" >>confdefs.h



# Check whether --with-holydir was given.
if test ${with_holydir+y}
then :
  withval=$with_holydir; holydirname="$with_holydir"
else case e in #(
  e) holydirname="$PACKAGE" ;;
esac
fi
...
printf "%s\n" "#define holy_DIR_NAME \"$holydirname\"" >>confdefs.h
...
if test "x$LEX" = "x:"; then
  ...
  if test -n "$version" -a "$version" -ge 20535; then
    ...
    as_fn_error $? "flex is too old. holy requires 2.5.35 or above" "$LINENO" 5
  fi
fi
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_HAVE_CXX+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$HAVE_CXX"; then
...
# declarations like 'int a3[[(sizeof (unsigned char)) >= 0]];'.
# This bug is HP SR number 8606223364.
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking size of TCHAR" >&5
printf %s "checking size of TCHAR... " >&6; }
if test ${ac_cv_sizeof_TCHAR+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if ac_fn_c_compute_int "$LINENO" "(long int) (sizeof (TCHAR))" "ac_cv_sizeof_TCHAR"        "#include <windows.h>
"
then :

else case e in #(
  e) if test "$ac_cv_type_TCHAR" = yes; then
     { { printf "%s\n" "$as_me:${as_lineno-$LINENO}: error: in '$ac_pwd':" >&5
printf "%s\n" "$as_me: error: in '$ac_pwd':" >&2;}
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_sizeof_TCHAR" >&5
printf "%s\n" "$ac_cv_sizeof_TCHAR" >&6; }



printf "%s\n" "#define SIZEOF_TCHAR $ac_cv_sizeof_TCHAR" >>confdefs.h
...
printf "%s\n" "#define SIZEOF_OFF_T $ac_cv_sizeof_off_t" >>confdefs.h
     ...
     test x"$ac_cv_sizeof_off_t" = x8 || as_fn_error $? "Large file support is required" "$LINENO" 5;;
esac
...
if test "x$cross_compiling" = xyes; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: WARNING: cannot generate manual pages while cross compiling" >&5
printf "%s\n" "$as_me: WARNING: cannot generate manual pages while cross compiling" >&2;}
else
...
ac_fn_c_check_func "$LINENO" "getextmntent" "ac_cv_func_getextmntent"
if test "x$ac_cv_func_getextmntent" = xyes
then :
  printf "%s\n" "#define HAVE_GETEXTMNTENT 1" >>confdefs.h
...
ac_fn_c_check_header_compile "$LINENO" "sys/mnttab.h" "ac_cv_header_sys_mnttab_h" "$ac_includes_default"
if test "x$ac_cv_header_sys_mnttab_h" = xyes
then :
  printf "%s\n" "#define HAVE_SYS_MNTTAB_H 1" >>confdefs.h
...
#include <sys/mount.h>
"
if test "x$ac_cv_member_struct_statfs_f_mntfromname" = xyes
then :

printf "%s\n" "#define HAVE_STRUCT_STATFS_F_MNTFROMNAME 1" >>confdefs.h
...
then :
  ...
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for opendisk in -lutil" >&5
printf %s "checking for opendisk in -lutil... " >&6; }
if test ${ac_cv_lib_util_opendisk+y}
then :
  printf %s "(cached) " >&6
else case e in #(
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_util_opendisk" >&5
printf "%s\n" "$ac_cv_lib_util_opendisk" >&6; }
if test "x$ac_cv_lib_util_opendisk" = xyes
then :

    LIBUTIL="-lutil"

printf "%s\n" "#define HAVE_OPENDISK 1" >>confdefs.h


fi

  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for getrawpartition in -lutil" >&5
printf %s "checking for getrawpartition in -lutil... " >&6; }
if test ${ac_cv_lib_util_getrawpartition+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) ac_check_lib_save_LIBS=$LIBS
...
then :
  ...
  e) ac_cv_lib_util_getrawpartition=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_util_getrawpartition" >&5
printf "%s\n" "$ac_cv_lib_util_getrawpartition" >&6; }
if test "x$ac_cv_lib_util_getrawpartition" = xyes
then :

    LIBUTIL="-lutil"

printf "%s\n" "#define HAVE_GETRAWPARTITION 1" >>confdefs.h
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -Wtrampolines work" >&5
printf %s "checking whether -Wtrampolines work... " >&6; }
if test ${holy_cv_host_cc_wtrampolines+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_host_cc_wtrampolines=yes
else case e in #(
  e) holy_cv_host_cc_wtrampolines=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
  CFLAGS="$SAVED_CFLAGS"
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_host_cc_wtrampolines" >&5
printf "%s\n" "$holy_cv_host_cc_wtrampolines" >&6; }

if test x"$holy_host_cv_cc_wtrampolines" = xyes ; then
...
then :
  ...
  e) gl_cv_func_working_getdelim="guessing no" ;;
esac
fi
rm -rf conftest*
...
then :
  ...
  e) am_cv_func_working_getline="guessing no" ;;
esac
fi
rm -rf conftest*
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking which extra warnings work" >&5
printf %s "checking which extra warnings work... " >&6; }
if test ${holy_cv_cc_w_extra_flags+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
  SAVED_CFLAGS="$CFLAGS"
  holy_cv_cc_w_extra_flags=
...
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
      if test x$flag = x1 ; then
         holy_cv_cc_w_extra_flags="$holy_cv_cc_w_extra_flags $x"
      fi
  done
  CFLAGS="$SAVED_CFLAGS"
 ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_w_extra_flags" >&5
printf "%s\n" "$holy_cv_cc_w_extra_flags" >&6; }

HOST_CFLAGS="$HOST_CFLAGS $holy_cv_cc_w_extra_flags"
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_ac_ct_TARGET_CC+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$ac_ct_TARGET_CC"; then
...
if test -n "$ac_ct_TARGET_CC"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_ct_TARGET_CC" >&5
printf "%s\n" "$ac_ct_TARGET_CC" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_TARGET_OBJCOPY+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$TARGET_OBJCOPY"; then
...
if test -n "$TARGET_OBJCOPY"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $TARGET_OBJCOPY" >&5
printf "%s\n" "$TARGET_OBJCOPY" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_ac_ct_TARGET_OBJCOPY+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$ac_ct_TARGET_OBJCOPY"; then
...
if test -n "$ac_ct_TARGET_OBJCOPY"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_ct_TARGET_OBJCOPY" >&5
printf "%s\n" "$ac_ct_TARGET_OBJCOPY" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
fi
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_TARGET_STRIP+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$TARGET_STRIP"; then
...
if test -n "$TARGET_STRIP"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $TARGET_STRIP" >&5
printf "%s\n" "$TARGET_STRIP" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_ac_ct_TARGET_STRIP+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$ac_ct_TARGET_STRIP"; then
...
if test -n "$ac_ct_TARGET_STRIP"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_ct_TARGET_STRIP" >&5
printf "%s\n" "$ac_ct_TARGET_STRIP" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
fi
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_TARGET_NM+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$TARGET_NM"; then
...
if test -n "$TARGET_NM"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $TARGET_NM" >&5
printf "%s\n" "$TARGET_NM" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_ac_ct_TARGET_NM+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$ac_ct_TARGET_NM"; then
...
if test -n "$ac_ct_TARGET_NM"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_ct_TARGET_NM" >&5
printf "%s\n" "$ac_ct_TARGET_NM" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_TARGET_RANLIB+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$TARGET_RANLIB"; then
...
if test -n "$TARGET_RANLIB"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $TARGET_RANLIB" >&5
printf "%s\n" "$TARGET_RANLIB" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_ac_ct_TARGET_RANLIB+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$ac_ct_TARGET_RANLIB"; then
...
if test -n "$ac_ct_TARGET_RANLIB"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_ct_TARGET_RANLIB" >&5
printf "%s\n" "$ac_ct_TARGET_RANLIB" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
fi
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking which extra warnings work" >&5
printf %s "checking which extra warnings work... " >&6; }
if test ${holy_cv_target_cc_w_extra_flags+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
  ...
  holy_cv_target_cc_w_extra_flags=
  for x in $EXTRA_WARN_FLAGS; do
...
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
      if test x$flag = x1 ; then
         holy_cv_target_cc_w_extra_flags="$holy_cv_target_cc_w_extra_flags $x"
      fi
  done
 ;;
esac
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_w_extra_flags" >&5
printf "%s\n" "$holy_cv_target_cc_w_extra_flags" >&6; }

TARGET_CFLAGS="$TARGET_CFLAGS $holy_cv_target_cc_w_extra_flags"

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if compiling with clang" >&5
printf %s "checking if compiling with clang... " >&6; }
if test ${holy_cv_cc_target_clang+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_target_clang=no
else case e in #(
  e) holy_cv_cc_target_clang=yes ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext ;;
esac
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_target_clang" >&5
printf "%s\n" "$holy_cv_cc_target_clang" >&6; }

if test x$target_cpu = xpowerpc -o x$target_cpu = xmips; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for options to get big-endian compilation" >&5
printf %s "checking for options to get big-endian compilation... " >&6; }
if test ${holy_cv_target_cc_big_endian+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_big_endian=no
    for cand in "-target $target_cpu -Wl,-EB" "-target $target_cpu" \
		"-target $target_cpu-linux-gnu -Wl,-EB" "-target $target_cpu-linux-gnu" \
		"-EB" "-mbig-endian"; do
      if test x"$holy_cv_target_cc_big_endian" != xno ; then
        break
      fi
...
if ac_fn_c_try_link "$LINENO"
then :
  holy_cv_target_cc_big_endian="$cand"
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
    done
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_big_endian" >&5
printf "%s\n" "$holy_cv_target_cc_big_endian" >&6; }

  if test x"$holy_cv_target_cc_big_endian" = xno ; then
    as_fn_error $? "could not force big-endian" "$LINENO" 5
  fi

  skip_linkflags="$(echo "$holy_cv_target_cc_big_endian"|sed 's@-Wl,-EB@@')"
  ...
  TARGET_LDFLAGS="$TARGET_LDFLAGS $holy_cv_target_cc_big_endian"
elif test x$target_cpu = xmipsel; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for options to get little-endian compilation" >&5
printf %s "checking for options to get little-endian compilation... " >&6; }
if test ${holy_cv_target_cc_little_endian+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_little_endian=no
    for cand in "-target $target_cpu -Wl,-EL" "-target $target_cpu" \
		"-target $target_cpu-linux-gnu -Wl,-EL" "-target $target_cpu-linux-gnu" \
		"-EL"; do
      if test x"$holy_cv_target_cc_little_endian" != xno ; then
        break
      fi
      CFLAGS="$TARGET_CFLAGS $cand -Werror"
      cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
if ac_fn_c_try_link "$LINENO"
then :
  holy_cv_target_cc_little_endian="$cand"
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
    done
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_little_endian" >&5
printf "%s\n" "$holy_cv_target_cc_little_endian" >&6; }

  if test x"$holy_cv_target_cc_little_endian" = xno ; then
    as_fn_error $? "could not force little-endian" "$LINENO" 5
  fi

  skip_linkflags="$(echo "$holy_cv_target_cc_little_endian"|sed 's@-Wl,-EL@@')"

  TARGET_CFLAGS="$TARGET_CFLAGS $skip_linkflags"
  TARGET_CPPFLAGS="$TARGET_CPPFLAGS $skip_linkflags"
  TARGET_CCASFLAGS="$TARGET_CCASFLAGS $skip_linkflags"
  TARGET_LDFLAGS="$TARGET_LDFLAGS $holy_cv_target_cc_little_endian"
fi

# holy code is N32-compliant but it's experimental and we would prefer to
...
if test "x$target_cpu" = xmips || test "x$target_cpu" = xmipsel ; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for options to force MIPS o32 ABI" >&5
printf %s "checking for options to force MIPS o32 ABI... " >&6; }
if test ${holy_cv_target_cc_mips_o32_abi+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_mips_o32_abi=no
    for arg in "" "-mabi=32" "-target $target_cpu -mabi=32" ; do
      if test x"$holy_cv_target_cc_mips_o32_abi" != xno ; then
        break
      fi
...
if ac_fn_c_try_link "$LINENO"
then :
  holy_cv_target_cc_mips_o32_abi="$arg"
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
    done
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_mips_o32_abi" >&5
printf "%s\n" "$holy_cv_target_cc_mips_o32_abi" >&6; }

  if test x"$holy_cv_target_cc_mips_o32_abi" = xno ; then
    as_fn_error $? "could not force MIPS o32 ABI" "$LINENO" 5
  fi

  TARGET_CFLAGS="$TARGET_CFLAGS $holy_cv_target_cc_mips_o32_abi"
  TARGET_CCASFLAGS="$TARGET_CCASFLAGS $holy_cv_target_cc_mips_o32_abi"
fi

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for options to compile assembly" >&5
printf %s "checking for options to compile assembly... " >&6; }
if test ${holy_cv_cc_target_asm_compile+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if test x"$test_program" = x ; then
  ...
  for arg in "" "-no-integrated-as"; do
    ...
    if $cmdline >&5 2>&5; then
      holy_cv_cc_target_asm_compile="$arg"
      found=yes
      break
    fi
  done
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_target_asm_compile" >&5
printf "%s\n" "$holy_cv_cc_target_asm_compile" >&6; }

TARGET_CCASFLAGS="$TARGET_CCASFLAGS $holy_cv_cc_target_asm_compile"
...
if test "x$holy_cv_cc_target_clang" = xno && test "x$target_cpu" = xi386 && test "x$platform" != xemu && test "x$platform" != xefi; then
   TARGET_CFLAGS="$TARGET_CFLAGS -mrtd -mregparm=3"
fi

# on mips redirect cache flushing function to non-existant one.
if test "x$target_cpu" = xmips || test "x$target_cpu" = xmipsel ; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -mflush-func=holy_red_herring works" >&5
printf %s "checking whether -mflush-func=holy_red_herring works... " >&6; }
if test ${holy_cv_cc_mflush_func+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    CFLAGS="$TARGET_CFLAGS -mflush-func=holy_red_herring -Werror"
    cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_mflush_func=yes
else case e in #(
  e) holy_cv_cc_mflush_func=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_mflush_func" >&5
printf "%s\n" "$holy_cv_cc_mflush_func" >&6; }

  if test "x$holy_cv_cc_mflush_func" = xyes; then
    TARGET_CFLAGS="$TARGET_CFLAGS -mflush-func=holy_red_herring"
  fi
fi
...
if test "x$target_cpu" = xi386; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -falign-loops works" >&5
printf %s "checking whether -falign-loops works... " >&6; }
if test ${holy_cv_cc_falign_loop+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_falign_loop=yes
else case e in #(
  e) holy_cv_cc_falign_loop=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_falign_loop" >&5
printf "%s\n" "$holy_cv_cc_falign_loop" >&6; }

  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -malign-loops works" >&5
printf %s "checking whether -malign-loops works... " >&6; }
if test ${holy_cv_cc_malign_loop+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_malign_loop=yes
else case e in #(
  e) holy_cv_cc_malign_loop=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_malign_loop" >&5
printf "%s\n" "$holy_cv_cc_malign_loop" >&6; }

  if test "x$holy_cv_cc_falign_loop" = xyes; then
    TARGET_CFLAGS="$TARGET_CFLAGS -falign-jumps=1 -falign-loops=1 -falign-functions=1"
  elif test "x$holy_cv_cc_malign_loop" = xyes; then
    TARGET_CFLAGS="$TARGET_CFLAGS -malign-jumps=1 -malign-loops=1 -malign-functions=1"
  fi
fi

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -freg-struct-return works" >&5
printf %s "checking whether -freg-struct-return works... " >&6; }
if test ${holy_cv_cc_freg_struct_return+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_freg_struct_return=yes
else case e in #(
  e) holy_cv_cc_freg_struct_return=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
 ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_freg_struct_return" >&5
printf "%s\n" "$holy_cv_cc_freg_struct_return" >&6; }

if test "x$holy_cv_cc_freg_struct_return" = xyes; then
    TARGET_CFLAGS="$TARGET_CFLAGS -freg-struct-return"
fi

if ( test "x$target_cpu" = xi386 || test "x$target_cpu" = xx86_64 ) && test "x$platform" != xemu; then
  # Some toolchains enable these features by default, but they need
  # registers that aren't set up properly in holy.
  TARGET_CFLAGS="$TARGET_CFLAGS -mno-mmx -mno-sse -mno-sse2 -mno-sse3 -mno-3dnow"
fi

# holy doesn't use float or doubles at all. Yet some toolchains may decide
# that floats are a good fit to run instead of what's written in the code.
# Given that floating point unit is disabled (if present to begin with)
# when holy is running which may result in various hard crashes.
if test x"$platform" != xemu ; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for options to get soft-float" >&5
printf %s "checking for options to get soft-float... " >&6; }
if test ${holy_cv_target_cc_soft_float+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_soft_float=no
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_target_cc_soft_float="-mgeneral-regs-only"
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
    fi
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_target_cc_soft_float="-mno-inline-float-divide -mno-inline-sqrt"
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
    fi
    for cand in "-msoft-float -Xclang -msoft-float -Xclang -no-implicit-float" \
      ...
      if test x"$holy_cv_target_cc_soft_float" != xno ; then
        break
      fi
      CFLAGS="$TARGET_CFLAGS $cand -Werror"
      cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_target_cc_soft_float="$cand"
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
    done
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_soft_float" >&5
printf "%s\n" "$holy_cv_target_cc_soft_float" >&6; }

  if test x"$holy_cv_target_cc_soft_float" = xno ; then
    as_fn_error $? "could not force soft-float" "$LINENO" 5
  fi

  case x"$holy_cv_target_cc_soft_float" in
    x*"-Xclang"*)
      # A trick so that clang doesn't see it on link stаge
      TARGET_CPPFLAGS="$TARGET_CPPFLAGS $holy_cv_target_cc_soft_float"
      ;;
    *)
      TARGET_CFLAGS="$TARGET_CFLAGS $holy_cv_target_cc_soft_float"
      ;;
  esac
  TARGET_CCASFLAGS="$TARGET_CCASFLAGS $holy_cv_target_cc_soft_float"

fi

if test x"$target_cpu" = xsparc64 ; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for options to reserve application registers" >&5
printf %s "checking for options to reserve application registers... " >&6; }
if test ${holy_cv_target_cc_mno_app_regs+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_mno_app_regs=no
    for cand in "-mllvm -sparc-reserve-app-registers" \
		"-mno-app-regs"; do
      if test x"$holy_cv_target_cc_mno_app_regs" != xno ; then
        break
      fi
      CFLAGS="$TARGET_CFLAGS $cand -Werror"
      CPPFLAGS="$TARGET_CPPFLAGS"
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_target_cc_mno_app_regs="$cand"
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
    done
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_mno_app_regs" >&5
printf "%s\n" "$holy_cv_target_cc_mno_app_regs" >&6; }

  if test x"$holy_cv_target_cc_mno_app_regs" = xno ; then
    as_fn_error $? "could not reserve application registers" "$LINENO" 5
  fi
  if test x"$holy_cv_target_cc_mno_app_regs" = x"-mllvm -sparc-reserve-app-registers" ; then
    # A trick so that clang doesn't see it on link stаge
    TARGET_CPPFLAGS="$TARGET_CPPFLAGS $holy_cv_target_cc_mno_app_regs"
  else
    TARGET_CFLAGS="$TARGET_CFLAGS $holy_cv_target_cc_mno_app_regs"
  fi

  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for no-relax options" >&5
printf %s "checking for no-relax options... " >&6; }
if test ${holy_cv_target_cc_mno_relax+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_mno_relax=no
    for cand in "-mno-relax" "-Wl,--no-relax"; do
      if test x"$holy_cv_target_cc_mno_relax" != xno ; then
        break
      fi
...
if ac_fn_c_try_link "$LINENO"
then :
  holy_cv_target_cc_mno_relax="$cand"
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
    done
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_mno_relax" >&5
printf "%s\n" "$holy_cv_target_cc_mno_relax" >&6; }
  LDFLAGS="$TARGET_LDFLAGS"
  CFLAGS="$TARGET_CFLAGS"
  ...
  TARGET_LDFLAGS="$TARGET_LDFLAGS $holy_cv_target_cc_mno_relax"
fi

# By default, GCC 4.4 generates .eh_frame sections containing unwind
# information in some cases where it previously did not. holy doesn't need
# these and they just use up vital space. Restore the old compiler
# behaviour.
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -fno-dwarf2-cfi-asm works" >&5
printf %s "checking whether -fno-dwarf2-cfi-asm works... " >&6; }
if test ${holy_cv_cc_fno_dwarf2_cfi_asm+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_fno_dwarf2_cfi_asm=yes
else case e in #(
  e) holy_cv_cc_fno_dwarf2_cfi_asm=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
 ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_fno_dwarf2_cfi_asm" >&5
printf "%s\n" "$holy_cv_cc_fno_dwarf2_cfi_asm" >&6; }

if test "x$holy_cv_cc_fno_dwarf2_cfi_asm" = xyes; then
  TARGET_CFLAGS="$TARGET_CFLAGS -fno-dwarf2-cfi-asm"
fi

if test x"$target_os" = xcygwin; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether option -fno-reorder-functions works" >&5
printf %s "checking whether option -fno-reorder-functions works... " >&6; }
if test ${holy_cv_cc_no_reorder_functions+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_no_reorder_functions=yes
else case e in #(
  e) holy_cv_cc_no_reorder_functions=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_no_reorder_functions" >&5
printf "%s\n" "$holy_cv_cc_no_reorder_functions" >&6; }
fi

if test x"$target_os" = xcygwin && test "x$holy_cv_cc_no_reorder_functions" = xyes; then
  TARGET_CFLAGS="$TARGET_CFLAGS -fno-reorder-functions"
fi

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -mno-stack-arg-probe works" >&5
printf %s "checking whether -mno-stack-arg-probe works... " >&6; }
if test ${holy_cv_cc_mno_stack_arg_probe+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_mno_stack_arg_probe=yes
else case e in #(
  e) holy_cv_cc_mno_stack_arg_probe=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
 ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_mno_stack_arg_probe" >&5
printf "%s\n" "$holy_cv_cc_mno_stack_arg_probe" >&6; }

if test "x$holy_cv_cc_mno_stack_arg_probe" = xyes; then
...
# these and they just use up vital space. Restore the old compiler
# behaviour.
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -fno-asynchronous-unwind-tables works" >&5
printf %s "checking whether -fno-asynchronous-unwind-tables works... " >&6; }
if test ${holy_cv_cc_fno_asynchronous_unwind_tables+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_fno_asynchronous_unwind_tables=yes
else case e in #(
  e) holy_cv_cc_fno_asynchronous_unwind_tables=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
 ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_fno_asynchronous_unwind_tables" >&5
printf "%s\n" "$holy_cv_cc_fno_asynchronous_unwind_tables" >&6; }

if test "x$holy_cv_cc_fno_asynchronous_unwind_tables" = xyes; then
  TARGET_CFLAGS="$TARGET_CFLAGS -fno-asynchronous-unwind-tables"
fi

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -fno-unwind-tables works" >&5
printf %s "checking whether -fno-unwind-tables works... " >&6; }
if test ${holy_cv_cc_fno_unwind_tables+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_fno_unwind_tables=yes
else case e in #(
  e) holy_cv_cc_fno_unwind_tables=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
 ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_fno_unwind_tables" >&5
printf "%s\n" "$holy_cv_cc_fno_unwind_tables" >&6; }

if test "x$holy_cv_cc_fno_unwind_tables" = xyes; then
...
if test x"$platform" = xemu ; then
  TARGET_OBJ2ELF=
  holy_cv_target_cc_link_format=
  case "$host_os" in
    *darwin* | *mac*)
       holy_cv_target_cc_link_format="-arch,${target_cpu}"
       TARGET_LDFLAGS="$TARGET_LDFLAGS -Wl,$holy_cv_target_cc_link_format"
        ;;
    *windows* | *cygwin* | *mingw*)
      if test x${target_cpu} = xi386 ; then
        holy_cv_target_cc_link_format=-mi386pe
	TARGET_OBJ2ELF='./build-holy-pe2elf$(BUILD_EXEEXT)'
      fi
      if test x${target_cpu} = xx86_64 ; then
        holy_cv_target_cc_link_format=-mi386pep
	TARGET_OBJ2ELF='./build-holy-pep2elf$(BUILD_EXEEXT)'
      fi
      TARGET_LDFLAGS="$TARGET_LDFLAGS -Wl,$holy_cv_target_cc_link_format"
      ;;
  esac
elif test x"$target_cpu" = xi386 || test x"$target_cpu" = xx86_64; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for target linking format" >&5
printf %s "checking for target linking format... " >&6; }
if test ${holy_cv_target_cc_link_format+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_link_format=unknown
...
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
      if test x"$flag" = x1; then
        holy_cv_target_cc_link_format="$format"
	break
      fi
    done ;;
esac
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_link_format" >&5
printf "%s\n" "$holy_cv_target_cc_link_format" >&6; }
  if test x"$holy_cv_target_cc_link_format" = xunknown; then
    as_fn_error $? "no suitable link format found" "$LINENO" 5
  fi
  TARGET_LDFLAGS="$TARGET_LDFLAGS -Wl,$holy_cv_target_cc_link_format"
  if test x"$holy_cv_target_cc_link_format" = x-mi386pe ; then
    TARGET_OBJ2ELF='./build-holy-pe2elf$(BUILD_EXEEXT)'
  fi
  if test x"$holy_cv_target_cc_link_format" = x-mi386pep ; then
    TARGET_OBJ2ELF='./build-holy-pep2elf$(BUILD_EXEEXT)'
  fi
fi

if test x$holy_cv_target_cc_link_format = x-arch,i386 || test x$holy_cv_target_cc_link_format = x-arch,x86_64; then
   TARGET_APPLE_LINKER=1
   # Extract the first word of "objconv", so it can be a program name with args.
set dummy objconv; ac_word=$2
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_TARGET_OBJCONV+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$TARGET_OBJCONV"; then
...
if test -n "$TARGET_OBJCONV"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $TARGET_OBJCONV" >&5
printf "%s\n" "$TARGET_OBJCONV" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
...
printf "%s\n" "no" >&6; }
   ...
   TARGET_LDFLAGS_OLDMAGIC=""
elif test x$holy_cv_target_cc_link_format = x-mi386pe || test x$holy_cv_target_cc_link_format = x-mi386pep ; then
  TARGET_APPLE_LINKER=0
  TARGET_LDFLAGS_OLDMAGIC="-Wl,-N"
  TARGET_IMG_LDSCRIPT='$(top_srcdir)'"/${holy_coredir}/conf/i386-cygwin-img-ld.sc"
  TARGET_IMG_LDFLAGS="-Wl,-T${TARGET_IMG_LDSCRIPT}"
  TARGET_IMG_LDFLAGS_AC="-Wl,-T${srcdir}/${holy_coredir}/conf/i386-cygwin-img-ld.sc"
...
if test x"$holy_cv_target_cc_link_format" = x-mi386pe || test x"$holy_cv_target_cc_link_format" = x-mi386pep ; then
...
if test x"$efiemu_excuse" = x ; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether options required for efiemu work" >&5
printf %s "checking whether options required for efiemu work... " >&6; }
if test ${holy_cv_cc_efiemu+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_efiemu=yes
else case e in #(
  e) holy_cv_cc_efiemu=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_efiemu" >&5
printf "%s\n" "$holy_cv_cc_efiemu" >&6; }
  if test x$holy_cv_cc_efiemu = xno; then
     efiemu_excuse="cannot compile with -m64 -mcmodel=large -mno-red-zone -nostdlib"
  fi
fi
if test x"$efiemu_excuse" = x ; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for efiemu64 linking format" >&5
printf %s "checking for efiemu64 linking format... " >&6; }
if test ${holy_cv_target_cc_efiemu64_link_format+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_efiemu64_link_format=unknown
...
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
      if test x"$flag" = x1; then
        holy_cv_target_cc_efiemu64_link_format="$format"
	break
      fi
    done ;;
esac
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_efiemu64_link_format" >&5
printf "%s\n" "$holy_cv_target_cc_efiemu64_link_format" >&6; }
  if test x"$holy_cv_target_cc_efiemu64_link_format" = xunknown; then
    efiemu_excuse="no suitable link format for efiemu64 found"
  else
    EFIEMU64_LINK_FORMAT="-Wl,$holy_cv_target_cc_efiemu64_link_format"
  fi
fi
...
if test "$target_cpu" = x86_64 || test "$target_cpu" = sparc64 ; then
  ...
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether option -mcmodel=large works" >&5
printf %s "checking whether option -mcmodel=large works... " >&6; }
if test ${holy_cv_cc_mcmodel+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_mcmodel=yes
else case e in #(
  e) holy_cv_cc_mcmodel=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_mcmodel" >&5
printf "%s\n" "$holy_cv_cc_mcmodel" >&6; }
  if test "x$holy_cv_cc_mcmodel" = xyes; then
...
if test "$target_cpu"-"$platform" = x86_64-efi; then
  ...
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether option -mno-red-zone works" >&5
printf %s "checking whether option -mno-red-zone works... " >&6; }
if test ${holy_cv_cc_no_red_zone+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_no_red_zone=yes
else case e in #(
  e) holy_cv_cc_no_red_zone=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_no_red_zone" >&5
printf "%s\n" "$holy_cv_cc_no_red_zone" >&6; }
  if test "x$holy_cv_cc_no_red_zone" = xno; then
...
if test "x$target_cpu" = xarm; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for options to disable movt and movw" >&5
printf %s "checking for options to disable movt and movw... " >&6; }
if test ${holy_cv_target_cc_mno_movt+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_mno_movt=no
    for cand in "-mno-movt" \
		"-mllvm -arm-use-movt=0"; do
      if test x"$holy_cv_target_cc_mno_movt" != xno ; then
        break
      fi
      CFLAGS="$TARGET_CFLAGS $cand -Werror"
      CPPFLAGS="$TARGET_CPPFLAGS"
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_target_cc_mno_movt="$cand"
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
    done
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_mno_movt" >&5
printf "%s\n" "$holy_cv_target_cc_mno_movt" >&6; }

  if test x"$holy_cv_target_cc_mno_movt" != xno ; then
    # A trick so that clang doesn't see it on link stage
    TARGET_CPPFLAGS="$TARGET_CPPFLAGS $holy_cv_target_cc_mno_movt"
  fi
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether option -mthumb-interwork works" >&5
printf %s "checking whether option -mthumb-interwork works... " >&6; }
if test ${holy_cv_cc_mthumb_interwork+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_mthumb_interwork=yes
else case e in #(
  e) holy_cv_cc_mthumb_interwork=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
   ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_mthumb_interwork" >&5
printf "%s\n" "$holy_cv_cc_mthumb_interwork" >&6; }
  if test "x$holy_cv_cc_mthumb_interwork" = xyes; then
    TARGET_CFLAGS="$TARGET_CFLAGS -mthumb-interwork"
  # Clang defaults to thumb interworking
  elif test "x$holy_cv_cc_target_clang" = xno ; then
    as_fn_error $? "your compiler doesn't support -mthumb-interwork" "$LINENO" 5
  fi
fi

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether option -Qn works" >&5
printf %s "checking whether option -Qn works... " >&6; }
if test ${holy_cv_target_cc_qn+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_target_cc_qn=yes
else case e in #(
  e) holy_cv_target_cc_qn=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext ;;
esac
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_qn" >&5
printf "%s\n" "$holy_cv_target_cc_qn" >&6; }
if test "x$holy_cv_target_cc_qn" = xyes; then
...
# Position independent executable.
pie_possible=yes
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether \`$CC' has \`-fPIE' as default" >&5
printf %s "checking whether \`$CC' has \`-fPIE' as default... " >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether linker accepts -no-pie" >&5
printf %s "checking whether linker accepts -no-pie... " >&6; }
if test ${holy_cv_cc_ld_no_pie+y}
then :
  printf %s "(cached) " >&6
else case e in #(
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_ld_no_pie" >&5
printf "%s\n" "$holy_cv_cc_ld_no_pie" >&6; }
nopie_possible=no
if test "x$holy_cv_cc_ld_no_pie" = xyes ; then
  nopie_possible=yes
fi

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether linker accepts -nopie" >&5
printf %s "checking whether linker accepts -nopie... " >&6; }
if test ${holy_cv_cc_ld_no_pie_oneword+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) save_LDFLAGS="$LDFLAGS"
...
then :
  ...
  e) holy_cv_cc_ld_no_pie_oneword=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_ld_no_pie_oneword" >&5
printf "%s\n" "$holy_cv_cc_ld_no_pie_oneword" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether linker needs disabling of PIE to work" >&5
printf %s "checking whether linker needs disabling of PIE to work... " >&6; }
cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
# Position independent executable.
pic_possible=yes
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether \`$CC' has \`-fPIC' as default" >&5
printf %s "checking whether \`$CC' has \`-fPIC' as default... " >&6; }
# Is this a reliable test case?
cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
# explicitly.
if ( test x$target_cpu = xmips || test x$target_cpu = xmipsel || test x$target_cpu = xarm64 ) && test "x$holy_cv_cc_target_clang" = xyes ; then
...
sap_possible=yes
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether \`$CC' accepts \`-mstack-arg-probe'" >&5
printf %s "checking whether \`$CC' accepts \`-mstack-arg-probe'... " >&6; }
cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
if test "$target_cpu" = arm; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for compile options to get strict alignment" >&5
printf %s "checking for compile options to get strict alignment... " >&6; }
if test ${holy_cv_target_cc_strict_align+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
    holy_cv_target_cc_strict_align=
...
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
      if test x"$flag" = x1; then
        holy_cv_target_cc_strict_align="$arg"
	break
      fi
    done ;;
esac
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_target_cc_strict_align" >&5
printf "%s\n" "$holy_cv_target_cc_strict_align" >&6; }

  TARGET_CFLAGS="$TARGET_CFLAGS $holy_cv_target_cc_strict_align"
  if test x"$holy_cv_target_cc_strict_align" = x"-Xclang -mstrict-align"; then
    TARGET_LDFLAGS="$TARGET_LDFLAGS -Qunused-arguments"
  fi
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if compiler generates unaligned accesses" >&5
printf %s "checking if compiler generates unaligned accesses... " >&6; }
if test ${holy_cv_cc_target_emits_unaligned+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) CFLAGS="$TARGET_CFLAGS"
   cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_target_emits_unaligned=no
else case e in #(
  e) holy_cv_cc_target_emits_unaligned=yes ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext ;;
esac
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_target_emits_unaligned" >&5
printf "%s\n" "$holy_cv_cc_target_emits_unaligned" >&6; }
  if test x$holy_cv_cc_target_emits_unaligned = xyes; then
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if C symbols get an underscore after compilation" >&5
printf %s "checking if C symbols get an underscore after compilation... " >&6; }
if test ${holy_cv_asm_uscore+y}
then :
  printf %s "(cached) " >&6
else case e in #(
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_asm_uscore" >&5
printf "%s\n" "$holy_cv_asm_uscore" >&6; }

if test "x$TARGET_APPLE_LINKER" = x0 && test x"$platform" != xemu; then
if test x$holy_cv_asm_uscore = xyes; then
...
if test x"$platform" = xemu; then
ac_fn_c_check_func "$LINENO" "__udivsi3" "ac_cv_func___udivsi3"
if test "x$ac_cv_func___udivsi3" = xyes
then :
  printf "%s\n" "#define HAVE___UDIVSI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__umodsi3" "ac_cv_func___umodsi3"
if test "x$ac_cv_func___umodsi3" = xyes
then :
  printf "%s\n" "#define HAVE___UMODSI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__divsi3" "ac_cv_func___divsi3"
if test "x$ac_cv_func___divsi3" = xyes
then :
  printf "%s\n" "#define HAVE___DIVSI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__modsi3" "ac_cv_func___modsi3"
if test "x$ac_cv_func___modsi3" = xyes
then :
  printf "%s\n" "#define HAVE___MODSI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__divdi3" "ac_cv_func___divdi3"
if test "x$ac_cv_func___divdi3" = xyes
then :
  printf "%s\n" "#define HAVE___DIVDI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__moddi3" "ac_cv_func___moddi3"
if test "x$ac_cv_func___moddi3" = xyes
then :
  printf "%s\n" "#define HAVE___MODDI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__udivdi3" "ac_cv_func___udivdi3"
if test "x$ac_cv_func___udivdi3" = xyes
then :
  printf "%s\n" "#define HAVE___UDIVDI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__umoddi3" "ac_cv_func___umoddi3"
if test "x$ac_cv_func___umoddi3" = xyes
then :
  printf "%s\n" "#define HAVE___UMODDI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__ctzdi2" "ac_cv_func___ctzdi2"
if test "x$ac_cv_func___ctzdi2" = xyes
then :
  printf "%s\n" "#define HAVE___CTZDI2 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__ctzsi2" "ac_cv_func___ctzsi2"
if test "x$ac_cv_func___ctzsi2" = xyes
then :
  printf "%s\n" "#define HAVE___CTZSI2 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_uidiv" "ac_cv_func___aeabi_uidiv"
if test "x$ac_cv_func___aeabi_uidiv" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_UIDIV 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_uidivmod" "ac_cv_func___aeabi_uidivmod"
if test "x$ac_cv_func___aeabi_uidivmod" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_UIDIVMOD 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_idiv" "ac_cv_func___aeabi_idiv"
if test "x$ac_cv_func___aeabi_idiv" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_IDIV 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_idivmod" "ac_cv_func___aeabi_idivmod"
if test "x$ac_cv_func___aeabi_idivmod" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_IDIVMOD 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_ulcmp" "ac_cv_func___aeabi_ulcmp"
if test "x$ac_cv_func___aeabi_ulcmp" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_ULCMP 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__muldi3" "ac_cv_func___muldi3"
if test "x$ac_cv_func___muldi3" = xyes
then :
  printf "%s\n" "#define HAVE___MULDI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_lmul" "ac_cv_func___aeabi_lmul"
if test "x$ac_cv_func___aeabi_lmul" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_LMUL 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_memcpy" "ac_cv_func___aeabi_memcpy"
if test "x$ac_cv_func___aeabi_memcpy" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_MEMCPY 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_memcpy4" "ac_cv_func___aeabi_memcpy4"
if test "x$ac_cv_func___aeabi_memcpy4" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_MEMCPY4 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_memcpy8" "ac_cv_func___aeabi_memcpy8"
if test "x$ac_cv_func___aeabi_memcpy8" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_MEMCPY8 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_memclr" "ac_cv_func___aeabi_memclr"
if test "x$ac_cv_func___aeabi_memclr" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_MEMCLR 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_memclr4" "ac_cv_func___aeabi_memclr4"
if test "x$ac_cv_func___aeabi_memclr4" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_MEMCLR4 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_memclr8" "ac_cv_func___aeabi_memclr8"
if test "x$ac_cv_func___aeabi_memclr8" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_MEMCLR8 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_memset" "ac_cv_func___aeabi_memset"
if test "x$ac_cv_func___aeabi_memset" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_MEMSET 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_lasr" "ac_cv_func___aeabi_lasr"
if test "x$ac_cv_func___aeabi_lasr" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_LASR 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_llsl" "ac_cv_func___aeabi_llsl"
if test "x$ac_cv_func___aeabi_llsl" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_LLSL 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__aeabi_llsr" "ac_cv_func___aeabi_llsr"
if test "x$ac_cv_func___aeabi_llsr" = xyes
then :
  printf "%s\n" "#define HAVE___AEABI_LLSR 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "_restgpr_14_x" "ac_cv_func__restgpr_14_x"
if test "x$ac_cv_func__restgpr_14_x" = xyes
then :
  printf "%s\n" "#define HAVE__RESTGPR_14_X 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__ucmpdi2" "ac_cv_func___ucmpdi2"
if test "x$ac_cv_func___ucmpdi2" = xyes
then :
  printf "%s\n" "#define HAVE___UCMPDI2 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__ashldi3" "ac_cv_func___ashldi3"
if test "x$ac_cv_func___ashldi3" = xyes
then :
  printf "%s\n" "#define HAVE___ASHLDI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__ashrdi3" "ac_cv_func___ashrdi3"
if test "x$ac_cv_func___ashrdi3" = xyes
then :
  printf "%s\n" "#define HAVE___ASHRDI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__lshrdi3" "ac_cv_func___lshrdi3"
if test "x$ac_cv_func___lshrdi3" = xyes
then :
  printf "%s\n" "#define HAVE___LSHRDI3 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__bswapsi2" "ac_cv_func___bswapsi2"
if test "x$ac_cv_func___bswapsi2" = xyes
then :
  printf "%s\n" "#define HAVE___BSWAPSI2 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__bswapdi2" "ac_cv_func___bswapdi2"
if test "x$ac_cv_func___bswapdi2" = xyes
then :
  printf "%s\n" "#define HAVE___BSWAPDI2 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__bzero" "ac_cv_func___bzero"
if test "x$ac_cv_func___bzero" = xyes
then :
  printf "%s\n" "#define HAVE___BZERO 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__register_frame_info" "ac_cv_func___register_frame_info"
if test "x$ac_cv_func___register_frame_info" = xyes
then :
  printf "%s\n" "#define HAVE___REGISTER_FRAME_INFO 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__deregister_frame_info" "ac_cv_func___deregister_frame_info"
if test "x$ac_cv_func___deregister_frame_info" = xyes
then :
  printf "%s\n" "#define HAVE___DEREGISTER_FRAME_INFO 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "___chkstk_ms" "ac_cv_func____chkstk_ms"
if test "x$ac_cv_func____chkstk_ms" = xyes
then :
  printf "%s\n" "#define HAVE____CHKSTK_MS 1" >>confdefs.h

fi
ac_fn_c_check_func "$LINENO" "__chkstk_ms" "ac_cv_func___chkstk_ms"
if test "x$ac_cv_func___chkstk_ms" = xyes
then :
  printf "%s\n" "#define HAVE___CHKSTK_MS 1" >>confdefs.h
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether target compiler is working" >&5
printf %s "checking whether target compiler is working... " >&6; }
if test ${holy_cv_prog_target_cc+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_prog_target_cc" >&5
printf "%s\n" "$holy_cv_prog_target_cc" >&6; }
...
if test "x$TARGET_APPLE_LINKER" != x1 ; then
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether ${TARGET_OBJCOPY} works for absolute addresses" >&5
printf %s "checking whether ${TARGET_OBJCOPY} works for absolute addresses... " >&6; }
if test ${holy_cv_prog_objcopy_absolute+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) cat > conftest.c <<\EOF
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_prog_objcopy_absolute" >&5
printf "%s\n" "$holy_cv_prog_objcopy_absolute" >&6; }

if test "x$holy_cv_prog_objcopy_absolute" = xno; then
  as_fn_error $? "holy requires a working absolute objcopy; upgrade your binutils" "$LINENO" 5
fi

fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether linker accepts --build-id=none" >&5
printf %s "checking whether linker accepts --build-id=none... " >&6; }
if test ${holy_cv_prog_ld_build_id_none+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) save_LDFLAGS="$LDFLAGS"
...
then :
  ...
  e) holy_cv_prog_ld_build_id_none=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_prog_ld_build_id_none" >&5
printf "%s\n" "$holy_cv_prog_ld_build_id_none" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if __bss_start is defined by the compiler" >&5
printf %s "checking if __bss_start is defined by the compiler... " >&6; }
if test ${holy_cv_check_uscore_uscore_bss_start_symbol+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
then :
  ...
  e) holy_cv_check_uscore_uscore_bss_start_symbol=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_check_uscore_uscore_bss_start_symbol" >&5
printf "%s\n" "$holy_cv_check_uscore_uscore_bss_start_symbol" >&6; }

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if edata is defined by the compiler" >&5
printf %s "checking if edata is defined by the compiler... " >&6; }
if test ${holy_cv_check_edata_symbol+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
then :
  ...
  e) holy_cv_check_edata_symbol=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_check_edata_symbol" >&5
printf "%s\n" "$holy_cv_check_edata_symbol" >&6; }

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if _edata is defined by the compiler" >&5
printf %s "checking if _edata is defined by the compiler... " >&6; }
if test ${holy_cv_check_uscore_edata_symbol+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
then :
  ...
  e) holy_cv_check_uscore_edata_symbol=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_check_uscore_edata_symbol" >&5
printf "%s\n" "$holy_cv_check_uscore_edata_symbol" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if end is defined by the compiler" >&5
printf %s "checking if end is defined by the compiler... " >&6; }
if test ${holy_cv_check_end_symbol+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_check_end_symbol" >&5
printf "%s\n" "$holy_cv_check_end_symbol" >&6; }

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if _end is defined by the compiler" >&5
printf %s "checking if _end is defined by the compiler... " >&6; }
if test ${holy_cv_check_uscore_end_symbol+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
then :
  ...
  e) holy_cv_check_uscore_end_symbol=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_check_uscore_end_symbol" >&5
printf "%s\n" "$holy_cv_check_uscore_end_symbol" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether nm works" >&5
printf %s "checking whether nm works... " >&6; }
if test ${holy_cv_prog_nm_works+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_prog_nm_works" >&5
printf "%s\n" "$holy_cv_prog_nm_works" >&6; }

if test "x$holy_cv_prog_nm_works" != xyes; then
  as_fn_error $? "nm does not work" "$LINENO" 5
fi

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether nm accepts -P" >&5
printf %s "checking whether nm accepts -P... " >&6; }
if test ${holy_cv_prog_nm_minus_p+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_prog_nm_minus_p" >&5
printf "%s\n" "$holy_cv_prog_nm_minus_p" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether nm accepts --defined-only" >&5
printf %s "checking whether nm accepts --defined-only... " >&6; }
if test ${holy_cv_prog_nm_defined_only+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_prog_nm_defined_only" >&5
printf "%s\n" "$holy_cv_prog_nm_defined_only" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -nostdinc -isystem works" >&5
printf %s "checking whether -nostdinc -isystem works... " >&6; }
if test ${holy_cv_cc_isystem+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_isystem=yes
else case e in #(
  e) holy_cv_cc_isystem=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_isystem" >&5
printf "%s\n" "$holy_cv_cc_isystem" >&6; }

if test x"$holy_cv_cc_isystem" = xyes ; then
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -Wtrampolines work" >&5
printf %s "checking whether -Wtrampolines work... " >&6; }
if test ${holy_cv_cc_wtrampolines+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e)
...
if ac_fn_c_try_compile "$LINENO"
then :
  holy_cv_cc_wtrampolines=yes
else case e in #(
  e) holy_cv_cc_wtrampolines=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext
 ;;
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $holy_cv_cc_wtrampolines" >&5
printf "%s\n" "$holy_cv_cc_wtrampolines" >&6; }

if test x"$holy_cv_cc_wtrampolines" = xyes ; then
...
if test ${enable_mm_debug+y}
then :
  enableval=$enable_mm_debug;
printf "%s\n" "#define MM_DEBUG 1" >>confdefs.h

fi


# Check whether --enable-cache-stats was given.
if test ${enable_cache_stats+y}
then :
...
# Check whether --enable-holy-emu-sdl was given.
if test ${enable_holy_emu_sdl+y}
then :
  enableval=$enable_holy_emu_sdl;
fi


# Check whether --enable-holy-emu-pci was given.
if test ${enable_holy_emu_pci+y}
then :
  enableval=$enable_holy_emu_pci;
fi


if test "$platform" = emu; then

if test x"$enable_holy_emu_sdl" = xno ; then
  holy_emu_sdl_excuse="explicitly disabled"
fi
if [ x"$holy_emu_sdl_excuse" = x ]; then
    # Check for libSDL libraries.
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for SDL_Init in -lSDL" >&5
printf %s "checking for SDL_Init in -lSDL... " >&6; }
if test ${ac_cv_lib_SDL_SDL_Init+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) ac_check_lib_save_LIBS=$LIBS
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_SDL_SDL_Init" >&5
printf "%s\n" "$ac_cv_lib_SDL_SDL_Init" >&6; }
if test "x$ac_cv_lib_SDL_SDL_Init" = xyes
then :
  LIBSDL="-lSDL"
else case e in #(
  e) holy_emu_sdl_excuse="libSDL libraries are required to build \`holy-emu' with SDL support" ;;
esac
fi
...
do :
  ac_fn_c_check_header_compile "$LINENO" "SDL/SDL.h" "ac_cv_header_SDL_SDL_h" "$ac_includes_default"
if test "x$ac_cv_header_SDL_SDL_h" = xyes
then :
  printf "%s\n" "#define HAVE_SDL_SDL_H 1" >>confdefs.h

else case e in #(
  e) holy_emu_sdl_excuse="libSDL header file is required to build \`holy-emu' with SDL support" ;;
esac
fi
...
if test x"enable_holy_emu_sdl" = xyes && test x"$holy_emu_sdl_excuse" != x ; then
  as_fn_error $? "SDL support for holy-emu was explicitly requested but can't be compiled ($holy_emu_sdl_excuse)" "$LINENO" 5
fi
if test x"$holy_emu_sdl_excuse" = x ; then
enable_holy_emu_sdl=yes
else
enable_holy_emu_sdl=no
fi

if test x"$enable_holy_emu_pci" != xyes ; then
   holy_emu_pci_excuse="not enabled"
fi

if [ x"$holy_emu_pci_excuse" = x ]; then
      # Check for libpci libraries.
   { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for pci_system_init in -lpciaccess" >&5
printf %s "checking for pci_system_init in -lpciaccess... " >&6; }
if test ${ac_cv_lib_pciaccess_pci_system_init+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) ac_check_lib_save_LIBS=$LIBS
...
then :
  ...
  e) ac_cv_lib_pciaccess_pci_system_init=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_pciaccess_pci_system_init" >&5
printf "%s\n" "$ac_cv_lib_pciaccess_pci_system_init" >&6; }
if test "x$ac_cv_lib_pciaccess_pci_system_init" = xyes
then :
  LIBPCIACCESS="-lpciaccess"
else case e in #(
  e) holy_emu_pci_excuse="need libpciaccess library" ;;
esac
fi
...
do :
  ac_fn_c_check_header_compile "$LINENO" "pciaccess.h" "ac_cv_header_pciaccess_h" "$ac_includes_default"
if test "x$ac_cv_header_pciaccess_h" = xyes
then :
  printf "%s\n" "#define HAVE_PCIACCESS_H 1" >>confdefs.h

else case e in #(
  e) holy_emu_pci_excuse="need libpciaccess headers" ;;
esac
fi
...
if test x"$holy_emu_pci_excuse" = x ; then
enable_holy_emu_pci=yes
else

enable_holy_emu_pci=no
fi
...
# Check whether --enable-holy-mkfont was given.
if test ${enable_holy_mkfont+y}
then :
  enableval=$enable_holy_mkfont;
fi

if test x"$enable_holy_mkfont" = xno ; then
  holy_mkfont_excuse="explicitly disabled"
fi

if test x"$holy_mkfont_excuse" = x ; then
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_FREETYPE+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$FREETYPE"; then
...
if test -n "$FREETYPE"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $FREETYPE" >&5
printf "%s\n" "$FREETYPE" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_ac_ct_FREETYPE+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$ac_ct_FREETYPE"; then
...
if test -n "$ac_ct_FREETYPE"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_ct_FREETYPE" >&5
printf "%s\n" "$ac_ct_FREETYPE" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
...
ac_tool_warned=yes ;;
  ...
  if test "x$FREETYPE" = x ; then
    holy_mkfont_excuse="need freetype2 library"
  fi
fi
...
then :
  ...
  e) holy_mkfont_excuse="need freetype2 headers" ;;
esac
fi
...
then :
  ...
  e) holy_mkfont_excuse="freetype2 library unusable" ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
if test x"$enable_holy_mkfont" = xyes && test x"$holy_mkfont_excuse" != x ; then
  as_fn_error $? "holy-mkfont was explicitly requested but can't be compiled ($holy_mkfont_excuse)" "$LINENO" 5
fi
if test x"$holy_mkfont_excuse" = x ; then
enable_holy_mkfont=yes
else
enable_holy_mkfont=no
fi
...
if ac_fn_c_compute_int "$LINENO" "sizeof (void *)" "BUILD_SIZEOF_VOID_P"        ""
then :

fi

if ac_fn_c_compute_int "$LINENO" "sizeof (long)" "BUILD_SIZEOF_LONG"        ""
then :
...
if test x"$holy_build_mkfont_excuse" = x ; then
  # Check for freetype libraries.
  for ac_prog in freetype-config
do
  # Extract the first word of "$ac_prog", so it can be a program name with args.
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
printf %s "checking for $ac_word... " >&6; }
if test ${ac_cv_prog_BUILD_FREETYPE+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) if test -n "$BUILD_FREETYPE"; then
...
if test -n "$BUILD_FREETYPE"; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $BUILD_FREETYPE" >&5
printf "%s\n" "$BUILD_FREETYPE" >&6; }
else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
  ...
  if test "x$BUILD_FREETYPE" = x ; then
    holy_build_mkfont_excuse="need freetype2 library"
  fi
fi
...
then :
  ...
  e) holy_build_mkfont_excuse="need freetype2 headers" ;;
esac
fi
...
then :
  ...
  e) holy_build_mkfont_excuse="freetype2 library unusable" ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
if test x"$enable_build_holy_mkfont" = xyes && test x"$holy_build_mkfont_excuse" != x ; then
  as_fn_error $? "build-holy-mkfont was explicitly requested but can't be compiled ($holy_build_mkfont_excuse)" "$LINENO" 5
fi
if test x"$holy_build_mkfont_excuse" = x ; then
  enable_build_holy_mkfont=yes
else
  enable_build_holy_mkfont=no
fi
if test x"$enable_build_holy_mkfont" = xno && ( test "x$platform" = xqemu || test "x$platform" = xloongson || test "x$platform" = xqemu_mips || test "x$platform" = xcoreboot ); then
  if test x"$holy_build_mkfont_excuse" = x ; then
    as_fn_error $? "qemu, coreboot and loongson ports need build-time holy-mkfont" "$LINENO" 5
  else
    as_fn_error $? "qemu, coreboot and loongson ports need build-time holy-mkfont ($holy_build_mkfont_excuse)" "$LINENO" 5
  fi
fi
...
# Check whether --enable-holy-themes was given.
if test ${enable_holy_themes+y}
then :
  enableval=$enable_holy_themes;
fi

if test x"$enable_holy_themes" = xno ; then
  starfield_excuse="explicitly disabled"
fi

if test x"$starfield_excuse" = x && test x"$enable_build_holy_mkfont" = xno ; then
  starfield_excuse="No build-time holy-mkfont"
fi
...
if test x"$enable_holy_themes" = xyes &&  test x"$starfield_excuse" != x; then
...
if test x"$enable_build_holy_mkfont" = xno ; then
  FONT_SOURCE=
fi
...
if test "x$FONT_SOURCE" = x && ( test "x$platform" = xqemu || test "x$platform" = xloongson || test "x$platform" = xqemu_mips || test "x$platform" = xcoreboot ); then
  if test x"$holy_build_mkfont_excuse" = x ; then
    ...
    as_fn_error $? "qemu, coreboot and loongson ports need unifont ($holy_build_mkfont_excuse)" "$LINENO" 5
  fi
fi



if test x"$FONT_SOURCE" = x &&  test x"$DJVU_FONT_SOURCE" = x && test x"$holy_build_mkfont_excuse" = x; then
  holy_build_mkfont_excuse="no fonts"
fi


# Check whether --enable-holy-mount was given.
if test ${enable_holy_mount+y}
then :
  enableval=$enable_holy_mount;
fi

if test x"$enable_holy_mount" = xno ; then
  holy_mount_excuse="explicitly disabled"
fi

if test x"$holy_mount_excuse" = x ; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for fuse_main_real in -lfuse" >&5
printf %s "checking for fuse_main_real in -lfuse... " >&6; }
if test ${ac_cv_lib_fuse_fuse_main_real+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) ac_check_lib_save_LIBS=$LIBS
...
then :
  ...
  e) ac_cv_lib_fuse_fuse_main_real=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_fuse_fuse_main_real" >&5
printf "%s\n" "$ac_cv_lib_fuse_fuse_main_real" >&6; }
if test "x$ac_cv_lib_fuse_fuse_main_real" = xyes
then :
  ...
  e) holy_mount_excuse="need FUSE library" ;;
esac
fi
...
do :
  ac_fn_c_check_header_compile "$LINENO" "fuse/fuse.h" "ac_cv_header_fuse_fuse_h" "$ac_includes_default"
if test "x$ac_cv_header_fuse_fuse_h" = xyes
then :
  printf "%s\n" "#define HAVE_FUSE_FUSE_H 1" >>confdefs.h

else case e in #(
  e) holy_mount_excuse="need FUSE headers" ;;
esac
fi
...
if test x"$enable_holy_mount" = xyes && test x"$holy_mount_excuse" != x ; then
  as_fn_error $? "holy-mount was explicitly requested but can't be compiled ($holy_mount_excuse)" "$LINENO" 5
fi
if test x"$holy_mount_excuse" = x ; then
enable_holy_mount=yes
else
enable_holy_mount=no
fi
...
then :
  ...
  e) device_mapper_excuse="need libdevmapper header" ;;
esac
fi
...
then :
  ...
  e) device_mapper_excuse="need devmapper library" ;;
esac
fi
...
if test x"$device_mapper_excuse" = x ; then
  # Check for device-mapper library.
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for dm_log_with_errno_init in -ldevmapper" >&5
printf %s "checking for dm_log_with_errno_init in -ldevmapper... " >&6; }
if test ${ac_cv_lib_devmapper_dm_log_with_errno_init+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) ac_check_lib_save_LIBS=$LIBS
...
extern "C"
#endif
char dm_log_with_errno_init (void);
int
main (void)
{
...
then :
  ...
  e) ac_cv_lib_devmapper_dm_log_with_errno_init=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_devmapper_dm_log_with_errno_init" >&5
printf "%s\n" "$ac_cv_lib_devmapper_dm_log_with_errno_init" >&6; }
if test "x$ac_cv_lib_devmapper_dm_log_with_errno_init" = xyes
then :
  printf "%s\n" "#define HAVE_LIBDEVMAPPER 1" >>confdefs.h
...
printf "%s\n" "#define HAVE_DEVICE_MAPPER 1" >>confdefs.h
...
if test x$host_kernel = xkfreebsd; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for geom_gettree in -lgeom" >&5
printf %s "checking for geom_gettree in -lgeom... " >&6; }
if test ${ac_cv_lib_geom_geom_gettree+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) ac_check_lib_save_LIBS=$LIBS
...
then :
  ...
  e) ac_cv_lib_geom_geom_gettree=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_geom_geom_gettree" >&5
printf "%s\n" "$ac_cv_lib_geom_geom_gettree" >&6; }
if test "x$ac_cv_lib_geom_geom_gettree" = xyes
then :
  printf "%s\n" "#define HAVE_LIBGEOM 1" >>confdefs.h

  LIBS="-lgeom $LIBS"

else case e in #(
  e) as_fn_error $? "Your platform requires libgeom" "$LINENO" 5 ;;
esac
fi
...
then :
  ...
  e) liblzma_excuse="need lzma library" ;;
esac
fi
...
then :
  ...
  e) liblzma_excuse="need lzma header" ;;
esac
fi
...
printf "%s\n" "#define USE_LIBLZMA 1" >>confdefs.h
...
ac_fn_c_check_header_compile "$LINENO" "libnvpair.h" "ac_cv_header_libnvpair_h" "$ac_includes_default"
if test "x$ac_cv_header_libnvpair_h" = xyes
then :
  printf "%s\n" "#define HAVE_LIBNVPAIR_H 1" >>confdefs.h
...
if test x"$libzfs_excuse" = x ; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for libzfs_init in -lzfs" >&5
printf %s "checking for libzfs_init in -lzfs... " >&6; }
if test ${ac_cv_lib_zfs_libzfs_init+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) ac_check_lib_save_LIBS=$LIBS
...
then :
  ...
  e) ac_cv_lib_zfs_libzfs_init=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_zfs_libzfs_init" >&5
printf "%s\n" "$ac_cv_lib_zfs_libzfs_init" >&6; }
if test "x$ac_cv_lib_zfs_libzfs_init" = xyes
then :
  printf "%s\n" "#define HAVE_LIBZFS 1" >>confdefs.h

  LIBS="-lzfs $LIBS"

else case e in #(
  e) libzfs_excuse="need zfs library" ;;
esac
fi
...
if test x"$libzfs_excuse" = x ; then
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for nvlist_lookup_string in -lnvpair" >&5
printf %s "checking for nvlist_lookup_string in -lnvpair... " >&6; }
if test ${ac_cv_lib_nvpair_nvlist_lookup_string+y}
then :
  printf %s "(cached) " >&6
else case e in #(
  e) ac_check_lib_save_LIBS=$LIBS
...
extern "C"
#endif
char nvlist_lookup_string (void);
int
main (void)
{
...
then :
  ...
  e) ac_cv_lib_nvpair_nvlist_lookup_string=no ;;
esac
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_nvpair_nvlist_lookup_string" >&5
printf "%s\n" "$ac_cv_lib_nvpair_nvlist_lookup_string" >&6; }
if test "x$ac_cv_lib_nvpair_nvlist_lookup_string" = xyes
then :
  printf "%s\n" "#define HAVE_LIBNVPAIR 1" >>confdefs.h

  LIBS="-lnvpair $LIBS"

else case e in #(
  e) libzfs_excuse="need nvpair library" ;;
esac
fi
...
if test x$target_cpu = xi386 -a x$platform = xqemu
then :
  holy_BOOT_MACHINE_LINK_ADDR=0xffe00
...
holy_TARGET_CPU="${target_cpu}"
holy_PLATFORM="${platform}"
...
#
 ...
 if test x$enable_holy_emu_sdl = xyes; then
  COND_holy_EMU_SDL_TRUE=
  COND_holy_EMU_SDL_FALSE='#'
else
  COND_holy_EMU_SDL_TRUE='#'
 ...
 if test x$enable_holy_mkfont = xyes; then
  COND_holy_MKFONT_TRUE=
  COND_holy_MKFONT_FALSE='#'
else
  COND_holy_MKFONT_TRUE='#'
 ...
 if test x$enable_holy_mount = xyes; then
  COND_holy_MOUNT_TRUE=
  COND_holy_MOUNT_FALSE='#'
else
  COND_holy_MOUNT_TRUE='#'
...
holy_libdir="$(eval echo "$libdir")"
holy_localedir="$(eval echo "$localedir")"
holy_datadir="$(eval echo "$datadir")"
holy_sysconfdir="$(eval echo "$sysconfdir")"

printf "%s\n" "#define LOCALEDIR \"$holy_localedir\"" >>confdefs.h


printf "%s\n" "#define holy_LIBDIR \"$holy_libdir\"" >>confdefs.h


printf "%s\n" "#define holy_DATADIR \"$holy_datadir\"" >>confdefs.h


printf "%s\n" "#define holy_SYSCONFDIR \"$holy_sysconfdir\"" >>confdefs.h
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether ln -s can handle directories properly" >&5
printf %s "checking whether ln -s can handle directories properly... " >&6; }
...
rm -rf testdir
  ...
  if test x"$link_dir" = xyes ; then
    ac_config_links="$ac_config_links include/holy/cpu:include/holy/$cpudir"

    if test "$platform" != emu ; then
      ac_config_links="$ac_config_links include/holy/machine:include/holy/$cpudir/$platform"

    fi
  else
    mkdir -p include/holy 2>/dev/null
    rm -rf include/holy/cpu
    cp -rp $srcdir/include/holy/$cpudir include/holy/cpu 2>/dev/null
    if test "$platform" != emu ; then
      rm -rf include/holy/machine
      cp -rp $srcdir/include/holy/$cpudir/$platform include/holy/machine 2>/dev/null
    fi
  fi
else
  # Just enough to stop the compiler failing with -I$(srcdir)/include.
  mkdir -p include 2>/dev/null
  rm -rf include/holy/cpu include/holy/machine
fi

ac_config_files="$ac_config_files Makefile"

ac_config_files="$ac_config_files holy-core/Makefile"

ac_config_files="$ac_config_files holy-core/gnulib/Makefile"
...
if test -z "${COND_holy_EMU_SDL_TRUE}" && test -z "${COND_holy_EMU_SDL_FALSE}"; then
  as_fn_error $? "conditional \"COND_holy_EMU_SDL\" was never defined.
Usually this means the macro was only invoked conditionally." "$LINENO" 5
fi
if test -z "${COND_holy_EMU_PCI_TRUE}" && test -z "${COND_holy_EMU_PCI_FALSE}"; then
  as_fn_error $? "conditional \"COND_holy_EMU_PCI\" was never defined.
Usually this means the macro was only invoked conditionally." "$LINENO" 5
fi
if test -z "${COND_holy_MKFONT_TRUE}" && test -z "${COND_holy_MKFONT_FALSE}"; then
  as_fn_error $? "conditional \"COND_holy_MKFONT\" was never defined.
Usually this means the macro was only invoked conditionally." "$LINENO" 5
fi
if test -z "${COND_holy_MOUNT_TRUE}" && test -z "${COND_holy_MOUNT_FALSE}"; then
  as_fn_error $? "conditional \"COND_holy_MOUNT\" was never defined.
Usually this means the macro was only invoked conditionally." "$LINENO" 5
fi
...
ac_log="
This file was extended by holy $as_me 2.02, which was
generated by GNU Autoconf 2.72.  Invocation command line was
...
do
  case $ac_config_target in
    ...
    "include/holy/cpu") CONFIG_LINKS="$CONFIG_LINKS include/holy/cpu:include/holy/$cpudir" ;;
    "include/holy/machine") CONFIG_LINKS="$CONFIG_LINKS include/holy/machine:include/holy/$cpudir/$platform" ;;
    "Makefile") CONFIG_FILES="$CONFIG_FILES Makefile" ;;
    "holy-core/Makefile") CONFIG_FILES="$CONFIG_FILES holy-core/Makefile" ;;
    "holy-core/gnulib/Makefile") CONFIG_FILES="$CONFIG_FILES holy-core/gnulib/Makefile" ;;
...
echo "*******************************************************"
echo holy2 will be compiled with following components:
echo Platform: "$target_cpu"-"$platform"
if [ x"$platform" = xemu ]; then
if [ x"$holy_emu_sdl_excuse" = x ]; then
echo SDL support for holy-emu: Yes
else
echo SDL support for holy-emu: No "($holy_emu_sdl_excuse)"
fi
if [ x"$holy_emu_pci_excuse" = x ]; then
echo PCI support for holy-emu: Yes
else
echo PCI support for holy-emu: No "($holy_emu_pci_excuse)"
fi
fi
if test x"$device_mapper_excuse" = x ; then
...
if [ x"$holy_mkfont_excuse" = x ]; then
echo holy-mkfont: Yes
else
echo holy-mkfont: No "($holy_mkfont_excuse)"
fi
if [ x"$holy_mount_excuse" = x ]; then
echo holy-mount: Yes
else
echo holy-mount: No "($holy_mount_excuse)"
fi
...
if [ x"$holy_build_mkfont_excuse" = x ]; then
  echo Build-time holy-mkfont: Yes
  if test "x$FONT_SOURCE" = x ; then
  ...
  echo Build-time holy-mkfont: No "($holy_build_mkfont_excuse)"
  echo "Without unifont (no build-time holy-mkfont)"
fi
if test x"$liblzma_excuse" != x ; then
