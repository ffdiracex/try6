...
struct btrfs_ioctl_dev_info_args
{
  ...
  holy_uint64_t bytes_used;
  holy_uint64_t total_bytes;
  holy_uint64_t unused[379];
  holy_uint8_t path[1024];
};

struct btrfs_ioctl_fs_info_args
{
  holy_uint64_t max_id;
  holy_uint64_t num_devices;
  holy_uint8_t fsid[16];
  holy_uint64_t reserved[124];
};

struct btrfs_ioctl_ino_lookup_args
{
  holy_uint64_t treeid;
  holy_uint64_t objectid;
  char name[4080];
};

struct btrfs_ioctl_search_key
{
  holy_uint64_t tree_id;
  holy_uint64_t min_objectid;
  holy_uint64_t max_objectid;
  holy_uint64_t min_offset;
  holy_uint64_t max_offset;
  holy_uint64_t min_transid;
  holy_uint64_t max_transid;
  holy_uint32_t min_type;
  holy_uint32_t max_type;
  holy_uint32_t nr_items;
  holy_uint32_t unused[9];
};

struct btrfs_ioctl_search_args {
  struct btrfs_ioctl_search_key key;
  holy_uint64_t buf[(4096 - sizeof(struct btrfs_ioctl_search_key))
...
static int
holy_util_is_imsm (const char *os_dev);
...
static char **
holy_util_raid_getmembers (const char *name, int bootable)
{
  int fd, ret, i, j;
  char **devicelist;
  ...
  if (fd == -1)
    holy_util_error (_("cannot open `%s': %s"), name, strerror (errno));

  ret = ioctl (fd, RAID_VERSION, &version);
  if (ret != 0)
    holy_util_error (_("ioctl RAID_VERSION error: %s"), strerror (errno));

  if ((version.major != 0 || version.minor != 90)
      && (version.major != 1 || version.minor != 0)
      && (version.major != 1 || version.minor != 1)
      && (version.major != 1 || version.minor != 2))
    holy_util_error (_("unsupported RAID version: %d.%d"),
  ...
  if (ret != 0)
    holy_util_error (_("ioctl GET_ARRAY_INFO error: %s"), strerror (errno));
  ...
  for (i = 0, j = 0; j < info.nr_disks; i++)
    {
      ...
      if (ret != 0)
	holy_util_error (_("ioctl GET_DISK_INFO error: %s"), strerror (errno));
      
      if (disk.state & (1 << MD_DISK_REMOVED))
	continue;

      if (disk.state & (1 << MD_DISK_ACTIVE))
	devicelist[j] = holy_find_device (NULL,
...
static char **
holy_find_root_devices_from_btrfs (const char *dir)
{
  int fd;
...
get_btrfs_fs_prefix (const char *mount_path)
{
  ...
  struct stat st;
  int fd;
  holy_uint64_t tree_id, inode_id;
  char *ret = NULL;
  ...
  args.objectid = holy_BTRFS_TREE_ROOT_OBJECTID;
  ...
  while (tree_id != holy_BTRFS_ROOT_VOL_OBJECTID
	 || inode_id != holy_BTRFS_TREE_ROOT_OBJECTID)
    {
      const char *name;
      size_t namelen;
      ...
      if (inode_id == holy_BTRFS_TREE_ROOT_OBJECTID)
	{
	  struct holy_btrfs_root_backref *br;
	  ...
	  sargs.key.min_type = holy_BTRFS_ITEM_TYPE_ROOT_BACKREF;
	  sargs.key.max_type = holy_BTRFS_ITEM_TYPE_ROOT_BACKREF;
	  ...
	  tree_id = sargs.buf[2];
	  br = (struct holy_btrfs_root_backref *) (sargs.buf + 4);
	  inode_id = holy_le_to_cpu64 (br->inode_id);
	  name = br->name;
	  namelen = holy_le_to_cpu16 (br->n);
	}
      else
	{
	  struct holy_btrfs_inode_ref *ir;
	  ...
	  sargs.key.min_transid = 0;
	  sargs.key.max_transid = ~0ULL;
	  sargs.key.min_type = holy_BTRFS_ITEM_TYPE_INODE_REF;
	  sargs.key.max_type = holy_BTRFS_ITEM_TYPE_INODE_REF;
	  ...
	  ir = (struct holy_btrfs_inode_ref *) (sargs.buf + 4);
	  name = ir->name;
	  namelen = holy_le_to_cpu16 (ir->n);
	}
      old = ret;
...
holy_find_root_devices_from_mountinfo (const char *dir, char **relroot)
{
  ...
  char *buf = NULL;
  size_t len = 0;
  holy_size_t entry_len, entry_max = 4;
...
again:
  fp = holy_util_fopen ("/proc/self/mountinfo", "r");
  if (! fp)
  ...
  for (i = entry_len - 1; i >= 0; i--)
	...
	continue;
      ...
      if (holy_strcmp (entries[i].fstype, "fuse.zfs") == 0
	  || holy_strcmp (entries[i].fstype, "zfs") == 0)
	{
	  char *slash;
	  slash = strchr (entries[i].device, '/');
	  if (slash)
	    *slash = 0;
	  ret = holy_util_find_root_devices_from_poolname (entries[i].device);
	  if (slash)
	    *slash = '/';
	  if (relroot)
	    {
      ...
      else if (holy_strcmp (entries[i].fstype, "btrfs") == 0)
	{
	  ret = holy_find_root_devices_from_btrfs (dir);
	  fs_prefix = get_btrfs_fs_prefix (entries[i].enc_path);
	}
      else if (!retry && holy_strcmp (entries[i].fstype, "autofs") == 0)
	{
	...
	{
	  char *ptr;
	  holy_size_t enc_root_len = strlen (fs_prefix);
	  holy_size_t enc_path_len = strlen (entries[i].enc_path);
	  holy_size_t dir_strlen = strlen (dir);
	  *relroot = xmalloc (enc_root_len +
			      2 + dir_strlen);
	  ptr = holy_stpcpy (*relroot, fs_prefix);
	  if (dir_strlen > enc_path_len)
		...
		ptr--;
	      if (dir[enc_path_len] != '/')
		*ptr++ = '/';
	      ptr = holy_stpcpy (ptr, dir + enc_path_len);
	    }
	  *ptr = 0;
	}
      ...
      while (getline (&buf, &len, mdadm) > 0)
	{
	  if (strncmp (buf, "MD_CONTAINER=", sizeof ("MD_CONTAINER=") - 1) == 0
	      && !container_seen)
	    {
	      ...
	      for (; ptr >= newdev && (*ptr == '\n' || *ptr == '\r'); ptr--);
	      ptr[1] = 0;
	      holy_util_info ("Container of %s is %s", dev, newdev);
	      dev = newdev;
...
static char *
holy_util_get_raid_holy_dev (const char *os_dev)
{
  char *holy_dev = NULL;
  if (os_dev[7] == '_' && os_dev[8] == 'd')
	...
	*q = ',';
    ...
    holy_util_error (_("unknown kind of RAID device `%s'"), os_dev);

  {
	...
	if (q >= os_dev && *q == 'p')
	  {
	    free (holy_dev);
	    holy_dev = xasprintf ("mduuid/%s,%s", mdadm_name, q + 1);
	    goto done;
	  }
	free (holy_dev);
	holy_dev = xasprintf ("mduuid/%s", mdadm_name);
...
holy_util_get_dev_abstraction_os (const char *os_dev)
{
  ...
  /* Check for RAID.  */
  if (!strncmp (os_dev, "/dev/md", 7) && ! holy_util_device_is_mapped (os_dev)
      && !holy_util_is_imsm (os_dev))
    return holy_DEV_ABSTRACTION_RAID;
			  ...
			  enum holy_dev_abstraction_types ab)
{
  switch (ab)
    {
    case holy_DEV_ABSTRACTION_RAID:
      {
	char **devicelist = holy_util_raid_getmembers (os_dev, 0);
	int i;
	for (i = 0; devicelist[i];i++)
	  {
	    holy_util_pull_device (devicelist[i]);
	    free (devicelist[i]);
	  }
	free (devicelist);
      }
  ...
  switch (holy_util_get_dev_abstraction (os_dev))
    {
	 ...
	 in holy_util_get_devmapper_holy_dev and this point isn't reached.
       */
    ...
    case holy_DEV_ABSTRACTION_RAID:
      holy_dev = holy_util_get_raid_holy_dev (os_dev);
      break;

    default:  /* holy_DEV_ABSTRACTION_NONE */
      break;
    }
...
holy_make_system_path_relative_to_its_root_os (const char *path)
{
  char *bind = NULL;
  holy_size_t len;
  holy_free (holy_find_root_devices_from_mountinfo (path, &bind));
  if (bind && bind[0])
    {
      len = strlen (bind);
