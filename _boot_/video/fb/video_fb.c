...
typedef holy_err_t (*holy_video_fb_doublebuf_update_screen_t) (void);
...
static struct
{
  struct holy_video_fbrender_target *render_target;
  struct holy_video_fbrender_target *back_target;
  ...
  holy_video_fb_set_page_t set_page;
  char *offscreen_buffer;
  holy_video_fb_doublebuf_update_screen_t update_screen;
} framebuffer;

/* Specify "standard" VGA palette, some video cards may
   need this and this will also be used when using RGB modes.  */
struct holy_video_palette_data holy_video_fbstd_colors[holy_VIDEO_FBSTD_EXT_NUMCOLORS] =
  {
...
holy_err_t
holy_video_fb_init (void)
{
  holy_free (framebuffer.palette);
...
holy_err_t
holy_video_fb_fini (void)
{
  /* TODO: destroy render targets.  */

  holy_free (framebuffer.offscreen_buffer);
  holy_free (framebuffer.palette);
  framebuffer.render_target = 0;
  framebuffer.back_target = 0;
  framebuffer.palette = 0;
...
holy_err_t
holy_video_fb_get_info (struct holy_video_mode_info *mode_info)
{
  /* Copy mode info from active render target.  */
  holy_memcpy (mode_info, &framebuffer.render_target->mode_info,
               sizeof (struct holy_video_mode_info));

  return holy_ERR_NONE;
}

holy_err_t
holy_video_fb_get_palette (unsigned int start, unsigned int count,
...
holy_err_t
holy_video_fb_set_palette (unsigned int start, unsigned int count,
			   struct holy_video_palette_data *palette_data)
{
  unsigned i;
  if (start + count > framebuffer.palette_size)
    {
      ...
      framebuffer.palette = holy_realloc (framebuffer.palette,
...
holy_video_fb_set_area (void)
{
  ...
  unsigned int max_x1 = holy_max (viewport_x1, region_x1);
  unsigned int min_x2 = holy_min (viewport_x2, region_x2);
  unsigned int max_y1 = holy_max (viewport_y1, region_y1);
  unsigned int min_y2 = holy_min (viewport_y2, region_y2);
...
holy_err_t
holy_video_fb_set_viewport (unsigned int x, unsigned int y,
			    unsigned int width, unsigned int height)
{
  ...
  if (framebuffer.render_target->area_enabled)
    holy_video_fb_set_area ();

  return holy_ERR_NONE;
}

holy_err_t
holy_video_fb_get_viewport (unsigned int *x, unsigned int *y,
			    unsigned int *width, unsigned int *height)
{
...
holy_err_t
holy_video_fb_set_region (unsigned int x, unsigned int y,
                          unsigned int width, unsigned int height)
{
...
holy_err_t
holy_video_fb_get_region (unsigned int *x, unsigned int *y,
                          unsigned int *width, unsigned int *height)
{
...
holy_err_t
holy_video_fb_set_area_status (holy_video_area_status_t area_status)
{
...
holy_err_t
holy_video_fb_get_area_status (holy_video_area_status_t *area_status)
{
  if (!area_status)
    return holy_ERR_NONE;

  if (framebuffer.render_target->area_enabled)
    *area_status = holy_VIDEO_AREA_ENABLED;
  else
    *area_status = holy_VIDEO_AREA_DISABLED;
...
holy_video_color_t
holy_video_fb_map_color (holy_uint32_t color_name)
{
  ...
  if (color_name < framebuffer.palette_size)
    {
      if ((framebuffer.render_target->mode_info.mode_type
      ...
      else
        {
          ...
          color = holy_video_fb_map_rgb (framebuffer.palette[color_name].r,
...
holy_video_color_t
holy_video_fb_map_rgb (holy_uint8_t red, holy_uint8_t green,
		       holy_uint8_t blue)
{
  if ((framebuffer.render_target->mode_info.mode_type
       & holy_VIDEO_MODE_TYPE_INDEX_COLOR) != 0)
    {
      ...
      return minindex;
            ...
            & holy_VIDEO_MODE_TYPE_1BIT_BITMAP) != 0)
    {
    ...
    {
      holy_uint32_t value;
      holy_uint8_t alpha = 255; /* Opaque color.  */
...
holy_video_color_t
holy_video_fb_map_rgba (holy_uint8_t red, holy_uint8_t green,
...
holy_err_t
holy_video_fb_unmap_color (holy_video_color_t color,
			   holy_uint8_t *red, holy_uint8_t *green,
			   holy_uint8_t *blue, holy_uint8_t *alpha)
{
  struct holy_video_fbblit_info target_info;

  target_info.mode_info = &framebuffer.render_target->mode_info;
  target_info.data = framebuffer.render_target->data;

  holy_video_fb_unmap_color_int (&target_info, color, red, green, blue, alpha);
...
holy_err_t
holy_video_fb_fill_rect (holy_video_color_t color, int x, int y,
			 unsigned int width, unsigned int height)
{
  struct holy_video_fbblit_info target;
  unsigned int area_x;
  unsigned int area_y;
  ...
  holy_video_fb_fill_dispatch (&target, color, x, y,
			       width, height);
  return holy_ERR_NONE;
}

static inline holy_err_t __attribute__ ((always_inline))
holy_video_fb_blit_source (struct holy_video_fbblit_info *source,
                           ...
                           unsigned int width, unsigned int height)
{
  ...
  /* Do actual blitting.  */
  dirty (y, height);
  holy_video_fb_dispatch_blit (&target, source, oper, x, y, width, height,
                               offset_x, offset_y);
...
holy_err_t
holy_video_fb_blit_bitmap (struct holy_video_bitmap *bitmap,
                           enum holy_video_blit_operators oper, int x, int y,
                           int offset_x, int offset_y,
                           unsigned int width, unsigned int height)
{
  struct holy_video_fbblit_info source_info;
  source_info.mode_info = &bitmap->mode_info;
  source_info.data = bitmap->data;

  return holy_video_fb_blit_source (&source_info, oper, x, y,
                                    offset_x, offset_y, width, height);
}

holy_err_t
holy_video_fb_blit_render_target (struct holy_video_fbrender_target *source,
...
holy_err_t
holy_video_fb_scroll (holy_video_color_t color, int dx, int dy)
{
  int width;
  int height;
  int src_x;
  ...
  width = framebuffer.render_target->viewport.width - holy_abs (dx);
  height = framebuffer.render_target->viewport.height - holy_abs (dy);
  ...
  if ((holy_abs (dx) < framebuffer.render_target->viewport.width)
       && (holy_abs (dy) < framebuffer.render_target->viewport.height))
    {
...
#define DO_SCROLL                                                    \
      ...
      if (dy < 0 || (dy == 0 && dx < 0))                             \
	{                                                            \
	  dst = (void *) holy_video_fb_get_video_ptr (&target,       \
						      dst_x, dst_y); \
	  src = (void *) holy_video_fb_get_video_ptr (&target,	     \
						      src_x, src_y); \
	...
	{                                                            \
      ...
      if ((holy_addr_t) holy_video_fb_get_video_ptr (&target, src_x, src_y)
	  % sizeof (holy_uint32_t) == 0
	  && (holy_addr_t) holy_video_fb_get_video_ptr (&target, dst_x, dst_y)
	  % sizeof (holy_uint32_t) == 0
	  && linelen % sizeof (holy_uint32_t) == 0
	  && linedelta % sizeof (holy_uint32_t) == 0)
	{
	  holy_uint32_t *src, *dst;
	  linelen /= sizeof (holy_uint32_t);
	  linedelta /= sizeof (holy_uint32_t);
	  DO_SCROLL
	}
      /* If everything is aligned on 16-bit use 16-bit copy.  */
      else if ((holy_addr_t) holy_video_fb_get_video_ptr (&target, src_x, src_y)
	       % sizeof (holy_uint16_t) == 0
	       && (holy_addr_t) holy_video_fb_get_video_ptr (&target,
							     dst_x, dst_y) 
	       % sizeof (holy_uint16_t) == 0
	       && linelen % sizeof (holy_uint16_t) == 0
	       && linedelta % sizeof (holy_uint16_t) == 0)
	{
	  holy_uint16_t *src, *dst;
	  linelen /= sizeof (holy_uint16_t);
	  linedelta /= sizeof (holy_uint16_t);
	  DO_SCROLL
	}
      /* If not aligned at all use 8-bit copy.  */
      else
	{
	  holy_uint8_t *src, *dst;
	  DO_SCROLL
	}	
    }
  ...
  if (dy > 0)
    holy_video_fb_fill_rect (color, 0, 0, framebuffer.render_target->viewport.width, dy);
  else if (dy < 0)
    {
      if (framebuffer.render_target->viewport.height < holy_abs (dy))
        dy = -framebuffer.render_target->viewport.height;

      holy_video_fb_fill_rect (color, 0, framebuffer.render_target->viewport.height + dy,
                                framebuffer.render_target->viewport.width, -dy);
    }

  /* 4b. Fill left & right parts.  */
  if (dx > 0)
    holy_video_fb_fill_rect (color, 0, 0,
                              dx, framebuffer.render_target->viewport.height);
  else if (dx < 0)
    {
      if (framebuffer.render_target->viewport.width < holy_abs (dx))
        dx = -framebuffer.render_target->viewport.width;

      holy_video_fb_fill_rect (color, framebuffer.render_target->viewport.width + dx, 0,
                                -dx, framebuffer.render_target->viewport.height);
    }
...
holy_err_t
holy_video_fb_create_render_target (struct holy_video_fbrender_target **result,
				    unsigned int width, unsigned int height,
				    unsigned int mode_type __attribute__ ((unused)))
{
  struct holy_video_fbrender_target *target;
  unsigned int size;
  ...
  target = holy_malloc (sizeof (struct holy_video_fbrender_target));
  if (! target)
    return holy_errno;
  ...
  switch (mode_type)
    {
    case holy_VIDEO_MODE_TYPE_INDEX_COLOR
      | holy_VIDEO_MODE_TYPE_ALPHA:
      target->mode_info.mode_type = holy_VIDEO_MODE_TYPE_INDEX_COLOR
	| holy_VIDEO_MODE_TYPE_ALPHA;
      target->mode_info.bpp = 8;
      target->mode_info.bytes_per_pixel = 1;
      target->mode_info.number_of_colors = 16;
      target->mode_info.blit_format = holy_VIDEO_BLIT_FORMAT_INDEXCOLOR_ALPHA;
      break;
    default:
      target->mode_info.mode_type = holy_VIDEO_MODE_TYPE_RGB
	| holy_VIDEO_MODE_TYPE_ALPHA;
      ...
      target->mode_info.blit_format = holy_VIDEO_BLIT_FORMAT_RGBA_8888;
      break;
    }
  ...
  if (mode_type == (holy_VIDEO_MODE_TYPE_INDEX_COLOR
		    | holy_VIDEO_MODE_TYPE_ALPHA))
    holy_memset (target->data, 0xf0, size);
  else
    holy_memset (target->data, 0, size);
...
holy_err_t
holy_video_fb_create_render_target_from_pointer (struct holy_video_fbrender_target **result,
						 ...
						 void *ptr)
{
  ...
  if (!(mode_info->bytes_per_pixel & (mode_info->bytes_per_pixel - 1))
      && ((holy_addr_t) ptr & (mode_info->bytes_per_pixel - 1)))
    return holy_error (holy_ERR_BAD_ARGUMENT, "unaligned pointer");
  if (!(mode_info->bytes_per_pixel & (mode_info->bytes_per_pixel - 1))
      && (mode_info->pitch & (mode_info->bytes_per_pixel - 1)))
    return holy_error (holy_ERR_BAD_ARGUMENT, "unaligned pitch");
#endif
  ...
  holy_memcpy (&(target->mode_info), mode_info, sizeof (target->mode_info));
  ...
  for (y = 0; y < mode_info->height; y++)
    holy_memset (target->data + mode_info->pitch * y, 0,
...
holy_err_t
holy_video_fb_delete_render_target (struct holy_video_fbrender_target *target)
{
...
holy_err_t
holy_video_fb_set_active_render_target (struct holy_video_fbrender_target *target)
{
  if (target == (struct holy_video_fbrender_target *)
      holy_VIDEO_RENDER_TARGET_DISPLAY)
...
holy_err_t
holy_video_fb_get_active_render_target (struct holy_video_fbrender_target **target)
{
  *target = framebuffer.render_target;

  if (*target == framebuffer.back_target)
    *target = (struct holy_video_fbrender_target *) holy_VIDEO_RENDER_TARGET_DISPLAY;
...
doublebuf_blit_update_screen (void)
{
  if (framebuffer.current_dirty.first_line
    ...
    holy_memcpy ((char *) framebuffer.pages[0]
...
static holy_err_t
holy_video_fb_doublebuf_blit_init (struct holy_video_fbrender_target **back,
				   struct holy_video_mode_info mode_info,
				   volatile void *framebuf)
{
  holy_err_t err;
  holy_size_t page_size = mode_info.pitch * mode_info.height;

  framebuffer.offscreen_buffer = holy_zalloc (page_size);
  if (! framebuffer.offscreen_buffer)
    return holy_errno;

  err = holy_video_fb_create_render_target_from_pointer (&framebuffer.back_target,
							 &mode_info,
...
doublebuf_pageflipping_update_screen (void)
{
  ...
  if (first_line <= last_line)
    holy_memcpy ((char *) framebuffer.pages[framebuffer.render_page]
...
static holy_err_t
doublebuf_pageflipping_init (struct holy_video_mode_info *mode_info,
			     ...
			     volatile void *page1_ptr)
{
  ...
  framebuffer.offscreen_buffer = holy_malloc (page_size);
  if (! framebuffer.offscreen_buffer)
    {
      return holy_errno;
    }
  ...
  if (err)
    {
      holy_video_fb_delete_render_target (framebuffer.back_target);
      return err;
    }
...
holy_err_t
holy_video_fb_setup (unsigned int mode_type, unsigned int mode_mask,
		     struct holy_video_mode_info *mode_info,
		     volatile void *page0_ptr,
		     holy_video_fb_set_page_t set_page_in,
		     volatile void *page1_ptr)
{
  ...
  if (set_page_in && holy_video_check_mode_flag (mode_type, mode_mask,
						 holy_VIDEO_MODE_TYPE_DOUBLE_BUFFERED,
						 1))
    {
      mode_info->mode_type |= holy_VIDEO_MODE_TYPE_DOUBLE_BUFFERED;
  ...
  if (holy_video_check_mode_flag (mode_type, mode_mask,
				  ...
				  1))
    {
	 ...
	 some older gcc versions don't accept it properly.*/
      ...
      err = holy_video_fb_doublebuf_blit_init (&framebuffer.back_target,
					       *mode_info,
					       tmp);
...
holy_err_t
holy_video_fb_swap_buffers (void)
{
  holy_err_t err;
...
holy_err_t
holy_video_fb_get_info_and_fini (struct holy_video_mode_info *mode_info,
				 void **framebuf)
{
  holy_memcpy (mode_info, &(framebuffer.back_target->mode_info),
	       sizeof (*mode_info));
