...
#define holy_memset memset
#define holy_memcpy memcpy
#endif
...
#ifndef STANDALONE
#ifdef TEST
typedef unsigned int holy_size_t;
...
#endif
void
holy_reed_solomon_recover (void *ptr_, holy_size_t s, holy_size_t rs)
...
#define GF_SIZE 8
typedef holy_uint8_t gf_single_t;
...
static gf_single_t
pol_evaluate (gf_single_t *pol, holy_size_t degree, int log_x)
{
  int i;
  gf_single_t s = 0;
  int log_xn = 0;
...
static void
rs_encode (gf_single_t *data, holy_size_t s, holy_size_t rs)
{
  gf_single_t *rs_polynomial;
  int i, j;
  gf_single_t *m;
  m = xmalloc ((s + rs) * sizeof (gf_single_t));
  holy_memcpy (m, data, s * sizeof (gf_single_t));
  holy_memset (m + s, 0, rs * sizeof (gf_single_t));
  rs_polynomial = xmalloc ((rs + 1) * sizeof (gf_single_t));
  holy_memset (rs_polynomial, 0, (rs + 1) * sizeof (gf_single_t));
  rs_polynomial[rs] = 1;
  /* Multiply with X - a^r */
  for (j = 0; j < rs; j++)
    {
  ...
  free (rs_polynomial);
  holy_memcpy (data + s, m + s, rs * sizeof (gf_single_t));
  free (m);
}
#endif
...
static void
rs_recover (gf_single_t *mm, holy_size_t s, holy_size_t rs)
{
  holy_size_t rs2 = rs / 2;
  int errnum = 0;
  int i, j;
...
static void
decode_block (gf_single_t *ptr, holy_size_t s,
	      gf_single_t *rptr, holy_size_t rs)
{
  int i, j;
  for (i = 0; i < SECTOR_SIZE; i++)
    {
      holy_size_t ds = (s + SECTOR_SIZE - 1 - i) / SECTOR_SIZE;
      holy_size_t rr = (rs + SECTOR_SIZE - 1 - i) / SECTOR_SIZE;
...
#if !defined (STANDALONE)
static void
encode_block (gf_single_t *ptr, holy_size_t s,
	      gf_single_t *rptr, holy_size_t rs)
{
  int i, j;
  for (i = 0; i < SECTOR_SIZE; i++)
...
{
  holy_size_t s = data_size;
  holy_size_t rs = redundancy;
  gf_single_t *ptr = buffer;
  gf_single_t *rptr = ptr + s;
  void *tmp;

  tmp = xmalloc (data_size);
  holy_memcpy (tmp, buffer, data_size);
...
#ifndef TEST
  assert (holy_memcmp (tmp, buffer, data_size) == 0);
#endif
exit:
  free (tmp);
}
...
holy_reed_solomon_recover (void *ptr_, holy_size_t s, holy_size_t rs)
{
  ...
  for (cptr = rptr + rs - 1; cptr >= rptr; cptr--)
    if (*cptr)
      break;
  if (rptr + rs - 1 - cptr > (holy_ssize_t) rs / 2)
    return;
...
main (int argc, char **argv)
{
  ...
  holy_memset (gf_powx, 0xee, sizeof (gf_powx));
  holy_memset (gf_powx_inv, 0xdd, sizeof (gf_powx_inv));
  ...
  holy_reed_solomon_add_redundancy (buf, s, rs);

  out = holy_util_fopen ("tst_rs.bin", "wb");
  fwrite (buf, 1, s + rs, out);
  fclose (out);
#else
  out = holy_util_fopen ("tst_rs.bin", "rb");
...
#endif
#if 1
  holy_memset (buf + 512 * 15, 0, 512);
#endif

  out = holy_util_fopen ("tst_dam.bin", "wb");
  fwrite (buf, 1, s + rs, out);
  fclose (out);
  holy_reed_solomon_recover (buf, s, rs);

  out = holy_util_fopen ("tst_rec.bin", "wb");
  fwrite (buf, 1, s, out);
  fclose (out);
