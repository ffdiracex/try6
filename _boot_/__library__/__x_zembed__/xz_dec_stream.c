...
struct xz_dec {
	...
	holy_uint8_t hash_id;
#endif
	holy_size_t hash_size;
	...
	int crc32)
{
		...
		&& sizeof (s->hash_value) >= hash->mdlen)
	{
		...
		holy_memcpy (s->hash_value, hash->read(hash_context),
			     hash->mdlen);
		s->have_hash_value = 1;
...
static enum xz_ret dec_stream_header(struct xz_dec *s)
{
...
#ifndef holy_EMBED_DECOMPRESSOR
	s->crc32 = holy_crypto_lookup_md_by_name ("CRC32");
	...
	if (s->crc32)
	{
		...
		holy_crypto_hash (s->crc32, computed_hash,
		...
		if (holy_memcmp (readhash, computed_hash,
				 s->crc32->mdlen) != 0)
		  return XZ_DATA_ERROR;
	}
#endif
...
#ifndef holy_EMBED_DECOMPRESSOR
		s->hash = holy_crypto_lookup_md_by_name (hashes[s->temp.buf[HEADER_MAGIC_SIZE + 1]].name);
		if (s->hash)
		{
		...
		case SEQ_INDEX_PADDING:
			...
			if (s->hash)
			{
				...
				if (s->block.hash.unpadded != s->index.hash.unpadded
				    ...
				    || holy_memcmp (s->hash->read(s->block.hash.hash_context),
