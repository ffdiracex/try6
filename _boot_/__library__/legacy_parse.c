...
static struct legacy_command legacy_commands[] =
  {
    ...
    {"displaymem", "lsmmap\n", NULL, 0, 0, {}, 0, 0, 
     "Display what holy thinks the system address space map of the"
    ...
    {"fallback", "set fallback='%s'\n", NULL, 0,
     ...
     " was rebooted by a kernel that holy loaded."},
    ...
    {"kernel", "legacy_kernel %s %s '%s' %s\n", NULL, 0,
     ...
     " \"multiboot\". The option --no-mem-option tells holy not to pass a"
    ...
    {"makeactive", "parttool \"$root\" boot+\n", NULL, 0, 0, {}, 0, 0,
     "Set the active partition on the root disk to holy's root device."
    ...
    /* NOTE: md5crypt unsupported since holy has not enough entropy and this
    ...
    {"password", "if [ \"$superusers\" = "" ]; then superusers=legacy; fi;\n"
     ...
     " FILE as a new config file and restarts the holy Stage 2. If you"
     " omit the argument FILE, then holy just unlocks privileged"
     " instructions.  You can also use it in the script section, in"
     " which case it will ask for the password, before continuing."
     " The option --md5 tells holy that PASSWD is encrypted with"
     " md5crypt."},
    {"password", "if [ \"$superusers\" = "" ]; then superusers=legacy; fi;\n"
    ...
    /* NOTE: holy2 has a design principle of not eternally waiting for user
    ...
    {"rootnoverify", "set root='%s'; set legacy_hdbias='%s'\n", NULL, 0,
     ...
     " holy can read, but setting the correct root device is still"
    ...
    {"serial", "serial %s\n", NULL, 0, 1, {TYPE_REST_VERBATIM}, 0, 
     ...
     " in the holy shell, which specifies the file name of a tty device. The"
...
char *
holy_legacy_escape (const char *in, holy_size_t len)
{
  char *ptr;
  char *ret;
  char saved;
  ...
  saved = ptr[len];
  ptr[len] = '\0';
  holy_strchrsub (ret, ptr, '\'', "'\\''");
  ptr[len] = saved;
  return ret;
}

static char *
adjust_file (const char *in, holy_size_t len)
{
  ...
  if (in[0] != '(')
    return holy_legacy_escape (in, len);
  ...
  if (*comma == ')' && comma - in == 3
	...
	if (*ptr == '\'' || *ptr == '\\')
      ...
      ret = holy_malloc (ptr - in + overhead + 15);
      if (!ret)
	return NULL;

      outptr = holy_stpcpy (ret, "(tftp)");;
  ...
  if (*comma != ',')
    return holy_legacy_escape (in, len);
  part = holy_strtoull (comma + 1, (char **) &rest, 0);
  ...
  ret = holy_malloc (ptr - in + overhead + 35 + 5);
  if (!ret)
    return NULL;
  ...
  if (subpart != -1)
    holy_snprintf (outptr, 35, "%d,%d", part + 1, subpart + 1);
  else
    holy_snprintf (outptr, 35, "%d", part + 1);
  while (*outptr)
    outptr++;
  for (ptr = rest; ptr < in + len; ptr++)
    {
...
static int
check_option (const char *a, const char *b, holy_size_t len)
{
  if (holy_strlen (b) != len)
    return 0;
  return holy_strncmp (a, b, len) == 0;
}

static int
is_option (enum arg_type opt, const char *curarg, holy_size_t len)
{
  switch (opt)
    {
...
char *
holy_legacy_parse (const char *buf, char **entryname, char **suffix)
{
  const char *ptr;
  const char *cmdname;
  unsigned i, cmdnum;
  ...
  *suffix = NULL;
  ...
  if (!*ptr || *ptr == '#')
    {
      char *ret;
      int len = holy_strlen (buf);
      ret = holy_malloc (len + 2);
      holy_memcpy (ret, buf, len);
      if (len && ret[len - 1] == '\n')
	ret[len] = 0;
      else
	{
  ...
  cmdname = ptr;
  for (ptr = buf; *ptr && !holy_isspace (*ptr) && *ptr != '='; ptr++);

  for (cmdnum = 0; cmdnum < ARRAY_SIZE (legacy_commands); cmdnum++)
    if (holy_strncmp (legacy_commands[cmdnum].name, cmdname, ptr - cmdname) == 0
  ...
  if (cmdnum == ARRAY_SIZE (legacy_commands))
    return holy_xasprintf ("# Unsupported legacy command: %s\n", buf);

  for (; holy_isspace (*ptr) || *ptr == '='; ptr++);

  if (legacy_commands[cmdnum].flags & FLAG_TITLE)
    {
      const char *ptr2;
      ptr2 = ptr + holy_strlen (ptr);
      while (ptr2 > ptr && holy_isspace (*(ptr2 - 1)))
	ptr2--;
      *entryname = holy_strndup (ptr, ptr2 - ptr);
      return NULL;
    }
  ...
  if (legacy_commands[cmdnum].flags & FLAG_TERMINAL)
      ...
      while (*ptr)
	{
	  ...
	  if (holy_memcmp (ptr, "--dumb", sizeof ("--dumb") - 1) == 0)
	    dumb = 1;
#ifdef TODO
	  if (holy_memcmp (ptr, "--no-echo", sizeof ("--no-echo") - 1) == 0)
	    no_echo = 1;

	  if (holy_memcmp (ptr, "--no-edit", sizeof ("--no-edit") - 1) == 0)
	    no_edit = 1;
#endif
	  if (holy_memcmp (ptr, "--lines=", sizeof ("--lines=") - 1) == 0)
	    {
	      lines = holy_strtoul (ptr + sizeof ("--lines=") - 1, 0, 0);
	      if (holy_errno)
		{
		  lines = 24;
	  ...
	  if (holy_memcmp (ptr, "console", sizeof ("console") - 1) == 0)
	    console = 1;

	  if (holy_memcmp (ptr, "serial", sizeof ("serial") - 1) == 0)
	    serial = 1;
	  if (holy_memcmp (ptr, "hercules", sizeof ("hercules") - 1) == 0)
	    hercules = 1;
	  if (holy_memcmp (ptr, "graphics", sizeof ("graphics") - 1) == 0)
	    graphics = 1;
	...
	return holy_strdup ("terminal_input; terminal_output; terminfo\n");

      outptr = outbuf;

      if (graphics)
	outptr = holy_stpcpy (outptr, "insmod all_video; ");

      outptr = holy_stpcpy (outptr, "terminal_input ");
      if (serial)
	outptr = holy_stpcpy (outptr, "serial ");
      if (console || hercules || graphics)
	outptr = holy_stpcpy (outptr, "console ");
      outptr = holy_stpcpy (outptr, "; terminal_output ");
      if (serial)
	outptr = holy_stpcpy (outptr, "serial ");
      if (console)
	outptr = holy_stpcpy (outptr, "console ");
      if (hercules)
	outptr = holy_stpcpy (outptr, "mda_text ");
      if (graphics)
	outptr = holy_stpcpy (outptr, "gfxterm ");
      outptr = holy_stpcpy (outptr, "; ");
      *outptr = '\0';
      if (serial)
	{
	  holy_snprintf (outptr, outbuf + sizeof (outbuf) - outptr,
			 "terminfo serial -g 80x%d %s; ",
			 lines, dumb ? "dumb" : "vt100");
	  outptr += holy_strlen (outptr);
	}

      holy_strcpy (outptr, "\n");

      return holy_strdup (outbuf);
    }

  holy_memset (args, 0, sizeof (args));

  {
    ...
    for (i = 0; i < legacy_commands[cmdnum].argc; i++)
      {
 	holy_size_t curarglen;
	if (hold_arg)
	  {
	    ptr = curarg;
	    hold_arg = 0;
	  }
	for (; holy_isspace (*ptr); ptr++);
	curarg = ptr;
	if (!*curarg)
	  break;
	...
	if (i != legacy_commands[cmdnum].argc - 1
	  ...
	  {
	    curarglen = holy_strlen (curarg);
	    while (curarglen > 0 && holy_isspace (curarg[curarglen - 1]))
	      curarglen--;
	  }
	if (*ptr)
	  ptr++;
	switch (legacy_commands[cmdnum].argt[i])
	  ...
	  case TYPE_REST_VERBATIM:
	      ...
	      while (*ptr)
		{
	      ...
	      outptr0 = args[i] = holy_malloc (overhead + (ptr - curarg));
	      if (!outptr0)
		return NULL;
	      ptr = curarg;
	      outptr = outptr0;
	  ...
	  case TYPE_VERBATIM:
	    args[i] = holy_legacy_escape (curarg, curarglen);
	    break;
	  case TYPE_WITH_CONFIGFILE_OPTION:
	  case TYPE_FORCE_OPTION:
	  case TYPE_NOAPM_OPTION:
	  ...
	  case TYPE_OPTION:
	    if (is_option (legacy_commands[cmdnum].argt[i], curarg, curarglen))
	      {
		args[i] = holy_strndup (curarg, curarglen);
		break;
	      }
	    args[i] = holy_strdup ("");
	    hold_arg = 1;
	  ...
	  case TYPE_INT:
	      ...
	      for (; *brk && brk < curarg + curarglen; brk++)
		{
	      ...
	      if (brk == curarg)
		args[i] = holy_strdup ("0");
	      else
		args[i] = holy_strndup (curarg, brk - curarg);
	    }
	    break;
	  case TYPE_VBE_MODE:
	    {
	      unsigned mod;
	      struct holy_vesa_mode_table_entry *modedesc;

	      mod = holy_strtoul (curarg, 0, 0);
	      if (holy_errno)
		{
		  mod = 0;
		  holy_errno = holy_ERR_NONE;
		}
	      if (mod < holy_VESA_MODE_TABLE_START
		  || mod > holy_VESA_MODE_TABLE_END)
		{
		  args[i] = holy_strdup ("auto");
		  break;
		}
	      modedesc = &holy_vesa_mode_table[mod - holy_VESA_MODE_TABLE_START];
	      if (!modedesc->width)
		{
		  args[i] = holy_strdup ("auto");
		  break;
		}
	      args[i] = holy_xasprintf ("%ux%ux%u",
  ...
  if (legacy_commands[cmdnum].flags & FLAG_COLOR_INVERT)
    {
      ...
      char *slash = holy_strchr (corig, '/');
      char *invert;
      holy_size_t len;

      len = holy_strlen (corig);
      if (!slash)
	{
	  holy_error (holy_ERR_BAD_ARGUMENT, N_("invalid color specification `%s'"),
		      args[0]);
	  return NULL;
	}
      invert = holy_malloc (len + 1);
      if (!invert)
	return NULL;
      holy_memcpy (invert, slash + 1, len - (slash - corig) - 1);
      invert[len - (slash - args[0]) - 1] = '/'; 
      holy_memcpy (invert + len - (slash - corig), corig, slash - corig);
      invert[len] = 0;
  ...
  if (legacy_commands[cmdnum].suffix)
    {
      *suffix = holy_xasprintf (legacy_commands[cmdnum].suffix,
  ...
  {
    char *ret = holy_xasprintf (legacy_commands[cmdnum].map, args[0], args[1],
				args[2], args[3]);
    holy_free (args[0]);
    holy_free (args[1]);
    holy_free (args[2]);
