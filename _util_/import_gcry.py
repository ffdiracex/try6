...
basedir = os.path.join (outdir, "lib/libgcrypt-holy")
try:
    os.makedirs (basedir)
except:
...
conf = codecs.open (os.path.join ("holy-core", "Makefile.gcry.def"), "w", "utf-8")
...
confutil.write ("  name = libholygcry.a;\n");
confutil.write ("  cflags = '$(CFLAGS_GCRY)';\n");
confutil.write ("  cppflags = '$(CPPFLAGS_GCRY)';\n");
confutil.write ("  extra_dist = holy-core/lib/libgcrypt-holy/cipher/ChangeLog;\n");
confutil.write ("\n");
chlog = ""
modules_sym_md = []
...
for cipher_file in cipher_files:
    ...
    if re.match (".*\.[ch]$", cipher_file):
        ...
        fw.write ("#include <holy/dl.h>\n")
        if cipher_file == "camellia.h":
            fw.write ("#include <holy/misc.h>\n")
            fw.write ("void camellia_setup128(const unsigned char *key, holy_uint32_t *subkey);\n")
            fw.write ("void camellia_setup192(const unsigned char *key, holy_uint32_t *subkey);\n")
            fw.write ("void camellia_setup256(const unsigned char *key, holy_uint32_t *subkey);\n")
            fw.write ("void camellia_encrypt128(const holy_uint32_t *subkey, holy_uint32_t *io);\n")
            fw.write ("void camellia_encrypt192(const holy_uint32_t *subkey, holy_uint32_t *io);\n")
            fw.write ("void camellia_encrypt256(const holy_uint32_t *subkey, holy_uint32_t *io);\n")
            fw.write ("void camellia_decrypt128(const holy_uint32_t *subkey, holy_uint32_t *io);\n")
            fw.write ("void camellia_decrypt192(const holy_uint32_t *subkey, holy_uint32_t *io);\n")
            fw.write ("void camellia_decrypt256(const holy_uint32_t *subkey, holy_uint32_t *io);\n")
            fw.write ("#define memcpy holy_memcpy\n")
        # Whole libgcrypt is distributed under GPLv2+ or compatible
        if isc:
            fw.write ("holy_MOD_LICENSE (\"GPLv2+\");\n")
        ...
        if len (ciphernames) > 0 or len (mdnames) > 0 or len (pknames) > 0:
            if isglue:
                modfiles = "lib/libgcrypt-holy/cipher/%s lib/libgcrypt-holy/cipher/%s" \
                    % (cipher_file, cipher_file.replace ("-glue.c", ".c"))
            else:
                modfiles = "lib/libgcrypt-holy/cipher/%s" % cipher_file
            if len (ciphernames) > 0 or len (mdnames) > 0:
                modules_sym_md.append (modname)
            chmsg = "(holy_MOD_INIT(%s)): New function\n" % modname
            if nch:
                chlognew = "%s\n	%s" % (chlognew, chmsg)
            else:
                chlognew = "%s%s" % (chlognew, chmsg)
            ...
            for ciphername in ciphernames:
                ...
                chlognew = "%s\n	%s" % (chlognew, chmsg)
                fw.write ("  holy_cipher_register (&%s);\n" % ciphername)
            for ctxsize in mdctxsizes:
                fw.write ("  COMPILE_TIME_ASSERT(%s <= holy_CRYPTO_MAX_MD_CONTEXT_SIZE);\n" % ctxsize)
            for mdname in mdnames:
                chmsg = "Register digest %s" % mdname
                chlognew = "%s\n	%s" % (chlognew, chmsg)
                fw.write ("  holy_md_register (&%s);\n" % mdname)
            for pkname in pknames:
                chmsg = "Register pk %s" % mdname
                chlognew = "%s\n	%s" % (chlognew, chmsg)
                fw.write ("  holy_crypto_pk_%s = &%s;\n"
                          % (pkname.replace ("_gcry_pubkey_spec_", ""), pkname))
            fw.write ("}")
            chmsg = "(holy_MOD_FINI(%s)): New function\n" % modname
            chlognew = "%s\n	%s" % (chlognew, chmsg)
            fw.write ("\n\nholy_MOD_FINI(%s)\n" % modname)
            fw.write ("{\n")
            for ciphername in ciphernames:
                ...
                chlognew = "%s\n	%s" % (chlognew, chmsg)
                fw.write ("  holy_cipher_unregister (&%s);\n" % ciphername)
            for mdname in mdnames:
                chmsg = "Unregister MD %s" % mdname
                chlognew = "%s\n	%s" % (chlognew, chmsg)
                fw.write ("  holy_md_unregister (&%s);\n" % mdname)
            for pkname in pknames:
                chmsg = "Unregister pk %s" % mdname
                chlognew = "%s\n	%s" % (chlognew, chmsg)
                fw.write ("  holy_crypto_pk_%s = 0;\n"
            ...
            for src in modfiles.split():
                conf.write ("  common = %s;\n" % src)
                if len (ciphernames) > 0 or len (mdnames) > 0:
                    confutil.write ("  common = holy-core/%s;\n" % src)
            if modname == "gcry_ecc":
                conf.write ("  common = lib/libgcrypt-holy/mpi/ec.c;\n")
...
for src in sorted (os.listdir (os.path.join (indir, "src"))):
    ...
    if src == "gcrypt-module.h":
        ...
        continue
    if src == "visibility.h":
        fw.write ("# include <holy/gcrypt/gcrypt.h>\n")
        fw.close ()
        continue
    f = codecs.open (infile, "r", "utf-8")
    if src == "types.h":
...
fw=codecs.open (outfile, "w", "utf-8")
fw.write ("#include <holy/crypto.h>\n")
fw.write ("#include <cipher_wrap.h>\n")
...
initfile.write ("#include <holy/crypto.h>\n")
for module in modules_sym_md:
    initfile.write ("extern void holy_%s_init (void);\n" % module)
    initfile.write ("extern void holy_%s_fini (void);\n" % module)
initfile.write ("\n")
initfile.write ("void\n")
initfile.write ("holy_gcry_init_all (void)\n")
initfile.write ("{\n")
for module in modules_sym_md:
    initfile.write ("  holy_%s_init ();\n" % module)
initfile.write ("}\n")
initfile.write ("\n")
initfile.write ("void\n")
initfile.write ("holy_gcry_fini_all (void)\n")
initfile.write ("{\n")
for module in modules_sym_md:
    initfile.write ("  holy_%s_fini ();\n" % module)
initfile.write ("}\n")
initfile.close ()

confutil.write ("  common = holy-core/lib/libgcrypt-holy/cipher/init.c;\n")
confutil.write ("};\n");
confutil.close ();
...
fw.write ("\n")
fw.write ("	Imported ciphers to holy\n")
fw.write ("\n")
fw.write (chlog)
fw.write ("\n")
for line in f:
