...
holy_PLATFORMS = [ "emu", "i386_pc", "i386_efi", "i386_qemu", "i386_coreboot",
...
GROUPS["noemu"]   = holy_PLATFORMS[:]; GROUPS["noemu"].remove("emu")
...
#
RMAP = {}
for platform in holy_PLATFORMS:
    ...
    for k in GROUPS.keys():
        ...
        for group in RMAP[platform]:
            if group in holy_PLATFORMS: smaller.append(group)
...
#    emu_condition = COND_holy_EMU_SDL;
#  };
#
...
def is_platform_independent(defn):
    ...
    for suffix in [ "", "_nodist" ]:
        template = platform_values(defn, holy_PLATFORMS[0], suffix)
        for platform in holy_PLATFORMS[1:]:
    ...
    for suffix in [ "startup", "ldadd", "dependencies", "cflags", "ldflags", "cppflags", "ccasflags", "stripflags", "objcopyflags", "condition" ]:
        template = platform_specific_values(defn, holy_PLATFORMS[0], "_" + suffix, suffix)
        for platform in holy_PLATFORMS[1:]:
            if template != platform_specific_values(defn, platform, "_" + suffix, suffix):
                return False
    for tag in [ "nostrip" ]:
        template = platform_tagged(defn, holy_PLATFORMS[0], tag)
        for platform in holy_PLATFORMS[1:]:
...
""" + name + """.marker: $(""" + cname(defn) + """_SOURCES) $(nodist_""" + cname(defn) + """_SOURCES)
	$(TARGET_CPP) -Dholy_LST_GENERATOR $(CPPFLAGS_MARKER) $(DEFS) $(DEFAULT_INCLUDES) $(INCLUDES) $(""" + cname(defn) + """_CPPFLAGS) $(CPPFLAGS) $^ > $@.new || (rm -f $@; exit 1)
...
"""if test x$(TARGET_APPLE_LINKER) = x1; then \
     $(TARGET_OBJCONV) -f$(TARGET_MODULE_FORMAT) -nr:_holy_mod_init:holy_mod_init -nr:_holy_mod_fini:holy_mod_fini -ed2022 -wd1106 -nu -nd $< $@; \
   elif test ! -z '$(TARGET_OBJ2ELF)'; then \
     $(TARGET_OBJ2ELF) $< $@ || (rm -f $@; exit 1); \
   else cp $< $@; fi""",
"""if test x$(TARGET_APPLE_LINKER) = x1; then \
  $(TARGET_STRIP) -S -x $(""" + cname(defn) + """) -o $@.bin $<; \
  $(TARGET_OBJCONV) -f$(TARGET_MODULE_FORMAT) -nr:_holy_mod_init:holy_mod_init -nr:_holy_mod_fini:holy_mod_fini -ed2022 -ed2016 -wd1106 -nu -nd $@.bin $@; \
  rm -f $@.bin; \
   elif test ! -z '$(TARGET_OBJ2ELF)'; then \
...
def script(defn, platform):
    ...
    if 'testcase' in defn:
        ...
        if 'mansection' in defn:
            manpage(defn, "holy-mkconfig_lib")
...
def rules(target, closure):
    ...
    for defn in defparser.definitions.find_all(target):
        if is_platform_independent(defn):
            under_platform_specific_conditionals(defn, holy_PLATFORMS[0], closure)
        else:
